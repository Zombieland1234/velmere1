...
export function buildVlmShieldInvestigator(
      ...
      : holderTop10 > 65
        ...
        : holderTop10 > 45
          ...
          : holderTop10 > 30
            ? 15
            : 4) +
      (hasSignal(result, ["holder_concentration", "exchange_deposit_anomaly"])
        ? 28
        : 0) +
      (lowFloat ? 10 : 0),
  );
  ...
  const missingCoreEvidence = [
    ...
    !result.agentAssessments?.some(
      (assessment) =>
        assessment.id === "microstructure" && assessment.evidenceCount > 0,
    ),
  ...
  const primaryConcern =
    lanes.slice().sort((a, b) => b.score - a.score)[0]?.label ??
    "source uncertainty";
