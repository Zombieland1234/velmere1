...
if test "$ACTUAL" = "$EXPECTED_LOCK_SHA256"; then echo 'LOCK_MATCH=true' > lock-comparison.env; else echo 'LOCK_MATCH=false' > lock-comparison.env; printf '%s  expected-r44p4-package-lock.json\n' "$EXPECTED_LOCK_SHA256" > expected-lock-sha256.txt; fi
rm -rf node_modules
...
node - <<'NODE'
const fs=require('node:fs');const lock=JSON.parse(fs.readFileSync('package-lock.json','utf8'));const rows=[];for(const [path,meta] of Object.entries(lock.packages||{}))if(path&&meta?.hasInstallScript)rows.push({path,version:meta.version||null,resolved:meta.resolved||null,integrity:meta.integrity||null});rows.sort((a,b)=>a.path.localeCompare(b.path));fs.writeFileSync('lifecycle-script-packages.json',JSON.stringify({count:rows.length,rows},null,2)+'\n');
NODE
npx playwright install chromium
node - <<'NODE'
const fs=require('node:fs'),crypto=require('node:crypto');const{chromium}=require('playwright');(async()=>{const exe=chromium.executablePath(),b=fs.readFileSync(exe),browser=await chromium.launch({headless:true}),version=browser.version();await browser.close();fs.writeFileSync('chromium-identity.json',JSON.stringify({playwrightVersion:require('playwright/package.json').version,executablePath:exe,executableSha256:crypto.createHash('sha256').update(b).digest('hex'),browserVersion:version},null,2)+'\n');})().catch(e=>{console.error(e);process.exit(1)});
NODE
...
actual=Path('package-lock-sha256.txt').read_text().split()[0]; expected=os.environ['EXPECTED_LOCK_SHA256']
