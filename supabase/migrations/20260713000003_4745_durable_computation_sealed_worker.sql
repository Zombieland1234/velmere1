...
begin
  ...
  if p_sealed_payload is not null then
    ...
    if p_kind <> 'vlm_analysis' then raise exception 'sealed_payload_kind_not_enabled'; end if;
...
begin
  ...
  if p_lease_token is null or length(p_lease_token)<24 or length(p_lease_token)>120 then raise exception 'invalid_lease_token'; end if;
  if p_kinds is null or array_length(p_kinds,1) is null or not (p_kinds <@ array['vlm_analysis']::text[]) then raise exception 'invalid_worker_kinds'; end if;
