...
// Preserve caller cancellation and cap each upstream request.
const boundedFetch: typeof fetch = (input, init) => {
  const original = init?.signal ?? (input instanceof Request ? input.signal : undefined);
  const timeout = AbortSignal.timeout(8000);
  return fetch(input, { ...init, signal: original ? AbortSignal.any([original, timeout]) : timeout });
};
...
Deno.serve(async(req:Request)=>{
 try {
  ...
  if(keys.length!==expected.length||keys.some((k,i)=>k!==expected[i])||body.schemaVersion!=="velmere.product-entitlement-bridge-request.v1"||body.action!=="resolve"||body.productSlug!=="browser"||typeof body.requiredTier!=="string"||!["pro","advanced"].includes(body.requiredTier)) return reply(400,{ok:false,error:"request_invalid"});

  const url=Deno.env.get("SUPABASE_URL"),anon=Deno.env.get("SUPABASE_ANON_KEY"),service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if(!url||!anon||!service) return reply(503,{ok:false,error:"server_environment_unavailable"});
  const caller=createClient(url,anon,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false},global:{fetch:boundedFetch,headers:{Authorization:`Bearer ${token}`}}});
  ...
  const resolved=await admin.rpc("velmere_r7_resolve_browser_paid_entitlement_v1",{
    ...
    p_required_tier:body.requiredTier,
  ...
  return reply(200,{ok:true,schemaVersion:"velmere.product-entitlement-bridge-response.v1",allowed:resolved.data===true,productSlug:"browser",requiredTier:body.requiredTier,rawCapabilityReturned:false,serviceRoleReturned:false});
   ...
   // Never return SDK messages, stack traces, provider payloads or credentials.
   return reply(503,{ok:false,error:"entitlement_service_unavailable"});
 }
});
