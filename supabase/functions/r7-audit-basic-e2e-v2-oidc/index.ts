...
Deno.serve(async (request: Request) => {
  ...
  if (raw === null) return respond(400, { ok: false, error: "invalid_body" });
  ...
  if (body.action === "cleanup") {
    ...
    for (const userId of userIds) {
      try {
        ...
        // best-effort cleanup; log only in development
        if (Deno.env.get("VELMERE_OIDC_LOG")) {
          console.warn("cleanup_user_failed", { userId, error: String(cleanupError) });
        }
      }
    }
  ...
  try {
    ...
    for (const userId of createdIds) {
      try { await admin.auth.admin.deleteUser(userId); } catch (rollbackError) {
        if (Deno.env.get("VELMERE_OIDC_LOG")) {
          console.warn("rollback_user_failed", { userId, error: String(rollbackError) });
        }
      }
    }
