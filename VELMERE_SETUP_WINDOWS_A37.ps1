...
Write-Host "Registry configuration: redacted (run npm config get registry locally if diagnosis requires the host)"
...
if ($npmVersion -ne $RequiredNpm) {
  ...
  try {
    ...
    & npm.cmd install --global "npm@$RequiredNpm" --registry=https://registry.npmjs.org/ --fetch-retries=5 --fetch-retry-mintimeout=20000 --fetch-retry-maxtimeout=120000
...
& node.exe scripts/a37-install-diagnostics.mjs
...
for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
  $log = Join-Path $logDir "a37-npm-ci-attempt-$attempt.log"
