...
type DeliveryInput = Parameters<typeof buildPass2587ServerPaymentAccountDeliveryGateReport>[0];
type QaInput = Parameters<typeof buildPass2594AuditEvidenceQaReleaseGateMatrixReport>[0];
type FinalDeliveryInput = Parameters<typeof buildPass2376FinalDeliveryGate>[0];
const operatorBlocked={target:{chain:"ethereum",contractAddress:"0x1111111111111111111111111111111111111111",projectName:"P77"},summary:{canCustomerDeliverAdvanced:false,canFinalSignAdvanced:false,finalSignReadiness:0,operatorConsoleReadiness:0,critical:true,nextCriticalStep:"human lane intentionally unavailable"}} as DeliveryInput["advancedOperatorConsoleMerge"];
const paid={ok:true,ledgerMode:"memory",entitlement:{id:"ent-p77",auditQueueId:"AUD-P77Q01"}} as DeliveryInput["paidAccessReceipt"];
const account={id:"msg-p77-0001",requestId:"req-p77-0001",accountId:"acct-p77",auditQueueId:"AUD-P77Q01",deliveryStatus:"analysis_queue"} as DeliveryInput["accountMessage"];
...
const qa=buildPass2594AuditEvidenceQaReleaseGateMatrixReport({locale:"en",serverPaymentAccountDeliveryGate:delivery,advancedOperatorConsoleMerge:operatorBlocked} as QaInput);
...
const valueBlocked=buildPass4643AuditTierValueProof({automatedFinalDeliveryReady:false,operatorFinalSignReady:true} as Parameters<typeof buildPass4643AuditTierValueProof>[0]);
const valueAutomated=buildPass4643AuditTierValueProof({automatedFinalDeliveryReady:true,operatorFinalSignReady:false} as Parameters<typeof buildPass4643AuditTierValueProof>[0]);
...
const stale=await gate("pdf_attached",{...ledgerBase,lastEndpointPingAgeMinutes:99} as FinalDeliveryInput["routeHealthLedger"]);if(stale.canDeliver || !stale.reasons.some((r)=>r.key==="fresh_endpoint_ping_required")) throw new Error("p77_fresh_ping_gate_weakened");
const blockedWarning={key:"blocked-fixture",level:"blocked",summary:"blocked",nextAction:"fix"};const warned=await gate("pdf_attached",{...ledgerBase,customerDeliveryAllowed:false,deliveryWarningLevel:"blocked",warnings:[blockedWarning]} as FinalDeliveryInput["routeHealthLedger"]);if(warned.canDeliver) throw new Error("p77_warning_gate_weakened");
const noIdentity=await buildPass2376FinalDeliveryGate({locale:"en",message:{...messageBase,id:undefined,requestId:undefined,operatorStatus:"customer_safe_ready"} as FinalDeliveryInput["message"],routeHealth,routeHealthLedger:ledgerBase,staleAfterMinutes:15});if(noIdentity.canDeliver) throw new Error("p77_identity_gate_weakened");
