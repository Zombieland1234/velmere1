import { describe, it, expect } from "../../test/test-utils";
...
describe("AI Clients and Auditors 30-Scenario Tier Matrix", () => {
  it("verifies 30 AI client personas have correctly assigned tier access and value propositions", () => {
    expect(AI_CLIENT_PERSONAS).toHaveLength(30);
    ...
    expect(basicTruth.publicPriceLabel).toBe("0 PLN / mc");
    expect(proTruth.publicPriceLabel).toBe("14.99 € / mc");
    expect(advancedTruth.publicPriceLabel).toBe("49.99 € / mc");

    for (const client of AI_CLIENT_PERSONAS) {
      if (client.tier === "basic") {
        expect(basicTruth.freeAccessGuaranteed).toBe(true);
      } else if (client.tier === "pro") {
        expect(proTruth.commercialTarget).toBe("GO_PAID");
        expect(proTruth.boundaries).toContain("symulacja poślizgu VWAP");
      } else if (client.tier === "advanced") {
        expect(advancedTruth.commercialTarget).toBe("GO_PAID");
        expect(advancedTruth.boundaries).toContain("analiza mikrostruktury L3");
      }
    }
  });

  it("verifies runVlmAnalysis produces valid reports for Basic, Pro, and Advanced across sample tokens", async () => {
    const assets = ["BTC", "ETH", "SOL", "USDC", "PEPE"];

    for (const asset of assets) {
      const assetObj = { symbol: asset, name: asset, priceLabel: "100.00 USD" };
      const basicResult = await runVlmAnalysis(assetObj, "basic");
      expect(basicResult.tier).toBe("basic");
      expect(basicResult.signals.length).toBeGreaterThanOrEqual(10);

      const proResult = await runVlmAnalysis(assetObj, "pro", { entitlementVerified: true });
      expect(proResult.tier).toBe("pro");
      expect(proResult.signals.length).toBeGreaterThanOrEqual(14);

      const advancedResult = await runVlmAnalysis(assetObj, "advanced", { entitlementVerified: true });
      expect(advancedResult.tier).toBe("advanced");
      expect(advancedResult.signals.length).toBeGreaterThanOrEqual(20);
    }
  });

  it("verifies Market Impact and Whale Watch packets reflect tier capabilities", () => {
    ...
    const basicImpact = buildMarketImpactTierPacket(sampleImpact as any, "basic");
    const proImpact = buildMarketImpactTierPacket(sampleImpact as any, "pro");
    const advancedImpact = buildMarketImpactTierPacket(sampleImpact as any, "advanced");

    expect(basicImpact.tier).toBe("basic");
    expect(proImpact.tier).toBe("pro");
    expect(advancedImpact.tier).toBe("advanced");

    const basicWhale = buildWhaleWatchTierPacket(sampleWhale as any, "basic");
    const proWhale = buildWhaleWatchTierPacket(sampleWhale as any, "pro");
    const advancedWhale = buildWhaleWatchTierPacket(sampleWhale as any, "advanced");

    expect(basicWhale.tier).toBe("basic");
    expect(proWhale.tier).toBe("pro");
    expect(advancedWhale.tier).toBe("advanced");
  });
});