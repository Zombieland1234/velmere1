...
const workerPath = path.resolve(__dirname, "../../scripts/pass36/a102r44p9-local-e2e-worker.mjs");
const root = fs.mkdtempSync(path.join(os.tmpdir(), "velmere-r44p9-e2e-"));
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && url.pathname === "/intake") {
      ...
      const job = {
        schemaVersion: "velmere.pass36.a102r44p9.local-e2e.job.v1",
        id,
...
try {
  ...
  const receipt = {
    schemaVersion: "velmere.pass36.a102r44p9.real-local-e2e.v1",
