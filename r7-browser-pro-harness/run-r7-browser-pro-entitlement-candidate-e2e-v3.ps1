...
try {
  ...
  $TokenResponse = Invoke-RestMethod -Method Get -Uri $TokenUri -Headers @{ Authorization = 'Bearer ' + $env:ACTIONS_ID_TOKEN_REQUEST_TOKEN } -TimeoutSec 20
  ...
  $Driver = Join-Path $Root 'r7-browser-pro-harness/r7-browser-pro-candidate-e2e.mts'
  & node $Tsx $Driver
      ...
      if (Test-Path -LiteralPath $ReceiptPath -PathType Leaf) {
        ...
        foreach ($SnapshotId in @([string]$Receipt.proArtifactId, [string]$Receipt.basicArtifactId)) {
          if (-not [string]::IsNullOrWhiteSpace($SnapshotId)) {
            ...
            catch { Write-Warning 'Browser Pro artifact cleanup failed for one snapshot.' }
          }
        }
      }
