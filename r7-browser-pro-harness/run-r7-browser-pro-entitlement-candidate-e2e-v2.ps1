...
try {
  ...
  Require ($Provision.Status -eq 200 -and $Provision.Body.ok -eq $true) (
    'browser_pro_provision_failed:' + $Provision.Raw.Substring(0, [Math]::Min(500, $Provision.Raw.Length))
  )
  ...
  foreach ($Secret in @([string]$A.accessToken, [string]$B.accessToken, $BrowserCapability, $EntitlementCapability)) {
  ...
  foreach ($Name in @(
    'VELMERE_TRUSTED_ACCOUNT_HEADER_HMAC_SECRET_CURRENT',
    'VELMERE_LENS_SOURCE_TOKEN_SECRET_CURRENT',
    'VELMERE_LENS_RENDER_TOKEN_SECRET_CURRENT',
    'VELMERE_PROVIDER_RECEIPT_SIGNING_SECRET'
  )) {
    Write-Host "::add-mask::$([Environment]::GetEnvironmentVariable($Name))"
  }

  $ArtifactDirectory = Join-Path $Work 'artifacts/r7/browser-pro'
  New-Item -ItemType Directory -Force -Path $ArtifactDirectory | Out-Null
  $Stdout = Join-Path $ArtifactDirectory 'R7_BROWSER_PRO_NEXT_STDOUT.log'
  $Stderr = Join-Path $ArtifactDirectory 'R7_BROWSER_PRO_NEXT_STDERR.log'
  ...
  $NextProcess = Start-Process \
    -FilePath 'node.exe' \
    -ArgumentList @($NextBin, 'dev', '--webpack', '-p', '3100') \
  ...
  for ($Index = 0; $Index -lt 120; $Index += 1) {
    ...
    if ($NextProcess.HasExited) { throw "browser_pro_next_exited:$($NextProcess.ExitCode)" }
    try {
  ...
  & node \
    (Join-Path $Work 'node_modules/tsx/dist/cli.mjs') \
    (Join-Path $Root 'r7-browser-pro-harness/r7-browser-pro-candidate-e2e.mts')
  if ($LASTEXITCODE -ne 0) { throw "browser_pro_driver_failed:$LASTEXITCODE" }

  $ReceiptPath = Join-Path $ArtifactDirectory 'R7_BROWSER_PRO_ENTITLEMENT_CANDIDATE_E2E.json'
  ...
  Get-Content -LiteralPath (Join-Path $Work 'artifacts/r7/browser-pro/R7_BROWSER_PRO_NEXT_STDOUT.log') -Tail 240 -ErrorAction SilentlyContinue
  Get-Content -LiteralPath (Join-Path $Work 'artifacts/r7/browser-pro/R7_BROWSER_PRO_NEXT_STDERR.log') -Tail 240 -ErrorAction SilentlyContinue
  throw
}
finally {
        ...
        foreach ($SnapshotId in @([string]$Receipt.proArtifactId, [string]$Receipt.basicArtifactId)) {
          if (-not [string]::IsNullOrWhiteSpace($SnapshotId)) {
            try {
              Invoke-JsonPost \
                $HelperUrl \
                @{ Authorization = 'Bearer ' + $Oidc; Accept = 'application/json' } \
                @{ action = 'backup_erase'; userId = $UserIds[0]; snapshotId = $SnapshotId } | Out-Null
            } catch {
              Write-Warning 'Browser Pro artifact cleanup failed for one snapshot.'
            }
          }
        }
      }
      $Cleanup = Invoke-JsonPost \
        $HelperUrl \
        @{ Authorization = 'Bearer ' + $Oidc; Accept = 'application/json' } \
        @{ action = 'cleanup'; userIds = $UserIds; entitlementIds = $EntitlementIds }
      Write-Host "Browser Pro ephemeral cleanup: users=$($Cleanup.Body.deleted), entitlements=$($Cleanup.Body.revoked)"
    } catch {
      Write-Warning 'Browser Pro cleanup did not fully confirm.'
    }
  }
}
