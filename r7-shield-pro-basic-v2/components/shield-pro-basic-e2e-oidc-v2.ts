...
const AUDIENCE = "velmere-r7-shield-pro-basic-v2-e2e";
const HEADERS = {
...
Deno.serve(async (request: Request) => {
  ...
  if (capability.error || typeof capability.data !== "string" || capability.data.length < 48) return respond(503, { ok: false, error: "shield_pro_basic_capability_unavailable" });

  return respond(200, {
    ok: true,
    schemaVersion: "velmere.r7.shield-pro-basic-v2-e2e-capability.v1",
    runId: identity.runId,
    headSha: identity.headSha,
    shieldProBasicServerCapability: capability.data,
