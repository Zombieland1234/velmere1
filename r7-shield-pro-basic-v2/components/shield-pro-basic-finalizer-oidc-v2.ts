...
const WORKFLOW = "R7 Shield Pro Basic Customer Terminal E2E";
const WORKFLOW_FILE = ".github/workflows/r7-shield-pro-basic-customer-e2e-v2.yml";
const WORKFLOW_PATH = "/.github/workflows/r7-shield-pro-basic-customer-e2e-v2.yml@";
const AUDIENCE = "velmere-r7-shield-pro-basic-v2-finalizer";
const GH_HEADERS = { accept: "application/vnd.github+json", "user-agent": "velmere-r7-shield-pro-basic-v2-finalizer", "x-github-api-version": "2022-11-28" };
...
async function verify(token: string) {
  ...
  const header = json(segments[0]!);
  const claims = json(segments[1]!);
...
async function githubJson(path: string) {
  const response = await fetch(`https://api.github.com/repos/${REPO}${path}`, { headers: GH_HEADERS, signal: AbortSignal.timeout(12000) });
  if (!response.ok) throw new Error(`github_${response.status}`);
  return await response.json();
}
async function raw(headSha: string, path: string) {
  const response = await fetch(`https://raw.githubusercontent.com/${REPO}/${headSha}/${path}`, { headers: { "user-agent": "velmere-r7-shield-pro-basic-v2-finalizer" }, signal: AbortSignal.timeout(12000) });
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const e2e = jobs.jobs?.find((job) => job.name === "shield-pro-basic-customer-e2e");
    if (!e2e || e2e.conclusion !== "success") throw new Error("e2e_not_success");

    let artifact: Record<string, unknown> | undefined;
    for (let attempt = 0; attempt < 12 && !artifact; attempt += 1) {
      const artifacts = await githubJson(`/actions/runs/${identity.runId}/artifacts?per_page=100`) as { artifacts?: Record<string, unknown>[] };
      artifact = artifacts.artifacts?.find((item) => item.name === `r7-shield-pro-basic-v2-customer-e2e-${identity.headSha}-${identity.runId}`);
    ...
    const bridgeBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/components/shield-pro-basic-public-bridge-v2.ts");
    const helperBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/components/shield-pro-basic-e2e-oidc-v2.ts");
    const finalizerBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/components/shield-pro-basic-finalizer-oidc-v2.ts");
    const dataPlaneBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/sql/shield-pro-basic-data-plane-v2.sql");
    ...
    const finalization = await admin.rpc("velmere_r7_finalize_shield_pro_basic_v2", {
      p_github_run_id: identity.runId,
      p_github_sha: identity.headSha,
      p_workflow_sha256: await sha256(workflowBytes),
      p_artifact_digest_sha256: artifact.digest.slice(7),
      p_bridge_digest_sha256: await sha256(bridgeBytes),
      p_oidc_helper_digest_sha256: await sha256(helperBytes),
      p_finalizer_edge_digest_sha256: await sha256(finalizerBytes),
      p_data_plane_sql_digest_sha256: await sha256(dataPlaneBytes),
    });
    if (finalization.error) throw new Error(`rpc_${finalization.error.code ?? "failed"}_${finalization.error.message}`);
    const ledger = await admin.from("velmere_r7_customer_final_ledger").select("product_ordinal,product_slug,final_status,finalized_at,evidence").eq("product_slug", "shield-pro-basic").maybeSingle();
    if (ledger.error || !ledger.data || ledger.data.product_ordinal !== 10 || ledger.data.final_status !== "FINAL") throw new Error("ledger_reread_failed");

    return respond(200, { ok: true, schemaVersion: "velmere.r7.shield-pro-basic-v2-finalizer.v1", runId: identity.runId, headSha: identity.headSha, artifactId: artifact.id, artifactDigest: artifact.digest, result: finalization.data, ledgerReRead: ledger.data, serviceRoleReturned: false });
  } catch (error) {
    return respond(503, { ok: false, error: error instanceof Error ? error.message : "shield_pro_basic_finalization_failed" });
  }
});
