...
const GH_HEADERS = { accept: "application/vnd.github+json", "user-agent": "velmere-r7-shield-pro-basic-v3-finalizer", "x-github-api-version": "2022-11-28" };
...
async function raw(headSha: string, path: string) {
  const response = await fetch(`https://raw.githubusercontent.com/${REPO}/${headSha}/${path}`, { headers: { "user-agent": "velmere-r7-shield-pro-basic-v3-finalizer" }, signal: AbortSignal.timeout(12000) });
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const finalizerBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/components/shield-pro-basic-finalizer-oidc-v3.ts");
    const dataPlaneBytes = await raw(identity.headSha, "r7-shield-pro-basic-v2/sql/shield-pro-basic-data-plane-v3.sql");
    ...
    return respond(200, { ok: true, schemaVersion: "velmere.r7.shield-pro-basic-v3-finalizer.v1", runId: identity.runId, headSha: identity.headSha, artifactId: artifact.id, artifactDigest: artifact.digest, result: finalization.data, ledgerReRead: ledger.data, serviceRoleReturned: false });
  } catch (error) {
