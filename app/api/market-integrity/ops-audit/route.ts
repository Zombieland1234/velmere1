...
export async function GET(request: Request) {
  ...
  try {
    ...
    const opsAudit = buildProductOpsAudit(result, {
    ...
    return NextResponse.json({
      ...
      token: result.token,
      opsAudit,
      legalNote: "PASS62 ops audit is product-operational QA only. Not financial advice. Algorithmic risk flag only. Not legal proof or an accusation.",
      generatedAt: new Date().toISOString(),
    });
  } catch (error) {
    return NextResponse.json<ErrorPayload>(
      { mode: "error", error: error instanceof Error ? error.message : "Product ops audit generation failed" },
      { status: 502 },
    );
  }
}
