import { MARKET_INTEGRITY_VLM_ROUTES } from "@/lib/server/route-registries/market-integrity-vlm";
...
async function dispatch(method: "GET" | "POST", request: Request, context: { params: Promise<{ operation: string }> }) {
  ...
  return dispatchLazyRoute({
    ...
    key: operation,
    request,
    registry: MARKET_INTEGRITY_VLM_ROUTES,
    unknownError: "unknown_vlm_route",
    unavailableError: "vlm_route_temporarily_unavailable",
  });
}
...
export async function OPTIONS(_request: Request, context: { params: Promise<{ operation: string }> }) {
  const { operation } = await context.params;
  return optionsLazyRoute({ key: operation, registry: MARKET_INTEGRITY_VLM_ROUTES, unknownError: "unknown_vlm_route" });
}
