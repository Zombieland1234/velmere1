...
import { claimProAuditWorkerLease, PASS4616_AUDIT_REVIEW_ORCHESTRATION_BOUNDARY, PASS4616_AUDIT_REVIEW_ORCHESTRATION_ID } from "@/lib/security/audit-review-orchestration";
import {
...
export async function POST(request: Request) {
  ...
  const rate = await applyApiRateLimit(request, { keyPrefix: "pass4616-pro-worker-claim", limit: 30, windowMs: 60_000 });
  ...
  const result = await claimProAuditWorkerLease({
