...
export async function POST(request: Request) {
  ...
  const rate = await applyApiRateLimit(request, { keyPrefix: "audit-basic-worker-claim", limit: 30, windowMs: 60_000 });
  ...
  const claimRequestId = typeof body.claimRequestId === "string" ? body.claimRequestId.trim() : "";
  if (!/^[a-zA-Z0-9:_-]{8,96}$/.test(claimRequestId)) return securityJson({ ok: false, error: "invalid_claim_request_id" }, { status: 400 });
  ...
  if (!Number.isInteger(requestedLeaseSeconds) || requestedLeaseSeconds < 60 || requestedLeaseSeconds > 900) {
    return securityJson({ ok: false, error: "invalid_lease_seconds", acceptedRange: [60, 900] }, { status: 400 });
  }
  ...
  const result = await claimBasicAuditWorkerLease({
  ...
  return securityJson({
    ok: result.ok,
    result,
    leaseTokenAccepted: result.ok,
    leaseTokenBoundary: "The signed/MFA-bound worker supplies and retains one canonical 256-bit base64url token for idempotent retry; only SHA-256 is persisted.",
