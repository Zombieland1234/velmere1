...
ROOT=Path('/mnt/data/velmere_recover/p68_windows_pdfs')
REND=Path('/mnt/data/velmere_recover/p68_windows_pdf_renders')
OUT=Path('artifacts/closure/p68/receipts/P68_WINDOWS_AUDIT_CUSTOMER_PDF_QA.json')
...
receipt={'schemaVersion':'velmere.p68.audit-customer-pdf-qa.v1','status':'PASS' if not failures else 'FAIL','truthClass':'EXACT_WINDOWS_FIXTURE_CROSS_PLATFORM_QA_ONLY','pdfs':len(rows),'pages':total_pages,'failures':failures,'cases':rows,'customerFinalOutputCredit':0,'auditFinalCustomerPdfCredit':0,'rightsCredit':0,'saleCredit':0,'live':False,'truthBoundary':'Cross-platform physical PDF QA of exact-Windows synthetic P68 audit customer snapshots only. A4, embedded subset fonts with ToUnicode, text binding, no active root content/annotations, and rendered-edge integrity do not promote fixture PDFs to final customer artifacts.'}
