# VELMÈRE MASTER BUILD MAP — PASS351

## Brutalny stan po PASS351
Publiczny prototyp traktować jako ok. 20% finalnego produktu. PASS351 nie udaje, że live provider data jest gotowe: naprawia kolejne realne blokery użyteczności z Real Markets i Orbit 360. Real Markets przestał eksponować PASS/debug version w hero i dostał bardziej publiczny reader: karta tłumaczy po ludzku „co to znaczy” oraz „czego brakuje”, a crypto zostało schowane niżej jako osobna referencja Shield. Orbit drawer dostał nowy router scrolla: gdy kursor/palec jest w scroll-frame, przeglądarka robi natywny scroll; gdy kursor jest na paddingu/panelu, delta jest kierowana do wewnętrznego frame, żeby body/orbit nie przejmował przewijania.

## Co PASS351 naprawia
1. Real Markets hero nie pokazuje już publicznie `PASS350` ani `radar.version`.
2. Real Markets ma `data-pass351-clean-public-reader="true"` i publiczny tytuł `Velmère Real Markets`.
3. Każda karta instrumentu ma `data-pass351-real-market-card="true"`.
4. Każda karta ma nową linię readera: `Co to znaczy` + `Brakuje`.
5. Karty nie zaczynają już od surowego `Price lane`, tylko od prostszego `Market lane`.
6. Długi opis assetu idzie do details/foot, front zostaje krótszy.
7. Crypto reference jest osobnym, collapsed blokiem niżej: BTC/ETH/BNB/USDT nie miesza się z FX/stock/ETF.
8. Exchange Stability dostało pojedynczą human-line per karta, bez powtarzania stability explanation wszędzie.
9. Second Source dostał marker public boundary PASS351 i zostaje prostym explain systemem.
10. Orbit drawer ma `data-pass351-orbit-native-scroll-router="true"`.
11. Scroll-frame ma `data-pass351-native-scroll-zone="true"`.
12. Wheel wewnątrz scroll-frame jest natywny, a body/orbit leak jest stopowany.
13. Wheel na paddingu/panelu ręcznie przewija scroll-frame i robi preventDefault.
14. Touch wewnątrz scroll-frame idzie natywnie; touch poza nim jest kierowany do frame.
15. CSS zmienia PASS351 drawer na `touch-action: pan-y`, a nie wymuszone `none` dla nowego route.
16. Dodany guard `verify:pass351-clean-reader-orbit-scroll-router`.
17. Zachowany guard PASS350 przez compatibility marker bez psucia JSX.
18. `check:i18n` przechodzi.
19. `typecheck` przechodzi przez zmiany składniowe PASS351, ale nadal blokuje brak zależności/typów w paczce eksportowej.

## A-M progres po PASS351
...
| B | Brand/luxury public UI | P1 | 34% | Mniej debug/PASS copy, ale public language cleanup nadal trwa. |
| C | VLM Shield token surface | P0 | 33% | Full CoinGecko cache/token index/logos nadal P0. |
| D | VLM AI Brain / Orbit 360 | P0 | 44% | PASS351 poprawia router scrolla, ale potrzebny real browser QA/focus trap/ESC. |
...
| I | Real Markets / Cross-Asset | P0 | 48% | UI dużo czytelniejszy; provider data/cache/API keys nadal brak. |
| J | Interaction / scroll / performance | P0 | 45% | Nowy native scroll router jest lepszy, ale mobile/touchpad QA nadal brak. |
| K | AI bot / copy engine | P0 | 40% | Per-asset copy lepsze na froncie, ale pełne templates nadal brak. |
| L | Legal / compliance | P0 | 32% | Boundaries lepsze, final legal review nadal brak. |
...
## Następny kierunek PASS352
1. Focus trap + ESC close + scroll lock registry dla Orbit/search/modal.
