# VELMÈRE MASTER BUILD MAP — PASS349

## Brutalny stan po PASS349
Publiczny prototyp nadal traktować jako ok. 18% finalnego produktu. PASS349 docina Real Markets Reader Pro do czystych kart jak Shield: provider contract, source lanes, exchange metrics i second-source flow są schowane w kontrolowanych rozwinięciach. Orbit drawer dostaje osobny scroll-frame sandbox, żeby wheel/touch nie uciekał do body ani orbitu. Nadal P0: live adaptery, durable ledger, PDF renderer, security production, browser QA i pełny typecheck z zależnościami.

## Co PASS349 naprawia
1. Real Markets nie pokazuje już wszystkiego naraz: source lanes i next adapter step są w `details`, więc karta wygląda jak premium Shield, nie tabela/debug.
2. Provider contract nie zajmuje góry strony — jest collapsed i można go rozwinąć dopiero gdy ktoś chce technikalia.
3. Exchange Stability pokazuje score + opis, a głębokie metryki Depth/Withdrawals/Volume/Reserves/API/Social są pod jednym `Adapter checks`.
4. Second Source ma ludzki opis na froncie, a Primary/Second/Missing jest pod jednym rozwinięciem.
5. Real Markets dostał PASS349 guard na brak bocznego przesuwania i cleaner reader flow.
6. Orbit drawer dostał `shield-vlm-detail-scroll-frame-pass349`: scrolluje wewnętrzny frame, nie body i nie orbit layer.
7. Public drawer trim ukrywa ciężkie operatorowe dumpy: report capsule, handoff, release cockpit, durable snapshot i checklisty.
8. Drawer wysuwa się wolniej 1620ms i ma `overscroll-behavior: contain`.
9. CSS dodaje eleganckie rozwijane sekcje z małą animacją reveal, bez rozpychania kart.
10. Dodany guard `verify:pass349-clean-reader-orbit-scrollframe`.
11. `check:i18n`, PASS348 guard i PASS349 guard przechodzą.
12. PASS344-PASS348 guard compatibility zachowana.

## A-M progres po PASS349
...
| B | Brand/luxury public UI | P1 | 31% | Real Markets bardziej premium, ale część publicznych miejsc nadal ma techniczny język. |
| C | VLM Shield token surface | P0 | 33% | Full CoinGecko cache i token index nadal P0. |
| D | VLM AI Brain / Orbit 360 | P0 | 40% | Drawer ma scroll-frame sandbox, ale potrzebny real browser QA i focus trap. |
...
| I | Real Markets / Cross-Asset | P0 | 42% | Reader Pro jest dużo czystszy, ale provider data/cache/API keys nadal brak. |
| J | Interaction / scroll / performance | P0 | 41% | Orbit scroll-frame działa statycznie, ale potrzebny real browser profiler i mobile QA. |
| K | AI bot / copy engine | P0 | 38% | Second Source copy lepsze, ale per-asset copy dla wszystkich rynków nadal niedokończone. |
...
2. P0: PASS349 scroll-frame wymaga real browser QA na myszce, touchpadzie i mobile — statyczny guard to nie pełny dowód.
3. P0: Real Markets musi dostać live/cache/provider data, bo obecnie UI jest profesjonalny, ale dane nadal preview.
4. P0: Public Real Markets cards nie mogą wrócić do poziomych tabel ani debug/provider ściany.
5. P0: Orbit drawer focus trap/keyboard still missing.
6. P0: PDF renderer v6 i visual regression nadal P0.
7. P0: Shield search full CoinGecko cache nadal P0.
8. P0: Security headers/safe harbor/storage nadal P0.
9. P0: AI Brain tile copy musi pozostać unique per lane, bez operator dumpów publicznie.
10. P0: Logo/cache fallback dla wszystkich assetów nadal P0.
11. P0: Full typecheck runtime po zainstalowaniu zależności nadal wymagany.
12. P0: Durable source ledger nadal brak.
...
PASS349: Orbit focus trap/ESC/mobile QA + drawer read-mode accordion.
