# VELMÈRE MASTER BUILD MAP — PASS346

## Brutalny stan po PASS346
Publiczny prototyp nadal traktować jako ok. 15% finalnego produktu. PASS346 poprawia najbardziej widoczny bałagan Real Markets i wzmacnia scroll Orbit drawer, ale live adaptery, durable source ledger, finalny PDF renderer, browser QA i security production nadal są krytyczne.

## Co PASS346 naprawia
1. Real Markets nie jest już publicznie szeroką tabelą do przesuwania w bok.
2. Real Markets ma responsywne karty assetów jak Shield: logo, symbol, class, price lane, risk, mini chart, AI note i source lanes.
3. Stock/ETF/proxy logotypy mają realną mapę URL: Apple, Nvidia, Microsoft, Coinbase, JPMorgan, LVMH, SPY, QQQ, VNQ, GLD.
4. FX/commodities mają deterministic monogram fallback: EUR/USD, EUR/PLN, USD/PLN, USD/JPY, DXY, XAU, XAG, WTI, BRENT.
5. Exchange Stability jest kartami, nie szeroką tabelą.
6. Objaśnienie `Stability: wyżej = lepiej` jest tylko w nagłówku, nie powtarza się przy każdym venue.
7. Second Source Divergence jest reader-card flow: Primary → Second source → Missing before confidence.
8. User-facing second-source copy nie miesza już stock disclosure z copy o rezerwach giełd.
9. Orbit drawer dostał hard scroll lock: wheel delta idzie bezpośrednio w panel, a body/orbit nie przechwytuje scrolla.
10. Orbit drawer CSS wymusza fixed right-edge scroll panel, `overflow-y: scroll`, `touch-action: pan-y`, `overscroll-behavior: contain` i brak containment blokującego scroll.
11. Guard: `verify:pass346-real-market-cards-orbit-scroll`.
12. PASS344/PASS345 guard compatibility zachowana.

## A-M progres po PASS346
| ID | Obszar | Stan | Progress | Największy blocker |
|---|---|---:|---:|---|
| A | Core runtime / build | P0 | 37% | Brak `node_modules`/typów w paczce eksportowej; pełny typecheck nadal nie przechodzi przez zależności. |
| B | Brand/luxury public UI | P1 | 25% | Real Markets czytelniejsze, ale public UI nadal ma miejsca z technicznym językiem. |
| C | VLM Shield token surface | P0 | 33% | Full CoinGecko cache i token index nadal P0. |
| D | VLM AI Brain / Orbit 360 | P0 | 34% | Drawer ma hard scroll lock, ale potrzebny real browser QA i focus trap. |
...
| I | Real Markets / Cross-Asset | P0 | 29% | UI kart jest lepszy, ale provider data/cache/API keys nadal brak. |
| J | Interaction / scroll / performance | P0 | 34% | Orbit scroll hardlock jest, ale potrzebny real browser profiler i mobile QA. |
| K | AI bot / copy engine | P0 | 33% | Second Source copy lepsze, ale per-asset copy dla wszystkich rynków nadal niedokończone. |
