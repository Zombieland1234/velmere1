# VELMÈRE MASTER BUILD MAP — PASS348

## Brutalny stan po PASS348
Publiczny prototyp nadal traktować jako ok. 17% finalnego produktu. PASS348 poprawia publiczny Real Markets Reader Pro, usuwa stare table-first copy z hero, dodaje ludzkie role assetów, provider-state, prosty second-source i naprawia Orbit drawer tak, żeby główny panel scrollował natywnie zamiast przez wymuszony JS `preventDefault`. Nadal P0: live adaptery, durable ledger, PDF renderer, security production, browser QA i pełny typecheck z zależnościami.

## Co PASS348 naprawia
1. Cross-asset route nie reklamuje już starego `Real Markets Table`; hero mówi o kartach i braku szerokich tabel.
2. Real Market cards dostały role: waluta, spółka, ETF, real estate proxy, commodity.
3. Provider state dostał ludzki opis, więc `provider required` nie wygląda jak debug.
4. Review pressure jest podpisane jako ostrożność analizy, nie sygnał tradingowy.
5. Second Source ma prosty opis per lane: exchange depth, API freshness, reserves, stock disclosure, FX, real estate, FTX regression.
6. Exchange Stability nie powtarza notki w każdej karcie.
7. Karty mają mocniejszy layout anty-overflow: min-width 0, max-width 100%, overflow-wrap.
8. Orbit drawer v6 działa bardziej natywnie: główny panel nie dostaje `preventDefault` przy normalnym wheel.
9. JS wheel routing zostaje tylko dla zagnieżdżonych scroll-zone.
10. Drawer v6 ma wolniejsze wysunięcie 1480ms i overscroll containment.
11. Dodany guard `verify:pass348-real-market-pro-orbit-native`.
12. PASS344-PASS347 guard compatibility zachowana.

## A-M progres po PASS348
...
| B | Brand/luxury public UI | P1 | 29% | Real Markets bardziej premium, ale część publicznych miejsc nadal ma techniczny język. |
| C | VLM Shield token surface | P0 | 33% | Full CoinGecko cache i token index nadal P0. |
| D | VLM AI Brain / Orbit 360 | P0 | 38% | Drawer ma native scroll v6, ale potrzebny real browser QA i focus trap. |
...
| I | Real Markets / Cross-Asset | P0 | 38% | Reader Pro jest lepszy, ale provider data/cache/API keys nadal brak. |
| J | Interaction / scroll / performance | P0 | 39% | Orbit native scroll v6 jest, ale potrzebny real browser profiler i mobile QA. |
| K | AI bot / copy engine | P0 | 37% | Second Source copy lepsze, ale per-asset copy dla wszystkich rynków nadal niedokończone. |
