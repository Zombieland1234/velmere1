...
import {
  ...
  getAdminIdempotencyStoreSummary,
  adminIdempotencyStoreLaunchNote,
  createAdminIdempotencyPreview,

} from "@/lib/launch/admin-idempotency-store";
...
const copy = {
  pl: {
    eyebrow: "admin idempotency store",
    title: "Retry publish/import nie może robić drugiej mutacji.",
    body: "Ta warstwa opisuje normalization, persistent idempotency storage, duplicate response policy i TTL dla admin write actions.",
  ...
  de: {
    eyebrow: "admin idempotency store",
    title: "Retry von Publish/Import darf keine zweite Mutation erzeugen.",
    body: "Diese Ebene beschreibt Normalization, persistent Idempotency Storage, Duplicate Response Policy und TTL für Admin Write Actions.",
  ...
  en: {
    eyebrow: "admin idempotency store",
    title: "Publish/import retries must not create a second mutation.",
    body: "This layer defines normalization, persistent idempotency storage, duplicate response policy and TTL for admin write actions.",
...
export default function AdminIdempotencyStorePanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getAdminIdempotencyStoreSummary();
  const visibleItems = adminIdempotencyStoreMatrix.filter((item) => {
    return surface === "admin" || ["key-normalization", "idempotency-storage", "duplicate-response"].includes(item.id);
  });
  const preview = createAdminIdempotencyPreview({
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-black/[0.22] p-4 text-xs leading-6 text-white/[0.50]">
              {t.note}: {adminIdempotencyStoreLaunchNote}
            </p>
          </div>
