...
import {
  ...
  getCustomerSafeExportBoundarySummary,
  customerSafeExportLaunchNote,
  createCustomerSafeExportPreview,

} from "@/lib/launch/customer-safe-export-boundary";
...
const copy = {
  pl: {
    eyebrow: "customer-safe export boundary",
    title: "Support może eksportować tylko wersję bezpieczną dla klienta.",
    body: "Ta warstwa filtruje support timeline do customer-safe copy: bez sekretów, raw payloadów, prywatnych promptów i niepotwierdzonych obietnic.",
  ...
  de: {
    eyebrow: "customer-safe export boundary",
    title: "Support darf nur kundensichere Versionen exportieren.",
    body: "Diese Ebene filtert die Support Timeline zu Customer-Safe Copy: ohne Secrets, Raw Payloads, private Prompts und unbestätigte Versprechen.",
  ...
  en: {
    eyebrow: "customer-safe export boundary",
    title: "Support can export only a customer-safe version.",
    body: "This layer filters the support timeline into customer-safe copy: no secrets, raw payloads, private prompts or unsupported promises.",
...
export default function CustomerSafeExportBoundaryPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getCustomerSafeExportBoundarySummary();
  const visibleItems = customerSafeExportBoundaryMatrix.filter((item) => {
    return surface === "admin" || ["support-to-customer-filter", "approval-gate", "export-redaction"].includes(item.id);
  });
  const preview = createCustomerSafeExportPreview({ approved: false });
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4 text-xs leading-6 text-white/[0.48]">
              {t.note}: {customerSafeExportLaunchNote}
            </p>
          </div>
