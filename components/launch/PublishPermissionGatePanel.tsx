...
import { publishPermissionGate, getPublishPermissionSummary } from "@/lib/launch/publish-permission-gate";
...
const copy = {
  pl: {
    eyebrow: "publish permission gate",
    title: "Import produktu nie może oznaczać publikacji produktu.",
    body: "Ta warstwa rozdziela draft import, coming soon i active publish. Aktywny produkt wymaga provider truth, shipping/returns truth, roli operatora i audytu.",
  ...
  de: {
    eyebrow: "publish permission gate",
    title: "Produktimport darf nicht automatisch Veröffentlichung bedeuten.",
    body: "Diese Ebene trennt Draft Import, Coming Soon und Active Publish. Ein aktives Produkt braucht Provider Truth, Shipping/Returns Truth, Rolle und Audit.",
  ...
  en: {
    eyebrow: "publish permission gate",
    title: "Product import must not mean product publication.",
    body: "This layer separates draft import, coming soon and active publish. Active products require provider truth, shipping/returns truth, operator role and audit.",
...
export default function PublishPermissionGatePanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getPublishPermissionSummary();
  const visibleItems = publishPermissionGate.filter((item) => {
    return surface === "admin" || ["provider-truth-required", "shipping-returns-required", "active-publish-permission"].includes(item.id);
  });
