...
import { adminServerAuthContract, getAdminServerAuthSummary } from "@/lib/launch/admin-server-auth-contract";
...
const copy = {
  pl: {
    eyebrow: "server auth contract",
    title: "Admin musi mieć prawdziwy server auth, nie tylko locked UI.",
    body: "Ta warstwa opisuje provider auth, role, session expiry, permission scope i server kill switch. Client env gate zostaje tylko osłoną UX.",
  ...
  de: {
    eyebrow: "server auth contract",
    title: "Admin braucht echtes Server Auth, nicht nur Locked UI.",
    body: "Diese Ebene beschreibt Auth Provider, Rollen, Session Expiry, Permission Scope und Server Kill Switch. Client Env Gate bleibt nur UX-Schutz.",
  ...
  en: {
    eyebrow: "server auth contract",
    title: "Admin needs real server auth, not only locked UI.",
    body: "This layer defines auth provider, roles, session expiry, permission scope and server kill switch. Client env gate remains only a UX guard.",
...
export default function AdminServerAuthContractPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getAdminServerAuthSummary();
  const visibleItems = adminServerAuthContract.filter((item) => {
    return surface === "admin" || ["auth-provider", "role-contract", "server-kill-switch"].includes(item.id);
  });
