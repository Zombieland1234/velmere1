...
import {
  ...
  getPublishRollbackContextSummary,
  publishRollbackLaunchNote,
  createPublishRollbackDiff,

} from "@/lib/launch/publish-rollback-context";
...
const copy = {
  pl: {
    eyebrow: "publish rollback context",
    title: "Każdy publish musi mieć diff i drogę cofnięcia.",
    body: "Ta warstwa opisuje before/after diff, rollback id, checklist snapshot i customer impact classification.",
  ...
  de: {
    eyebrow: "publish rollback context",
    title: "Jeder Publish braucht Diff und Rückweg.",
    body: "Diese Ebene beschreibt Before/After Diff, Rollback ID, Checklist Snapshot und Customer Impact Classification.",
  ...
  en: {
    eyebrow: "publish rollback context",
    title: "Every publish needs a diff and a rollback path.",
    body: "This layer defines before/after diff, rollback id, checklist snapshot and customer impact classification.",
...
export default function PublishRollbackContextPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getPublishRollbackContextSummary();
  const visibleItems = publishRollbackContextMatrix.filter((item) => {
    return surface === "admin" || ["before-after-diff", "rollback-id", "customer-impact"].includes(item.id);
  });
  const preview = createPublishRollbackDiff({
    productId: "product:velmere-draft",
    previousState: "draft",
    nextState: "coming_soon",
    changedFields: ["status", "visibility", "launchCopy"],
    checklistSnapshot: ["provider truth missing", "shipping/returns partial", "legal copy review"],
    customerImpact: "low",
  });
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-black/[0.22] p-4 text-xs leading-6 text-white/[0.50]">
              {t.note}: {publishRollbackLaunchNote}
            </p>
          </div>
