...
import { getOrderEventLedgerSummary, orderEventLedgerMatrix } from "@/lib/launch/order-event-ledger";
...
const copy = {
  pl: {
    eyebrow: "order event ledger",
    title: "Każde zdarzenie zamówienia musi mieć ślad.",
    body: "Ta warstwa pilnuje event id, idempotency key, podpisanych webhooków, retry policy, timeline zamówienia i support handoff. Bez tego checkout zostaje zablokowany.",
  ...
  de: {
    eyebrow: "order event ledger",
    title: "Jedes Bestellereignis braucht eine Spur.",
    body: "Diese Ebene prüft Event ID, Idempotency Key, signierte Webhooks, Retry Policy, Bestell-Timeline und Support-Handoff. Ohne das bleibt Checkout gesperrt.",
  ...
  en: {
    eyebrow: "order event ledger",
    title: "Every order event needs a trace.",
    body: "This layer tracks event id, idempotency key, signed webhooks, retry policy, order timeline and support handoff. Without it, checkout stays blocked.",
...
export default function OrderEventLedgerPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getOrderEventLedgerSummary();
  const visibleItems = orderEventLedgerMatrix.filter((item) => {
    if (surface === "cart") return ["event-id", "idempotency-key", "order-timeline"].includes(item.id);
    if (surface === "admin") return ["signed-webhook", "retry-policy", "support-handoff"].includes(item.id);
    if (surface === "ops") return ["event-id", "signed-webhook", "retry-policy", "order-timeline", "support-handoff"].includes(item.id);
    return item.requiredBeforePayment;
  });
