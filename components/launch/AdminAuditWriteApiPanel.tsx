...
import {
  ...
  getAdminAuditWriteRouteSummary,
  adminAuditWriteContractLaunchNote,
  createAdminAuditWritePreview,

} from "@/lib/launch/admin-audit-write-contract";
...
const copy = {
  pl: {
    eyebrow: "admin audit write api",
    title: "Audit write route istnieje, ale pozostaje zablokowany bez server auth i storage.",
    body: "Ten panel pokazuje pierwszy server route contract dla admin audit events: idempotency, walidację pól, gate środowiska, preview envelope i brak realnego storage write.",
  ...
  de: {
    eyebrow: "admin audit write api",
    title: "Audit Write Route existiert, bleibt aber ohne Server Auth und Storage gesperrt.",
    body: "Dieses Panel zeigt den ersten Server Route Contract für Admin Audit Events: Idempotency, Feldvalidierung, Environment Gate, Envelope Preview und keinen echten Storage Write.",
  ...
  en: {
    eyebrow: "admin audit write api",
    title: "Audit write route exists, but stays locked without server auth and storage.",
    body: "This panel shows the first server route contract for admin audit events: idempotency, field validation, environment gate, envelope preview and no real storage write.",
...
export default function AdminAuditWriteApiPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getAdminAuditWriteRouteSummary();
  const visibleItems = adminAuditWriteRouteMatrix.filter((item) => {
    return surface === "admin" || ["api-route-skeleton", "idempotency-contract", "server-gate"].includes(item.id);
  });
  const preview = createAdminAuditWritePreview({
    action: "active_publish",
    operatorId: "operator:unknown",
    targetId: "product:velmere-draft",
    reason: "reason:required-before-production",
    sourceMode: "manual",
    idempotencyKey: "",
    checklist: ["provider truth", "shipping returns"],
    payload: { token: "[REDACTED_AUTHORIZATION]", email: "operator@example.com" },
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4 text-xs leading-6 text-white/[0.48]">
              {t.note}: {adminAuditWriteContractLaunchNote}
            </p>
          </div>
