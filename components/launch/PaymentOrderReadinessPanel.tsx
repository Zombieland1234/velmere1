...
import { getPaymentOrderReadinessSummary, paymentOrderReadinessMatrix } from "@/lib/launch/payment-order-readiness";
...
const copy = {
  pl: {
    eyebrow: "payment / order state",
    title: "Płatność i status zamówienia muszą być prawdziwe.",
    body: "Ta warstwa blokuje checkout, dopóki payment provider, podatki, order id, maile transakcyjne, webhooks i refund state nie są produkcyjnie spięte.",
  ...
  de: {
    eyebrow: "payment / order state",
    title: "Zahlung und Bestellstatus müssen echt sein.",
    body: "Diese Ebene sperrt Checkout, bis Payment Provider, Steuern, Order ID, Transaktionsmails, Webhooks und Refund State produktiv verbunden sind.",
  ...
  en: {
    eyebrow: "payment / order state",
    title: "Payment and order state must be real.",
    body: "This layer blocks checkout until payment provider, taxes, order id, transactional emails, webhooks and refund state are production wired.",
...
export default function PaymentOrderReadinessPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getPaymentOrderReadinessSummary();
  const visibleItems = paymentOrderReadinessMatrix.filter((item) => {
    if (surface === "cart") return ["payment-provider", "tax-calculation", "order-confirmation"].includes(item.id);
    if (surface === "ops") return ["webhook-audit", "refund-state", "customer-emails"].includes(item.id);
    return item.requiredBeforeCheckout;
  });
