...
import {
  ...
  getAdminAuditPersistenceSummary,
  adminAuditPersistenceLaunchNote,
  createAdminAuditPersistencePreview,

} from "@/lib/launch/admin-audit-persistence";
...
const copy = {
  pl: {
    eyebrow: "admin audit persistence",
    title: "Audit musi być zapisany po stronie serwera, nie tylko pokazany w UI.",
    body: "Ta warstwa opisuje storage adapter, operator context, idempotent write, redacted source snapshot i retention/export policy.",
  ...
  de: {
    eyebrow: "admin audit persistence",
    title: "Audit muss serverseitig gespeichert werden, nicht nur im UI erscheinen.",
    body: "Diese Ebene beschreibt Storage Adapter, Operator Context, idempotenten Write, redacted Source Snapshot und Retention/Export Policy.",
  ...
  en: {
    eyebrow: "admin audit persistence",
    title: "Audit must be stored server-side, not only shown in UI.",
    body: "This layer defines storage adapter, operator context, idempotent write, redacted source snapshot and retention/export policy.",
...
export default function AdminAuditPersistencePanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getAdminAuditPersistenceSummary();
  const visibleItems = adminAuditPersistenceMatrix.filter((item) => {
    return surface === "admin" || ["storage-adapter", "operator-context", "retention-export"].includes(item.id);
  });
  const preview = createAdminAuditPersistencePreview({
    caseId: "admin-active_publish-20260603000000-productvelmere",
    operatorId: "operator:preview",
    safePayload: "{\"decision\":\"blocked preview only\"}",
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-black/[0.22] p-4 text-xs leading-6 text-white/[0.50]">
              {t.note}: {adminAuditPersistenceLaunchNote}
            </p>
          </div>
