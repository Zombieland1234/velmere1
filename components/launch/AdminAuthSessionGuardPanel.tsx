...
import {
  ...
  getAdminAuthSessionSummary,
  adminAuthSessionLaunchNote,
  getAdminSessionPreviewFromEnv,
  requireAdminScope,

} from "@/lib/launch/admin-auth-session-guard";
...
const copy = {
  pl: {
    eyebrow: "admin auth session guard",
    title: "Route admina musi znać sesję, rolę i scope operatora.",
    body: "Ta warstwa opisuje server session reader, role/scope map, fresh session requirement i spokojne deny copy dla admin actions.",
  ...
  de: {
    eyebrow: "admin auth session guard",
    title: "Admin Route braucht Session, Rolle und Scope des Operators.",
    body: "Diese Ebene beschreibt Server Session Reader, Role/Scope Map, Fresh Session Requirement und ruhige Deny Copy für Admin Actions.",
  ...
  en: {
    eyebrow: "admin auth session guard",
    title: "Admin route must know the operator session, role and scope.",
    body: "This layer defines server session reader, role/scope map, fresh session requirement and calm deny copy for admin actions.",
...
export default function AdminAuthSessionGuardPanel({ locale, surface }: Props) {
  const t = localeCopy(locale);
  const summary = getAdminAuthSessionSummary();
  const visibleItems = adminAuthSessionMatrix.filter((item) => {
    return surface === "admin" || ["session-reader", "role-scope-map", "fresh-session"].includes(item.id);
  });
  const session = getAdminSessionPreviewFromEnv();
  const preview = {
    session,
    activePublish: requireAdminScope(session, "product:active_publish"),
    supportExport: requireAdminScope(session, "support:export"),
  };
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.45fr]">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/[0.08] bg-black/[0.22] p-4 text-xs leading-6 text-white/[0.50]">
              {t.note}: {adminAuthSessionLaunchNote}
            </p>
          </div>
