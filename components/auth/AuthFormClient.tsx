...
export default function AuthFormClient({ labels }: AuthFormClientProps) {
  ...
  const continueWithGooglePreview = () => {
    setLoading(true);
    setError(null);
    void startVelmereGoogleOAuth(locale).catch(() => {
      setLoading(false);
      setError("Google sign-in is not available yet. Use email access or member preview.");
    });
  };
    ...
    <section className="velmere-command-shell relative overflow-hidden rounded-[2rem] border-white/[0.08] bg-[#07090c] p-5 shadow-[0_30px_110px_rgba(0,0,0,0.35)] md:p-8" data-pass2005-auth-form="solid-cyan-focus-no-heavy-blur">
      ...
      <div className="relative">
        ...
        <div className="mt-4 flex justify-end">
          ...
          }} className="text-xs text-white/[0.38] transition hover:text-cyan-100">
        ...
        <div className="grid gap-3">
          <button
            type="button"
            onClick={continueWithGooglePreview}
            disabled={loading}
            ...
            data-pass2363-google-auth-preview="server-bridged-account-id"
          >
            <span className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-white/[0.78]">Google account bridge</span>
                <span className="mt-1 block truncate text-xs text-white/[0.36]">Stable accountId now, real OAuth after Supabase provider setup.</span>
              </span>
            </span>
          ...
          <button
            ...
            disabled={loading}
            className="group flex min-h-[4.25rem] w-full items-center justify-between gap-4 rounded-[1.35rem] border border-white/[0.06] bg-black/[0.18] px-4 text-left transition-all duration-150 hover:border-cyan-200/[0.18] hover:bg-cyan-300/[0.025] disabled:opacity-50"
          >
            <span className="flex min-w-0 items-center gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-white/[0.08] bg-white/[0.035] text-cyan-100">
                <Sparkles className="h-4 w-4" aria-hidden="true" />
              </span>
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-white/[0.72]">{labels?.googlePreview ?? "Explore member preview"}</span>
                <span className="mt-1 block truncate text-xs text-white/[0.34]">{local.previewBody}</span>
              </span>
            </span>
            <span className="rounded-full border border-white/[0.07] px-2.5 py-1 font-mono text-[8px] uppercase tracking-[0.12em] text-white/[0.28]">
              Local
            </span>
          </button>
        </div>
