...
function project(value, path, excludedPaths, omittedUndefinedPaths, seen) {
  ...
  try {
    if (Array.isArray(value)) {
      return value.map((entry, index) => project(entry, `${path}[${index}]`, excludedPaths, omittedUndefinedPaths, seen));
    }
    ...
    for (const key of Object.keys(value).sort()) {
      ...
      if (typeof value[key] === "undefined") {
        omittedUndefinedPaths.push(childPath);
        continue;
      }
      output[key] = project(value[key], childPath, excludedPaths, omittedUndefinedPaths, seen);
    }
    return output;
  } finally {
    seen.delete(value);
...
export function buildSemanticPayload(value) {
  const excludedPaths = [];
  const omittedUndefinedPaths = [];
  const payload = project(value, "$", excludedPaths, omittedUndefinedPaths, new WeakSet());
  const envelope = {
