...
export function buildPass4418HumanMarketBrief(
  ...
  const price = formatPrice(quote, asset.category);
...
export function buildPass4418RealMarketsAssetDetailData(
  ...
  // quote.confidenceCap is an internal evidence/completeness ceiling, not a calibrated probability.
  // Keep it out of customer confidence fields until an explicit calibration artifact exists.
  ...
  return {
    ...
    priceLabel: formatAssetDetailQuotePrice(quote, asset.category),
    changeLabel: formatSignedPercent(change),
    changeTone:
      typeof change !== "number" ? "neutral" : change >= 0 ? "up" : "down",
    sourceLabel: source,
    sourceVerified: pass4577CanShowPercent(quote, asset.category),
    ...
    riskLabel: risk === null ? null : formatDecimalPercent(risk),
    candles: pass4577CanShowChart(quote, asset.category, 2) ? quote?.candles ?? [] : [],
    marketDataState: hasServerVerifiedQuoteLiveGate(quote)
      ? "live_verified"
      : quote?.state === "live"
        ? "partial_not_live"
        : "unverified",
    detailMetrics: [
      ...(typeof marketCap === "number" && Number.isFinite(marketCap) && marketCap > 0
        ? [{ label: "Market cap", value: formatPass4474CompactNumber(marketCap, locale), caption: quote?.currency ?? "USD", tone: "evidence" as const }]
        : []),
      ...
      ...(risk === null
        ...
        : [{
            ...
            value: formatDecimalPercent(risk),
            caption: asset.category.replace("_", " "),
            tone: risk >= 70 ? "danger" as const : risk >= 42 ? "warning" as const : "positive" as const,
          }]),
      ...(confidence === null
        ? []
