...
type AssetMode = "standard" | "stablecoin" | "rwa" | "meme" | "unknown";
...
function missingCoreFieldNames(input: TokenRiskInput) {
  const names: string[] = [];
  if (!hasFiniteNumber(input.currentPrice)) names.push("price");
  if (!hasFiniteNumber(input.marketCap) && !hasFiniteNumber(input.fdv))
    names.push("market cap or FDV");
  if (!hasFiniteNumber(input.volume24h)) names.push("24h volume");
  if (!hasFiniteNumber(input.liquidityUsd)) names.push("liquidity depth");
  if (!hasSupplyCoverage(input)) names.push("supply float");
  if (!hasHolderCoverage(input)) names.push("holder concentration");
  if (!hasContractCoverage(input)) names.push("contract permissions");
  if (!hasOrderbookCoverage(input)) names.push("orderbook depth");
...
function buildLimitations(
  ...
  if (!input.pairAddress && !input.dexId)
    limitations.push("DEX pair/source routing not verified");
  if (!hasFiniteNumber(input.buyTaxPercentage) && !hasFiniteNumber(input.sellTaxPercentage))
    limitations.push("tax settings unavailable");
  if (!hasFiniteNumber(input.buys24h) || !hasFiniteNumber(input.sells24h))
...
export function analyzeTokenRisk(
  ...
  return {
    ...
    score,
    scoreFormula: "deterministic_weighted_multi_agent_fusion_v8",
    confidence,
    scoreBreakdown,
    agentAssessments,
    metaModel,
