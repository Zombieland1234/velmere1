...
import { buildPass2576AuditPermissionParserReport, PASS2576_AUDIT_PERMISSION_PARSER_ID } from "@/lib/security/audit-permission-parser";
...
export async function GET(request: Request) {
  return withPass4824AuditProviderPublicGet(request, "/api/security/audit-permission-parser", () =>
...
async function handlePass4824AuditProviderGet(request: Request) {
  ...
  return NextResponse.json({
    ok: true,
    pass2576AuditPermissionParser: permissionParser,
    summary: permissionParser.summary,
    basicRows: permissionParser.basicRows,
    proPdfRows: permissionParser.proPdfRows,
  }, {
    headers: {
      "cache-control": "no-store",
      "x-velmere-pass2576-permission-parser": PASS2576_AUDIT_PERMISSION_PARSER_ID,
    },
  });
}
