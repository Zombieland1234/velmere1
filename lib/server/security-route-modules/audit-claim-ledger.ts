...
import { buildPass2574AuditClaimLedgerReport, PASS2574_AUDIT_CLAIM_LEDGER_ID } from "@/lib/security/audit-claim-ledger";
...
export async function GET(request: Request) {
  return withPass4824AuditProviderPublicGet(request, "/api/security/audit-claim-ledger", () =>
...
async function handlePass4824AuditProviderGet(request: Request) {
  ...
  return NextResponse.json({
    ok: true,
    pass2574AuditClaimLedger: claimLedger,
    summary: claimLedger.summary,
    customerRows: claimLedger.customerRows,
    proPdfRows: claimLedger.proPdfRows,
  }, {
    headers: {
      "cache-control": "no-store",
      "x-velmere-pass2574-claim-ledger": PASS2574_AUDIT_CLAIM_LEDGER_ID,
    },
  });
}
