...
import { buildPass2570AuditSourceQuorumReport, PASS2570_AUDIT_SOURCE_QUORUM_RUNTIME_ID } from "@/lib/security/audit-source-quorum-runtime";
...
function response(report: ReturnType<typeof buildPass2570AuditSourceQuorumReport>, providerIntelligence: ReturnType<typeof buildPass2571AuditProviderIntelligenceReport>) {
  return NextResponse.json(
    {
      ok: true,
      surface: "velmere-audit-source-quorum-runtime",
      report,
      providerIntelligence,
      rule: report.quorumRule,
    },
    {
      headers: {
        "cache-control": "no-store",
        "x-velmere-pass2570-source-quorum-runtime": PASS2570_AUDIT_SOURCE_QUORUM_RUNTIME_ID,
...
export async function GET(request: Request) {
  ...
  const report = buildPass2570AuditSourceQuorumReport({ ...normalized, locale: payload.locale });
  return response(report, buildPass2571AuditProviderIntelligenceReport({ ...normalized, locale: payload.locale, sourceQuorum: report }));
}

export async function POST(request: Request) {
  const pass4281Guard = await guardPass4281AuditPostRequest(request, {
    routeId: "audit-source-quorum",
    maxBytes: 32_768,
    limit: 30,
    windowMs: 60_000,
  });
  ...
  const normalized = normalizeAuditReviewSubmission(payload);
  const report = buildPass2570AuditSourceQuorumReport({ ...normalized, locale });
  return response(report, buildPass2571AuditProviderIntelligenceReport({ ...normalized, locale, sourceQuorum: report }));
  });
}
