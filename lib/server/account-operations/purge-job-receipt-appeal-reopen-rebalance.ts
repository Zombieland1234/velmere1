...
export function GET(request: Request) {
  ...
  const selectedGuard = pass2556.purgeReleaseGuards.find((item) => !requestedSupportCaseId || item.supportCaseId === requestedSupportCaseId) ?? pass2556.purgeReleaseGuards[0];

  return NextResponse.json({
    ok: selectedGuard?.purgeStatusVisibleToCustomer ?? false,
    pass: "2556",
    selectedGuard,
    purgeJobReceiptAppealReopenRebalance: pass2556,
    purgeJobReceipts: pass2556.purgeJobReceipts,
    customerDeletionTimelines: pass2556.customerDeletionTimelines,
    appealReopenGates: pass2556.appealReopenGates,
    angelPurgeBoundaries: pass2556.angelPurgeBoundaries,
    purgeReleaseGuards: pass2556.purgeReleaseGuards,
    previousPassEvidenceRetentionRule: pass2555.releaseEquation,
    releaseEquation: pass2556.releaseEquation,
