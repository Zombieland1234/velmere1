...
export async function GET(request: Request) {
  ...
  const query = url.searchParams.get("query") || "VELMERE_SUPPORT_DASHBOARD_ACTION_RLS_POLICY_TEST";
  const symbol = url.searchParams.get("symbol") || undefined;
  const requestedSupportCaseId = url.searchParams.get("caseId") || undefined;
  ...
  const selectedGuard = pass2559.supportDashboardActionReleaseGuards.find((item) => !requestedSupportCaseId || item.supportCaseId === requestedSupportCaseId) ?? pass2559.supportDashboardActionReleaseGuards[0];

  return NextResponse.json({
    ok: selectedGuard?.supportActionVisible ?? false,
    pass: "2559",
    selectedGuard,
    supportDashboardActionRlsPolicyTestRebalance: pass2559,
    rlsPolicyTestFixtures: pass2559.rlsPolicyTestFixtures,
    supportActionExecutionReceipts: pass2559.supportActionExecutionReceipts,
    providerErasureRetryDeadLetters: pass2559.providerErasureRetryDeadLetters,
    customerDsarStatusTimelines: pass2559.customerDsarStatusTimelines,
    angelSupportActionBoundaries: pass2559.angelSupportActionBoundaries,
    supportDashboardActionReleaseGuards: pass2559.supportDashboardActionReleaseGuards,
    previousPassRlsSupportDashboardRule: pass2558.releaseEquation,
    releaseEquation: pass2559.releaseEquation,
