...
export async function GET(request: Request) {
  ...
  const rateLimit = await applyApiRateLimit(request, { keyPrefix: "historical-range-window", limit: 24, windowMs: 60_000 });
  ...
  try {
    ...
    const historicalBackfillOrchestrator = sourceSync.pass2462 ?? buildPass2462HistoricalBackfillOrchestrator({
      ...
      pointCount: pointCount ?? result?.chart?.sevenDay?.length ?? 0,
      sourceSync,
    ...
    return securityJson({
      mode: "historical_range_window_ledger",
      query,
      range,
      symbol: sourceSync.symbol,
      ...
      surfaceWindowContracts: historicalRangeWindowLedger.surfaceWindowContracts,
      ...
      hardLocks: historicalRangeWindowLedger.hardLocks,
      providerExecutionOrder: historicalRangeWindowLedger.providerExecutionOrder,
    ...
    return publicApiError(error, { route: "/api/market-integrity/historical-range-window", code: "historical_range_window_request_failed", status: 502 });
  }
}
