...
import { PASS4616_AUDIT_REVIEW_ORCHESTRATION_BOUNDARY, PASS4616_AUDIT_REVIEW_ORCHESTRATION_ID, settleProAuditWorkerLease } from "@/lib/security/audit-review-orchestration";
import {
...
import { completeProAuditWorkerLeaseWithSnapshot, persistAuditReportSnapshot } from "@/lib/security/audit-report-snapshot-store";
import {
...
export async function POST(request: Request) {
  ...
  const rate = await applyApiRateLimit(request, { keyPrefix: "pass4616-pro-worker-settle", limit: 40, windowMs: 60_000 });
  ...
  if (outcome === "complete") {
    if (record.tier !== "pro" || !record.accountId || !record.entitlementId || !record.entitlementVerified) {
      return securityJson({ ok: false, error: "pro_report_snapshot_case_binding_invalid" }, { status: 409 });
    }
    ...
    const reportId = (reportIdInput || `${record.caseRef.toLowerCase()}-pro-v1`).replace(/[^a-zA-Z0-9:._-]+/g, "-").slice(0, 120);
    ...
    try {
      ...
      if (!snapshot.evidenceReadiness.proReady) {
        const retryResult = await settleProAuditWorkerLease({
          record,
          workerPrincipal,
          leaseToken,
          outcome: "retry",
        ...
        return securityJson({
          ok: false,
          error: "pro_paid_evidence_not_ready",
      ...
      if (record.durable) {
        const completed = await completeProAuditWorkerLeaseWithSnapshot({
          reportId,
        ...
        reportBinding = {
          ...
          paidEvidenceReady: completed.record.snapshot.evidenceReadiness.proReady,
        ...
        reportBinding = {
          ...
          paidEvidenceReady: persisted.record.snapshot.evidenceReadiness.proReady,
      ...
      return publicApiError(error, {
        route: "/api/security/audit-review/pro/settle",
        code: "pro_report_snapshot_build_failed",
        status: 503,
      });
    }
  }
  const result = atomicCompletionResult ?? await settleProAuditWorkerLease({
    record,
    workerPrincipal,
    leaseToken,
    outcome,
