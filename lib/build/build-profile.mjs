...
function optionalTurbopackMemoryEviction(value, fallback) {
  const raw = optionalTrimmed(value);
  if (raw === null) return fallback;
  if (raw === "auto" || raw === "full") return raw;
  if (raw === "false" || raw === "0" || raw === "off") return false;
  fail("VELMERE_TURBOPACK_MEMORY_EVICTION must be auto, full, false, 0, or off");
}
...
export function resolveBuildSettings(environment = process.env) {
  if (optionalTrimmed(environment.VELMERE_TURBOPACK_MEMORY_LIMIT_BYTES) !== null) {
    fail("VELMERE_TURBOPACK_MEMORY_LIMIT_BYTES is obsolete in Next.js 16.3.5; use VELMERE_TURBOPACK_MEMORY_EVICTION");
  }
  if (environment.VERCEL) {
    return Object.freeze({
      profile: Object.freeze({ name: "balanced", ...BUILD_PROFILE_DEFAULTS.balanced }),
      runtimeBuildScope: null,
      runtimeDistDir: null,
      runtimeBuildId: null,
  ...
  const turbopackMemoryEviction = optionalTurbopackMemoryEviction(
    environment.VELMERE_TURBOPACK_MEMORY_EVICTION,
    profile.turbopackMemoryEviction,
  );
