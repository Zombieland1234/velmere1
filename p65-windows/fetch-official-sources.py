...
import csv, datetime as dt, hashlib, io, json, pathlib, urllib.request, urllib.error
...
# Normalized current data. Never package raw provider terms.
...
try:
    j=json.loads(by['nvd_sample']['_bytes'])
    cve=j['vulnerabilities'][0]['cve']
...
try:
    text=by['ecb_fx']['_bytes'].decode('utf-8-sig','replace')
    rows=list(csv.DictReader(io.StringIO(text)))
    def pick(r,k): return r.get(k) or r.get(k.upper()) or r.get(k.lower())
    slim=[]
    for r in rows:
        slim.append({'key':pick(r,'KEY'),'currency':pick(r,'CURRENCY'),'currencyDenom':pick(r,'CURRENCY_DENOM'),'frequency':pick(r,'FREQ'),'timePeriod':pick(r,'TIME_PERIOD'),'obsValue':pick(r,'OBS_VALUE'),'title':pick(r,'TITLE')})
...
try:
    j=json.loads(by['cisa_kev']['_bytes'])
    vulns=j.get('vulnerabilities',[])
    recent=sorted(vulns,key=lambda x:x.get('dateAdded',''))[-5:]
...
public=[]
for x in results:
    y={k:v for k,v in x.items() if k!='_bytes'}; public.append(y)
