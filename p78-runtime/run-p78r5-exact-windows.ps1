...
$outDir = 'p78r5-windows-out'
$groundTruthDir = 'p78r5-windows-groundtruth'
...
function RunExternal([string]$name, [scriptblock]$block) {
  Write-Host "P78R5 STEP START $name"
  & $block
  if ($LASTEXITCODE -ne 0) { throw "P78R5 external step failed: $name exit=$LASTEXITCODE" }
  $script:completed.Add($name)
  Write-Host "P78R5 STEP PASS $name"
}
...
try {
  ...
  RunExternal 'fetch_pinned_thirdweb_pair' { python p78-runtime/fetch-p78r3-thirdweb-pinned.py --output-dir $groundTruthDir --receipt "$outDir/P78R5_THIRDWEB_PINNED_SOURCE_TRANSPORT.json" }
  ...
  try {
    RunExternal 'npm_ci_ignore_scripts_at_r4' { & node $env:P47_NPM_CLI_PATH ci --ignore-scripts --include=dev --audit=false --fund=false }
    RunExternal 'npm_ls_all_at_r4' { & node $env:P47_NPM_CLI_PATH ls --all }
    RunExternal 'native_next_swc_probe_at_r4' {
  ...
  # Prove the exact R4 parent behavior in this same Windows execution before tightening semantics.
  $env:TSX_TSCONFIG_PATH = (Resolve-Path p75-work/source/tsconfig.json).Path
  $env:P78R4_RESULT_DIR = (Resolve-Path $outDir).Path
  $env:P78R4_GROUNDTRUTH_DIR = (Resolve-Path $groundTruthDir).Path
  Push-Location p75-work/source
  try { RunExternal 'p78r4_parent_target_correlation_runtime' { node ./node_modules/tsx/dist/cli.mjs ../../p78-runtime/test-p78r4-runtime.mts } } finally { Pop-Location }

  # Now apply the single-file authenticity precision repair.
  ...
  RunExternal 'p78r5_static' { python p78-runtime/test-p78r5-static.py --source-root p75-work/source --parent-manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --source-receipt "$outDir/P78R5_SOURCE_PATCH.json" --receipt "$outDir/P78R5_STATIC_CONTROL.json" }
  RunExternal 'projection_preinstall' { python p78-runtime/verify-p78r5-projection.py --source-root p75-work/source --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --mode check --receipt "$outDir/P78R5_PROJECTION_PREINSTALL.json" --build-label preinstall }

  Push-Location p75-work/source
  try {
    RunExternal 'typescript_semantic_r5' { node ./node_modules/typescript/bin/tsc --noEmit --pretty false }
    RunExternal 'eslint_zero_warning_r5' { node ./node_modules/eslint/bin/eslint.js app components lib store i18n.ts navigation.ts proxy.ts routing.ts tailwind.config.ts next.config.mjs --ext .js,.mjs,.cjs,.ts,.tsx --max-warnings 0 }
  } finally { Pop-Location }
  ...
  Push-Location p75-work/source
  try { RunExternal 'p78r5_context_authenticity_and_pinned_pair_runtime' { node ./node_modules/tsx/dist/cli.mjs ../../p78-runtime/test-p78r5-runtime.mts } } finally { Pop-Location }
  ...
  RunExternal 'capture_next_env' { python p78-runtime/verify-p78r5-projection.py --source-root p75-work/source --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --mode capture-next-env --snapshot $nextEnvSnapshot --receipt "$outDir/P78R5_NEXT_ENV_CAPTURE.json" --build-label before_builds }
  ...
  RunExternal 'webpack_projection_mutation_control' { python p78-runtime/verify-p78r5-projection.py --source-root p75-work/source --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --mode post-build --snapshot $nextEnvSnapshot --receipt "$outDir/P78R5_WEBPACK_PROJECTION_CONTROL.json" --build-label webpack }
  ...
  RunExternal 'turbopack_projection_mutation_control' { python p78-runtime/verify-p78r5-projection.py --source-root p75-work/source --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --mode post-build --snapshot $nextEnvSnapshot --receipt "$outDir/P78R5_TURBOPACK_PROJECTION_CONTROL.json" --build-label turbopack }
  RunExternal 'projection_final' { python p78-runtime/verify-p78r5-projection.py --source-root p75-work/source --manifest p75-work/P78R5_BUILD_PROJECTION_MANIFEST.json --mode check --receipt "$outDir/P78R5_PROJECTION_FINAL.json" --build-label final }

  $lockAfter = FileSha 'p75-work/source/package-lock.json'
  $packageAfter = FileSha 'p75-work/source/package.json'
  if ($lockBefore -ne $lockAfter -or $packageBefore -ne $packageAfter) { throw 'package-lock.json or package.json changed during P78R5 execution' }
  ...
  $receipt = [ordered]@{
    schemaVersion = 'velmere.p78r5.exact-windows-engineering.v1'
    status = $script:status
    startedAt = $script:startedAt
    finishedAt = (Get-Date).ToUniversalTime().ToString('o')
    parent = [ordered]@{
      revision = 'P78R4/V17'
      aggregateSha256 = '280fa6aa59b39e52c695951664664d525d12ab46124becd8c1ac1ff3539d6432'
      measuredClassNameOnlyFalsePositive = $true
    }
    candidate = [ordered]@{
      revision = 'P78R5/V17'
      fileCount = 1602
    ...
    detectorTruth = [ordered]@{
      exactR4ParentRuntimeReprovedBeforePatch = 'TESTED'
      classNameOnlyFalsePositiveStillObserved = $false
      openZeppelinImportAuthenticity = 'TESTED'
      sourceSemanticAuthenticity = 'TESTED'
      crossUnitCorrelation = 'TESTED_IN_R5_RUNTIME'
      pinnedThirdwebVulnerableFixedPair = 'TESTED_WITH_TARGET_AND_AUTHENTICITY_BINDING'
    ...
    truthBoundary = 'Native Windows engineering/regression proof for exact P78R5 bytes. PASS proves deterministic reconstruction, exact R4 parent runtime re-proof before the single-file authenticity repair, class-name false-positive removal, target/inheritance correlation, authenticated ERC2771 source controls, pinned historical vulnerable/fixed separation, prior-pass regressions and dual Next builds. It does not prove deployed runtime exploitability, population detector accuracy, customer FINAL, final PDF, rights, paid value, sale eligibility, LIVE or WORLD_CLASS.'
  }
  $receipt | ConvertTo-Json -Depth 12 | Set-Content "$outDir/P78R5_EXACT_WINDOWS_ENGINEERING.json" -Encoding utf8
}
