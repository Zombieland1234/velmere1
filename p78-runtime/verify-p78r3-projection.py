...
def compare(root: Path, manifest: dict) -> dict[str, object]:
    ...
    return {"exact": exact, "missing": missing, "unexpected": unexpected, "mismatches": mismatches, "identity": observed_identity}
...
def main() -> None:
    ...
    result: dict[str, object] = {
        "schemaVersion": "velmere.p78r3.exact-projection-control.v1",
        "mode": args.mode,
    ...
    if result["status"] != "PASS":
        raise SystemExit(f"P78R3 exact projection control failed:{args.mode}:{args.build_label}")
