...
$outDir = 'p78r4-windows-out'
$groundTruthDir = 'p78r4-windows-groundtruth'
...
function RunExternal([string]$name, [scriptblock]$block) {
  Write-Host "P78R4 STEP START $name"
  & $block
  if ($LASTEXITCODE -ne 0) { throw "P78R4 external step failed: $name exit=$LASTEXITCODE" }
  $script:completed.Add($name)
  Write-Host "P78R4 STEP PASS $name"
}
...
try {
  ...
  RunExternal 'p78r4_static' { python p78-runtime/test-p78r4-static.py --source-root p75-work/source --parent-manifest p75-work/P78R3_BUILD_PROJECTION_MANIFEST.json --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --source-receipt "$outDir/P78R4_SOURCE_PATCH.json" --receipt "$outDir/P78R4_STATIC_CONTROL.json" }
  RunExternal 'projection_preinstall' { python p78-runtime/verify-p78r4-projection.py --source-root p75-work/source --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --mode check --receipt "$outDir/P78R4_PROJECTION_PREINSTALL.json" --build-label preinstall }

  RunExternal 'fetch_pinned_thirdweb_pair' { python p78-runtime/fetch-p78r3-thirdweb-pinned.py --output-dir $groundTruthDir --receipt "$outDir/P78R4_THIRDWEB_PINNED_SOURCE_TRANSPORT.json" }
  ...
  # R3's historical pair omitted ContractName by design. Under R4 this bundle is intentionally ambiguous,
  # so run the inherited local R3 controls without that old ground-truth injection.
  Remove-Item Env:P78R3_GROUNDTRUTH_DIR -ErrorAction SilentlyContinue
  $env:P78R3_RESULT_DIR = (Resolve-Path $outDir).Path
  Push-Location p75-work/source
  try { RunExternal 'p78r3_local_detector_regression' { node ./node_modules/tsx/dist/cli.mjs ../../p78-runtime/test-p78r3-runtime.mts } } finally { Pop-Location }

  $env:P78R4_RESULT_DIR = (Resolve-Path $outDir).Path
  $env:P78R4_GROUNDTRUTH_DIR = (Resolve-Path $groundTruthDir).Path
  Push-Location p75-work/source
  try { RunExternal 'p78r4_target_correlation_and_pinned_pair_runtime' { node ./node_modules/tsx/dist/cli.mjs ../../p78-runtime/test-p78r4-runtime.mts } } finally { Pop-Location }
  ...
  RunExternal 'capture_next_env' { python p78-runtime/verify-p78r4-projection.py --source-root p75-work/source --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --mode capture-next-env --snapshot $nextEnvSnapshot --receipt "$outDir/P78R4_NEXT_ENV_CAPTURE.json" --build-label before_builds }
  ...
  RunExternal 'webpack_projection_mutation_control' { python p78-runtime/verify-p78r4-projection.py --source-root p75-work/source --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --mode post-build --snapshot $nextEnvSnapshot --receipt "$outDir/P78R4_WEBPACK_PROJECTION_CONTROL.json" --build-label webpack }
  ...
  RunExternal 'turbopack_projection_mutation_control' { python p78-runtime/verify-p78r4-projection.py --source-root p75-work/source --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --mode post-build --snapshot $nextEnvSnapshot --receipt "$outDir/P78R4_TURBOPACK_PROJECTION_CONTROL.json" --build-label turbopack }
  RunExternal 'projection_final' { python p78-runtime/verify-p78r4-projection.py --source-root p75-work/source --manifest p75-work/P78R4_BUILD_PROJECTION_MANIFEST.json --mode check --receipt "$outDir/P78R4_PROJECTION_FINAL.json" --build-label final }

  $lockAfter = FileSha 'p75-work/source/package-lock.json'
  $packageAfter = FileSha 'p75-work/source/package.json'
  if ($lockBefore -ne $lockAfter -or $packageBefore -ne $packageAfter) { throw 'package-lock.json or package.json changed during P78R4 execution' }
  ...
  $receipt = [ordered]@{
    schemaVersion = 'velmere.p78r4.exact-windows-engineering.v1'
    ...
    parent = [ordered]@{
      ...
      knownCrossUnitFalsePositive = $true
    }
    candidate = [ordered]@{
      revision = 'P78R4/V17'
      fileCount = 1602
    ...
    correlationTruth = [ordered]@{
      measuredP78R3CrossUnitFalsePositive = 'REPRODUCED_IN_SEPARATE_DIAGNOSTIC'
      targetContractNameBinding = 'TESTED_PRIVATE_DIGEST_BOUND'
      inheritanceClosureCorrelation = 'TESTED'
      crossUnitFalsePositiveStillObserved = $false
      pinnedThirdwebVulnerableFixedPair = 'TESTED_WITH_TARGET_CONTRACT_NAME'
    ...
    truthBoundary = 'Native Windows engineering/regression proof for exact P78R4 bytes. PASS proves deterministic reconstruction, target/inheritance correlation behavior, exact private ContractName handoff, bounded detector regressions, pinned historical vulnerable/fixed separation, inherited prior-pass regressions and dual Next builds. It does not prove deployed runtime exploitability, population accuracy, customer FINAL, final PDF, rights, paid value, sale eligibility, LIVE or WORLD_CLASS.'
  }
  $receipt | ConvertTo-Json -Depth 12 | Set-Content "$outDir/P78R4_EXACT_WINDOWS_ENGINEERING.json" -Encoding utf8
}
