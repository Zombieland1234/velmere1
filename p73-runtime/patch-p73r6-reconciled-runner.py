...
if sha(b)!=BEFORE_SHA:raise SystemExit(f'P73R6 runner preimage mismatch:{len(b)}:{sha(b)}')
m=json.loads(Path(a.manifest).read_text(encoding='utf-8'));current=m['projection'];gp,ga=generated_identity(m)
s=b.decode('utf-8')
if s.count(OLD_GENERATED_PAYLOAD)!=1:raise SystemExit(f'P73R6 generated payload anchor mismatch:{s.count(OLD_GENERATED_PAYLOAD)}')
if s.count(OLD_GENERATED_AGG)!=1:raise SystemExit(f'P73R6 generated aggregate anchor mismatch:{s.count(OLD_GENERATED_AGG)}')
s=s.replace(OLD_GENERATED_PAYLOAD,str(gp),1).replace(OLD_GENERATED_AGG,ga,1)
out=s.encode('utf-8');p.write_bytes(out)
r={'schemaVersion':'velmere.p73r6.reconciled-runner-patch.v1','status':'PASS','inputSha256':BEFORE_SHA,'outputSha256':sha(out),'bytes':len(out),'currentProjectionFileCount':current['fileCount'],'currentProjectionPayloadBytes':current['payloadBytes'],'currentProjectionPathSetSha256':current['pathSetSha256'],'currentProjectionAggregateSha256':current['sourceContentAggregateSha256'],'currentProjectionBinding':'RUNTIME_MANIFEST_NOT_LITERAL_PATCH','generatedNextEnvProjectionPayloadBytes':gp,'generatedNextEnvProjectionAggregateSha256':ga,'patchedAnchors':['controlledGeneratedFile.generatedProjectionPayloadBytes','controlledGeneratedFile.generatedProjectionAggregateSha256'],'truthBoundary':'Manifest-bound control-plane adaptation for exact P73R6 product bytes. It computes only the deterministic next-env.d.ts generated identity and grants no customer FINAL/PDF, rights, paid-value, sale, LIVE or WORLD_CLASS credit.'}
