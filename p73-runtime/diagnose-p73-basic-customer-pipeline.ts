...
async function main() {
  ...
  let pipelineWithoutLedger: ReturnType<typeof buildPass4820AuditCustomerReportPipeline> | null = null;
  ...
  let pipelineWithLedger: ReturnType<typeof buildPass4820AuditCustomerReportPipeline> | null = null;
  ...
  const result = {
    ...
    sourceQuorum: { overall: sourceQuorum.overall, laneStates: sourceQuorum.lanes.map((x) => ({ id:x.id, state:x.state, family:x.family, evidence:x.evidence.length, missing:x.missing })) },
    providerRuntime: {
      summary: providerRuntime.summary,
      strictLanes: providerRuntime.lanes.filter(isStrictAuditEvidenceLane).map((x) => ({ id:x.id, provider:x.provider, providerFamily:x.providerFamily, upstreamRoot:x.lineage?.upstreamRoot, bodyDigest:x.receipt?.bodyDigest, state:x.state })),
      lanes: providerRuntime.lanes.map((x) => ({ id:x.id, provider:x.provider, state:x.state, identity:x.identity, receipt: x.receipt ? { statusCode:x.receipt.statusCode, bodyBytes:x.receipt.bodyBytes, bodyDigest:x.receipt.bodyDigest } : null, missing:x.missing })),
    },
    runtimeConfidence: runtimeConfidence.overall,
    claimLedger: { summary: claimLedger.summary, confirmedAdverseLikeClaims: claimLedger.claims.filter((x) => x.grade==='confirmed' && ['identity','security_flags','source_code','public_audit'].includes(x.category)).map((x) => ({ id:x.id, category:x.category, claim:x.claim, sourceFamily:x.sourceFamily, customerLine:x.customerLine })) },
    publicSources: { summary: publicSources.summary, receipts: publicSources.receipts.map((x) => ({ kind:x.kind, state:x.state, requestedUrl:x.requestedUrl, finalUrl:x.finalUrl, contentBound:x.contentBound, exactAddressPresent:x.identity.exactAddressPresent, chainMentioned:x.identity.chainMentioned, scopeSignals:x.scopeSignals, missing:x.missing, bodyDigest:x.bodyDigest })) },
    report: { finalVerdict: report.finalVerdict, summary: report.summary, topFindings: report.topFindings },
    basicReadiness: readiness,
    pipelineWithoutLedger: pipelineWithoutLedger ? { releaseState:pipelineWithoutLedger.releaseState, deliveredTier:pipelineWithoutLedger.deliveredTier, visibleTier:pipelineWithoutLedger.customerReport?.deliveryPolicy?.visibleTier, blockedReasons:pipelineWithoutLedger.customerReport?.deliveryPolicy?.blockedReasons } : null,
