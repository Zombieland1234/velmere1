...
async function handlePass4824AuditProviderGet(request: Request) {
  ...
  const evidenceReadiness = evaluateAuditPaidEvidenceReadiness({
  ...
  const evidenceLedger = await appendProviderEvidencePacket({
    ...
    metadata: {
      tier,
      paymentVerified,
      evidenceReady: evidenceReadiness.met,
  ...
  if (customerPipeline.customerReport.deliveryPolicy.visibleTier === null) {
    return NextResponse.json({
      ok: false,
      ...
      blockedReasons: customerPipeline.customerReport.deliveryPolicy.blockedReasons,
