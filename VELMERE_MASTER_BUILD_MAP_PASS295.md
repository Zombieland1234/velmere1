...
PASS289 adds the Viewport Glass Sentinel for J03/J04/D20: Orbit detail scroll, right-edge sheet behavior, compact proof density, PDF forge control placement, mobile safe areas and anti-FOMO layout copy. Public/customer report status remains bounded by proof/source/privacy/retention gates.
...
PASS290 adds the **Private Disclosure Loom** for M07. Customer-visible report lines, redacted appendix fields and operator-only reasoning are separated before PDF/customer report release. It also fixes the PASS289 TypeScript issue where `layout-stability-sentinel-gate.ts` referenced `result.missingInputs` outside the `TokenRiskResult` contract.
...
PASS291 adds the **Ghost Replay Seal** for the M08 PDF/browser replay boundary. The branded Velmère Cybersecurity PDF preview now stays behind right-edge Orbit scroll proof, source appendix replay, operator-field separation and non-advisory signature boundaries. This pass also keeps the previous runtime download callbacks guarded so evidence/PDF/replay buttons never render against undefined handlers.
...
<!-- PASS291 marker: PDF browser replay boundary gate active. -->

## PASS293 — Social-Exchange Command Router Gate

PASS293 unifies the main Shield search, Shield Map investigator search and VLM Browser/Lens suggestions through one reusable Social-Exchange Command Router. It combines exchange-style decision proximity, social-feed-style signal ranking and ethical anti-FOMO psychology without exposing private scoring weights or adding buy/sell pressure.

- C02/C03: Shield search suggestions now show router score, source mode, depth/social/psychology reason pills and one next safe action.
- E02: VLM Browser/Lens suggestions now use the same router contract, making the browser feel like a discovery/capsule/Shield-handoff surface instead of a thin search page.
- D16/D17: Suggestions expose source state and missing-evidence pressure without pretending certainty.
- A03: Removed a duplicated `body` key from the Shield Map investigator evidence type.
- A05: Added a PASS293 guard and appended it to `verify:shield-all`.
...
PASS294 adds a transparent ranking layer over the PASS293 Social-Exchange Router. It turns exchange-style depth context, social-feed-style ranking and operator boundaries into visible trust reasons instead of hidden engagement pressure.

- E02: VLM Browser results now show a Trust Signal Feed with priority, rank reason and public-safe boundary per result.
- C02/C03: Shield and Shield Map search surfaces now show PASS294 trust-sync rails so source quorum, depth context and operator boundary stay aligned with the unified search contract.
- D16/D17: Source confidence and missing-data semantics are visible inside result cards through source quorum, operator boundary and anti-FOMO lanes.
- J03/J04: Result cards use compact responsive grids; mobile collapses to one column without covering the search or result content.
- A05: Added `verify:pass294-trust-signal-feed-gate` and appended it to `verify:shield-all`.
...
**PASS294 product delta:** +11% on touched rows.

<!-- PASS294 marker: Trust Signal Feed Gate active. -->

## PASS295 — Decision Flow Orchestrator Gate

PASS295 adds a transparent decision-flow layer over PASS293/PASS294. It turns search, source confidence, exchange depth and social context into one calm operator next action with an explicit decision receipt. This keeps the product premium and intelligent without using countdowns, fake scarcity, hidden engagement-only ranking or buy/sell pressure.

- E02: VLM Browser/Lens now shows a Decision Flow rail and per-result decision receipt.
- C02/C03: Shield and Shield Map search surfaces now sync the same decision flow: source lock → depth check → social context → operator action.
- D16/D17: Source confidence and missing-data states now control friction states such as `source_first`, `slow_down` and `go_to_review`.
- M01/M03: Report/customer brief direction gets a calm receipt model before PDF/report generation.
- J03: New decision-flow grids collapse cleanly on tablet/mobile.
- A05: Added `verify:pass295-decision-flow-orchestrator-gate` and appended it to `verify:shield-all`.

| ID | Obszar | Previous | Current | Change |
|---|---|---:|---:|---:|
| E02 | Lens search UX | 98% | 99% | +1% |
| C02 | Shield search dropdown | 98% | 99% | +1% |
| C03 | Global token lookup | 68% | 70% | +2% |
| D16 | Source confidence lanes | 91% | 92% | +1% |
| D17 | Missing-data semantics | 92% | 93% | +1% |
| M01 | Velmère Shield Report | 72% | 74% | +2% |
| M03 | Evidence Note | 75% | 77% | +2% |
| J03 | Responsive layout | 86% | 87% | +1% |
| A05 | Preflight guard system | 100% | 100% | +0% |

**PASS295 product delta:** +11% on touched rows.

<!-- PASS295 marker: Decision Flow Orchestrator Gate active. -->
