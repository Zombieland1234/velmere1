...
const OUT = process.env.P74_RESULT_DIR || path.resolve('p74-ancient8-official-replay-v3-out');
...
async function getJson(url) {
  const response = await fetch(url, {
    headers: { accept: 'application/json', 'user-agent': 'VelmereP74ReplayV3/1.0' },
...
async function rpc(method, params = []) {
  const body = JSON.stringify({ jsonrpc: '2.0', id: 7411, method, params });
  const response = await fetch(RPC, {
    method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'VelmereP74ReplayV3/1.0' },
...
function installSolc(tmp) {
  if (process.platform === 'win32') {
    ...
    const command = `cd /d "${tmp}" && npm install --no-audit --no-fund --ignore-scripts solc@0.8.26`;
    return run(comspec, ['/d', '/s', '/c', command]);
  }
  return run('npm', ['install', '--no-audit', '--no-fund', '--ignore-scripts', 'solc@0.8.26'], { cwd: tmp });
}

const result = {
  schemaVersion: 'velmere.p74r3.ancient8-official-multicall3-replay.v3',
...
try {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'velmere-p74r3-solc-'));
...
fs.writeFileSync(path.join(OUT, 'P74R3_ANCIENT8_OFFICIAL_MULTICALL3_REPLAY.json'), `${JSON.stringify(result, null, 2)}\n`);
