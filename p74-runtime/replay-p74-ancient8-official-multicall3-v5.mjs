...
const OUT = process.env.P74_RESULT_DIR || path.resolve('p74-ancient8-official-replay-v5-out');
...
async function getJson(url) {
  const response = await fetch(url, {
    headers: { accept: 'application/json', 'user-agent': 'VelmereP74ReplayV5/1.0' },
...
async function rpc(method, params = []) {
  const body = JSON.stringify({ jsonrpc: '2.0', id: 7413, method, params });
  const response = await fetch(RPC, {
    method: 'POST',
    headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'VelmereP74ReplayV5/1.0' },
...
function installSolc(tmp) {
  ...
  const install = run(process.execPath, [npmCli, 'install', '--no-audit', '--no-fund', '--ignore-scripts', 'solc@0.8.26'], { cwd: tmp });
  return { ...install, npmCli };
}

function sanitizeExplorerStdJsonInput(raw) {
  const std = structuredClone(raw);
  const removedFields = [];
  const allowedSourceKeys = new Set(['content', 'urls', 'keccak256']);
  if (!std || std.language !== 'Solidity' || !std.sources || typeof std.sources !== 'object') throw new Error('invalid_std_json_input');
  for (const [sourceName, source] of Object.entries(std.sources)) {
    if (!source || typeof source !== 'object') throw new Error(`invalid_source_entry:${sourceName}`);
    for (const key of Object.keys(source)) {
      if (key === 'highlightedHtml') {
        delete source[key];
        removedFields.push(`${sourceName}.highlightedHtml`);
      } else if (!allowedSourceKeys.has(key)) {
        throw new Error(`unexpected_explorer_source_key:${sourceName}:${key}`);
      }
    }
  }
  return { std, removedFields };
}

const result = {
  schemaVersion: 'velmere.p74r5.ancient8-official-multicall3-replay.v5',
  ...
  truthBoundary: 'PASS may grant only bounded source-to-current-runtime identity for the official Ancient8 Multicall3 deployment. Explorer-only highlightedHtml is removed before solc because it is not a Solidity standard-json input field; source content, compiler settings and target identity are preserved. Independent-provider quorum and every product/customer/commercial gate remain separate.',
};
...
try {
  ...
  const j = source.json;
  const rawStd = structuredClone(j.stdJsonInput);
  ...
  if (rawStd?.language !== 'Solidity' || !rawStd?.sources?.['Multicall3.sol']?.content) throw new Error('std_json_source_missing');
  if (rawStd?.settings?.optimizer?.enabled !== true || rawStd?.settings?.optimizer?.runs !== 200) throw new Error('optimizer_settings_mismatch');

  const sourceText = rawStd.sources['Multicall3.sol'].content;
  const rawStdJsonSha256 = sha256(Buffer.from(JSON.stringify(rawStd)));
  const { std, removedFields } = sanitizeExplorerStdJsonInput(rawStd);
  const sanitizedStdJsonSha256 = sha256(Buffer.from(JSON.stringify(std)));
  const sourceAfterSanitize = std.sources['Multicall3.sol'].content;
  if (sourceAfterSanitize !== sourceText) throw new Error('source_changed_during_sanitization');
  if (removedFields.length !== 1 || removedFields[0] !== 'Multicall3.sol.highlightedHtml') throw new Error(`unexpected_sanitization:${removedFields.join(',')}`);
  ...
  const compileInputSha256 = sha256(Buffer.from(inputText));
  fs.writeFileSync(path.join(OUT, 'SOLC_STANDARD_INPUT.json'), `${inputText}\n`);
  fs.writeFileSync(path.join(OUT, 'SANITIZATION.json'), `${JSON.stringify({ removedFields, rawStdJsonSha256, sanitizedStdJsonSha256, compileInputSha256, sourcePreserved: sourceAfterSanitize === sourceText }, null, 2)}\n`);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'velmere-p74r5-solc-'));
  fs.writeFileSync(path.join(OUT, 'TEMP_DIR.txt'), `${tmp}\n`);
  const install = installSolc(tmp);
  fs.writeFileSync(path.join(OUT, 'NPM_CLI.txt'), `${install.npmCli}\n`);
  ...
  result.observations = {
    source: {
      httpStatus: source.status, responseSha256: source.sha256, verifiedAt: j.verifiedAt, matchId: j.matchId,
      match: j.match, creationMatch: j.creationMatch, runtimeMatch: j.runtimeMatch,
      rawStdJsonSha256, sanitizedStdJsonSha256, compileInputSha256, removedExplorerFields: removedFields,
      sourceBytes: Buffer.byteLength(sourceText), sourceSha256: sha256(Buffer.from(sourceText)), compilation: j.compilation,
    },
    toolchain: { os: process.platform, node: process.version, npmCli: install.npmCli, npmPackageVersion: pkg.version, solcVersionOutput: versionText },
  ...
  result.checks = {
    ...
    explorerOnlySanitizationExact: removedFields.length === 1 && removedFields[0] === 'Multicall3.sol.highlightedHtml' && sourceAfterSanitize === sourceText,
  ...
  } else result.status = 'FAIL_NO_PRODUCT_CREDIT';
} catch (error) {
  result.status = 'BLOCKED_NO_PRODUCT_CREDIT';
  result.errors.push(error instanceof Error ? `${error.name}:${error.message}` : String(error));
}

fs.writeFileSync(path.join(OUT, 'P74R5_ANCIENT8_OFFICIAL_MULTICALL3_REPLAY.json'), `${JSON.stringify(result, null, 2)}\n`);
