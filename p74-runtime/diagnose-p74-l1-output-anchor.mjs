...
async function main(){
 const result={schemaVersion:'velmere.p74.l1-output-anchor-discovery.v1',status:'DISCOVERY_RUNNING_NO_PRODUCT_CREDIT',generatedAt:new Date().toISOString(),l1:{chain:'ethereum',oracle:L2OO,providers:ETH},l2:{chain:'ancient8',chainId:888888888,rpc:'scan.ancient8.gg/rpc',messagePasser:MESSAGE_PASSER},observations:{},anchor:null,errors:[],credit:{product:0,currentRuntimeBytecode:0,vulnerabilityGroundTruth:0,customerFinal:0,sale:0,live:false},truthBoundary:'Control-only L1/L2 anchor discovery. No product/release credit. Ethereum providers must agree at one exact L1 block. L2 data are observations only until the OP output-root commitment is independently recomputed and matched to the L1 proposal.'};
 ...
 const [l2Chain,l2BlockRow,msgProof,canonicalProof,officialProof]=await Promise.all([
   rpc(A8,'eth_chainId'),
   rpc(A8,'eth_getBlockByNumber',[l2Tag,false]),
   rpc(A8,'eth_getProof',[MESSAGE_PASSER,[],l2Tag]),
   rpc(A8,'eth_getProof',['0xca11bde05977b3631167028862be2a173976ca11',[],l2Tag]),
   rpc(A8,'eth_getProof',['0xb76d6e8c82d06fd262ef3799db73d5a724108d4e',[],l2Tag]),
 ]);
 result.observations.l2={l2Chain,l2Block:l2BlockRow,messagePasserProof:msgProof,canonicalProof,officialProof};
 if(l2Chain.status!=='PASS'||String(l2Chain.result).toLowerCase()!=='0x34fb5e38')throw new Error(`l2_chain_mismatch:${l2Chain.result}`);
 if(l2BlockRow.status!=='PASS'||!l2BlockRow.result||typeof l2BlockRow.result!=='object')throw new Error('l2_block_unavailable');
 if(msgProof.status!=='PASS'||canonicalProof.status!=='PASS'||officialProof.status!=='PASS')throw new Error('l2_eth_getProof_unavailable');
...
main().catch(e=>{const fail={schemaVersion:'velmere.p74.l1-output-anchor-discovery-failure.v1',status:'DISCOVERY_BLOCKED_NO_PRODUCT_CREDIT',generatedAt:new Date().toISOString(),error:e instanceof Error?`${e.name}:${e.message}`:String(e),credit:{product:0,currentRuntimeBytecode:0,sale:0,live:false}};fs.writeFileSync(path.join(OUT,'P74_L1_OUTPUT_ANCHOR_FAILURE.json'),JSON.stringify(fail,null,2)+'\n');console.log(JSON.stringify(fail,null,2));});
