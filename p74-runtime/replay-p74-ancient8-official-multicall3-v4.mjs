...
const OUT = process.env.P74_RESULT_DIR || path.resolve('p74-ancient8-official-replay-v4-out');
...
async function getJson(url) {
  const response = await fetch(url, {
    headers: { accept: 'application/json', 'user-agent': 'VelmereP74ReplayV4/1.0' },
...
async function rpc(method, params = []) {
  const body = JSON.stringify({ jsonrpc: '2.0', id: 7412, method, params });
  const response = await fetch(RPC, {
    method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'VelmereP74ReplayV4/1.0' },
...
const result = {
  schemaVersion: 'velmere.p74r4.ancient8-official-multicall3-replay.v4',
...
try {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'velmere-p74r4-solc-'));
...
fs.writeFileSync(path.join(OUT, 'P74R4_ANCIENT8_OFFICIAL_MULTICALL3_REPLAY.json'), `${JSON.stringify(result, null, 2)}\n`);
