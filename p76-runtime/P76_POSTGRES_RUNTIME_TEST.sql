...
-- Optional internal QA may be recorded, but must leave execution queued and customer history empty.
do $$
...
-- Claim, retry settle, reclaim, then atomically complete with immutable snapshot.
do $$
declare r jsonb;
begin
...
-- Wrong lease must not reserve the immutable snapshot.
do $$
...
-- Correct lease completes review and snapshot in one transaction; then idempotence/conflict are enforced.
do $$
