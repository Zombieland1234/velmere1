...
$OldEnd = "} finally { Pop-Location }`n# === End Risk Indicator v5 source overlay ==="
$NewEnd = "} finally { Pop-Location }`n& node 'r7-risk-v5/regenerate-risk-v5-execution-manifest.mjs' 'r7-work' `$RiskReceiptPath`nif (`$LASTEXITCODE -ne 0) { throw \"risk_v5_manifest_regeneration_failed:`$LASTEXITCODE\" }`n# === End Risk Indicator v5 source overlay ==="
