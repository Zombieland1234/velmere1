...
const root = path.resolve(process.env.P69_SOURCE_ROOT || process.cwd());
...
export async function resolve(specifier, context, nextResolve) {
  if (specifier === "next/server") {
    const target = sourceRequire.resolve("next/server.js");
    return { url: pathToFileURL(target).href, shortCircuit: true };
  }
  if (specifier === "next/headers") {
    const target = sourceRequire.resolve("next/headers.js");
    return { url: pathToFileURL(target).href, shortCircuit: true };
  }
...
export async function load(url, context, nextLoad) {
  ...
  if (ts) {
    ...
    if (errors.length) throw new Error(`p69_ts_transpile_failed:${filePath}\n${ts.formatDiagnosticsWithColorAndContext(errors,{getCanonicalFileName:v=>v,getCurrentDirectory:()=>root,getNewLine:()=>"\n"})}`);
    return { format: "module", source: result.outputText, shortCircuit: true };
  }
  if (extension === ".tsx") throw new Error(`p69_tsx_transpile_requires_typescript:${filePath}`);
