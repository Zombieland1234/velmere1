...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "Velmere-PASS434-Provider-Crosscheck/1.0" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
...
console.log("PASS434 provider crosscheck & missing-data hunter · real providers before PDF/chat\n");
for (const query of targets) {
  ...
  console.log(`--- ${query} ---`);
  console.log(`answer gate: ${answerGate}${div !== null ? ` · price divergence ${div.toFixed(2)}%` : ""}`);
