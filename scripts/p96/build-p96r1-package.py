...
IDENTITY_REL = "artifacts/closure/p96r1/P96R1_TREE_IDENTITY_EXCLUDING_SELF.json"
...
def parse_manifest(root: Path) -> list[dict[str, Any]]:
    ...
    if [row["path"] for row in result] != sorted(row["path"] for row in result) or len({row["path"] for row in result}) != len(result):
        raise RuntimeError("manifest_order_or_duplicate")
    return result
...
def identical(left: Path, right: Path) -> bool:
    ...
    with left.open("rb") as a, right.open("rb") as b:
        while True:
            x, y = a.read(8 * 1024 * 1024), b.read(8 * 1024 * 1024)
            if x != y:
                return False
            if not x:
                return True
...
def clean_unpack(zip_path: Path, rows: list[dict[str, Any]]) -> str:
    with tempfile.TemporaryDirectory(prefix="p96-clean-") as temporary:
...
def main() -> None:
    ...
    root = Path(args.root).resolve(); output = Path(args.output).resolve(); verification = Path(args.verification).resolve()
    ...
    current_binaries = [row["path"] for row in rows if row["path"].startswith(("receipts/p96/", "artifacts/p96/", "scripts/p96/", "artifacts/closure/p96r1/")) and Path(row["path"]).suffix.lower() in {".pdf", ".zip", ".woff", ".woff2", ".ttf", ".otf", ".exe", ".dll", ".bin", ".pyc", ".pyo"}]
    if current_binaries:
        raise RuntimeError(f"unexpected_current_binary:{current_binaries}")
    with tempfile.TemporaryDirectory(prefix="p96-package-") as temporary:
        build_a = Path(temporary) / "a.zip"; build_b = Path(temporary) / "b.zip"
        build_zip(root, build_a, rows); build_zip(root, build_b, rows)
        if not identical(build_a, build_b):
            raise RuntimeError("not_deterministic")
        verify_a = verify_zip(build_a, rows); verify_b = verify_zip(build_b, rows); clean = clean_unpack(build_a, rows)
        output.parent.mkdir(parents=True, exist_ok=True); shutil.copyfile(build_a, output)
    ...
    payload = {
        "schemaVersion": "velmere.p96r1.package-verification.v1",
        "status": "PASS",
        "output": output.name,
        ...
        "truthBoundary": "SOURCE_ONLY identity, determinism, CRC, clean unpack and full-tree secret/current-binary scanning only. It grants no PostgreSQL, Browser, deployment, exact Windows or Customer FINAL credit.",
    }
