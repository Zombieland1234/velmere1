...
check("contract:revision", contract.revisionId === "VELMERE_PASS35_A54_STRICT_SLO_ALERT_ACK_VENDOR_EXIT_RECOVERY_ACCEPTANCE", contract.revisionId);
check("contract:checks", contract.requiredChecks?.length === 42, contract.requiredChecks?.length);
check("contract:env", contract.requiredEnvironment?.length === 21, contract.requiredEnvironment?.length);
const regression = spawnSync(process.execPath, ["scripts/pass35/test-a54-strict-slo-alert-ack-vendor-exit-recovery-acceptance.mjs"], { cwd: root, encoding: "utf8", maxBuffer: 128 * 1024 * 1024 });
check("regression:a54_contract", regression.status === 0, { status: regression.status, stdout: regression.stdout?.trim(), stderr: regression.stderr?.trim() });
if (staticOnly) {
...
const report = {
  schemaVersion: "velmere.pass35.a54.runtime-diagnostics.v1",
  revisionId: contract.revisionId,
  generatedAt: new Date().toISOString(),
  truthBoundary: staticOnly ? "Static A54 diagnostics; no staging or sale credit." : "Exact A54 preflight; secrets are never printed.",
...
if (write) {
  const target = path.join(root, "artifacts/pass35/a54/PASS35_A54_RUNTIME_DIAGNOSTICS.json");
