...
def main() -> None:
    ...
    check("ledger_p94", "P94R1" in ledger_text)
    ...
    check("ledger_public_only", "public API now paginates only publishable history" in ledger_text)
    check("ledger_time_truth", "chart spacing reflects real time" in ledger_text)
    ...
    with zipfile.ZipFile(source) as archive:
        ...
        check("zip_identity", "artifacts/closure/p94r1/P94R1_TREE_IDENTITY_EXCLUDING_SELF.json" in names_set)
        check("zip_checkpoint", "artifacts/closure/p94r1/P94R1_CHECKPOINT_RECEIPT.json" in names_set)
        check("zip_boundary", "artifacts/closure/p94r1/P94R1_RISK_HISTORY_PUBLIC_PAGINATION_BOUNDARY.json" in names_set)
        check("zip_failure", "artifacts/closure/p94r1/P94R1_FAILURE_ADJUDICATION.json" in names_set)
        check("zip_product_contract", "lib/market-integrity/risk-history-contract.ts" in names_set)
        check("zip_public_reader", "lib/market-integrity/risk-ledger.ts" in names_set)
        check("zip_customer_client", "lib/market-integrity/risk-history-customer-client.ts" in names_set)
        check("zip_ui", "components/market-integrity/RiskHistoryControl.tsx" in names_set)
        check("zip_public_route", "lib/server/market-integrity-route-modules/history.ts" in names_set)
        check("zip_migration", "supabase/migrations/20260821000001_p94_risk_history_public_only_pagination_temporal_window.sql" in names_set)
        check("zip_active_pass", archive.read("VELMERE_ACTIVE_PASS.txt").decode("utf-8").strip() == "P94R1")
    ...
    payload = {
        "schemaVersion": "velmere.p94r1.final-four-file-verification.v1",
        "status": "PASS",
        ...
        "truthBoundary": "Four-file handoff identity and package integrity only. It grants no PostgreSQL, deployed HTTP, rendered Browser, exact-Windows, Risk Indicator FINAL or Customer FINAL credit.",
    }
