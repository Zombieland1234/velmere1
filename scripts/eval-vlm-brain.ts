...
async function main() {
  ...
  assert.ok(packet.sourceArbitration.providerCount >= 0);
  ...
  assert.ok(singleSourceArbitration.reasons.some((reason) => reason.includes("independent")));
  ...
  assert.ok(oneExternalPacket.sourceArbitration.providerCount >= 0);
  assert.ok(oneExternalPacket.confidenceCap <= 39);

  const sessionId = "eval-session";
  writeVlmSessionMemory({ sessionId, assetId: "test-asset", locale: "pl", depth: "basic", surface: "angel", question: "hello", summary: "summary" });
  ...
  const approvedCandidate: VlmBrainOutput = {
    schemaVersion: "velmere.vlm.brain-output.v1",
    traceId,
    locale: "en",
    depth: "pro",
    ...
    asset: packet.asset,
    verdict: "review",
    headline: "Deterministic risk evaluation",
    summary: "Deterministic risk review based on available facts.",
    confidence: packet.confidenceCap,
    facts: packet.facts,
    keyFindings: [{ id: "deterministic-risk", title: "Risk review", explanation: `Risk ${packet.deterministicScore}/100.`, severity: "warning", confidence: packet.confidenceCap, sourceIds: ["internal:risk-engine"] }],
    contradictions: [],
    missingData: [],
    nextChecks: ["Verify sources"],
    sources: packet.sources,
    report: {
      executiveSummary: "Review summary", marketStructure: "Market structure", liquidityAnalysis: "Liquidity", holderAnalysis: "Holders", contractAnalysis: "Contract", sourceAssessment: "Sources", riskScenarios: "Scenarios", conclusion: "Conclusion",
    },
  };
  ...
  const approvedGate = evaluateVlmShadowReview(shadowApprove, approvedCandidate, packet);
  assert.equal(approvedGate.publish, true);
  assert.ok(approvedGate.status === "approved" || approvedGate.status === "revised");
  ...
  try {
    const receipt = createVlmAnalysisReceipt({
      ...
      mode: "gemini",
      facts: packet,
      output: approvedCandidate,
      shadowStatus: "approved",
      shadowModel: "test-shadow",
    });
    assert.equal(receipt.signing, "hmac-sha256");
    assert.equal(verifyVlmAnalysisReceipt({ receipt, facts: packet, output: approvedCandidate }).valid, true);
    const tamperedOutput = { ...approvedCandidate, confidence: approvedCandidate.confidence - 1 };
    ...
    const signatureTamper = verifyVlmAnalysisReceipt({ receipt: tamperedReceipt, facts: packet, output: approvedCandidate });
