...
const output = outputIndex >= 0 ? process.argv[outputIndex + 1] : "artifacts/pass12/PASS12_FULL_TREE_AUDIT.json";
const manifestIndex = process.argv.indexOf("--manifest");
const manifestOutput = manifestIndex >= 0 ? process.argv[manifestIndex + 1] : "artifacts/pass12/PASS12_FULL_TREE_MANIFEST.json";
...
const IGNORE_DIRS = new Set([".git", ".next"]);
    ...
    if (!SOURCE_ANALYSIS_EXCLUDES.some((prefix) => relative.startsWith(prefix))) {
        ...
        for (const specifier of imports) {
          if (specifier.startsWith("./") || specifier.startsWith("../") || specifier.startsWith("@/")) {
            if (!await resolveLocalImport(relative, specifier)) {
              const buildGenerated = relative === "next-env.d.ts" && specifier.startsWith("./.next/types/");
      ...
      if (TS_EXTENSIONS.has(extension)) {
        ...
        try {
          ...
          const transpileErrors = relative.endsWith(".d.ts")
            ? []
...
await writeFile(path.resolve(root, manifestOutput), `${JSON.stringify({
  schemaVersion: "velmere.pass11.full-tree-manifest.v1",
