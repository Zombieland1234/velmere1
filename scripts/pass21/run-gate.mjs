...
const policyPath = path.join(root, "config", "pass21", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS21 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass21-gates");
...
console.log(`PASS21 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass21.gate-execution.v1",
...
console.log(`PASS21 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS21 gate receipt: ${path.relative(root, receiptPath)}`);
