...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "Velmere-PASS437-Adaptive-Evidence-Planner/1.0" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
function missing(row) { return [row.price ? null : "price", row.change24h === null || row.change24h === undefined ? "24h change" : null, row.volume24h ? null : "24h volume", row.marketCap ? null : "market cap", row.liquidity ? null : "liquidity", row.candles ? null : "candles", row.security ? null : "security flags", row.providerCount >= 2 ? null : "second provider"].filter(Boolean); }
...
async function probeCoinGecko(query) {
  ...
  return { provider: "CoinGecko markets", family: "crypto_market", status: "ok", symbol: String(row.symbol || "").toUpperCase(), price: num(row.current_price), change24h: num(row.price_change_percentage_24h_in_currency ?? row.price_change_percentage_24h), volume24h: num(row.total_volume), marketCap: num(row.market_cap), liquidity: null, candles: Array.isArray(row.sparkline_in_7d?.price) ? row.sparkline_in_7d.price.length : 0, security: false };
}
...
async function probeBinance(query) {
  ...
  return { provider: "Binance spot klines", family: "candles", status: "partial", symbol, price: num(last), change24h: first ? num(((last - first) / first) * 100) : null, volume24h: num(volume), marketCap: null, liquidity: null, candles: Array.isArray(rows) ? rows.length : 0, security: false };
}
async function probeYahoo(query) {
  ...
  return { provider: "Yahoo Finance chart unofficial", family: "real_market", status: "partial", symbol, price: num(last), change24h: first && last ? num(((last - first) / first) * 100) : null, volume24h: num(volume), marketCap: null, liquidity: null, candles: closes.length, security: false };
}
...
function plan(rows) {
  const ok = rows.filter((row) => row.status === "ok" || row.status === "partial");
  const providerCount = ok.length;
  const fields = Array.from(new Set(rows.flatMap((row) => missing({ ...row, providerCount }))));
  const critical = fields.filter((field) => ["price", "second provider", "candles"].includes(field));
  const mode = critical.length >= 2 ? "operator_review" : critical.length ? "guided_probe" : "auto_probe_ready";
  const next = fields.includes("price") ? "CoinGecko/Yahoo price fallback" : fields.includes("second provider") ? "second independent provider lane" : fields.includes("liquidity") ? "DEX Screener liquidity lane" : fields.includes("security flags") ? "GoPlus security lane" : "provider health monitor";
  return { mode, next, fields, providerCount };
}
console.log("PASS437 adaptive evidence planner · what to probe next before PDF/chat\n");
for (const query of targets) {
  const rows = await Promise.all([probeCoinGecko, probeDexScreener, probeBinance, probeYahoo].map((fn) => settle(fn, query)));
  const p = plan(rows);
  console.log(`--- ${query} ---`);
  console.log(`planner: ${p.mode} · next: ${p.next} · provider lanes: ${p.providerCount}`);
  for (const row of rows) console.log(`${row.provider}: ${row.status} · ${row.symbol || "?"} · ${price(row.price)} · missing ${missing({ ...row, providerCount: p.providerCount }).join("/") || "none"}${row.error ? ` · error ${row.error}` : ""}`);
  console.log(`missing-data plan: ${p.fields.join(" · ") || "none"}\n`);
}
console.log("PASS437 invariant: plan next provider, keep no-fake-live, expose missing data, never let long memory override live source gaps.");
