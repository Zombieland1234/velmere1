...
const policyPath = path.join(root, "config", "pass15", "gate-policy.json");
...
function sourceTreeDigest() {
  ...
  const visit = (directory) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      if (entry.isDirectory() && (excludedDirectories.has(entry.name) || entry.name.startsWith(".next-pass25-"))) continue;
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS15 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass15-gates");
...
console.log(`PASS15 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass15.gate-execution.v1",
...
console.log(`PASS15 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS15 gate receipt: ${path.relative(root, receiptPath)}`);
