...
IDENTITY_REL='artifacts/closure/p99r1/P99R1_TREE_IDENTITY_EXCLUDING_SELF.json'
...
def clean_unpack(zp:Path,rows):
 with tempfile.TemporaryDirectory(prefix='p99-clean-') as td:
  t=Path(td)
...
def main():
 ...
 current_binaries=[r['path'] for r in rows if r['path'].startswith(('receipts/p99/','artifacts/p99/','scripts/p99/','artifacts/closure/p99r1/')) and Path(r['path']).suffix.lower() in {'.pdf','.zip','.woff','.woff2','.ttf','.otf','.exe','.dll','.bin','.pyc','.pyo'}]
 if current_binaries:raise RuntimeError(f'unexpected_current_binary:{current_binaries}')
 with tempfile.TemporaryDirectory(prefix='p99-package-') as td:
