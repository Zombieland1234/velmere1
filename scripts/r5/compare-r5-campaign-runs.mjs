...
const OUT = path.join(ROOT, "artifacts/r5/VELMERE_R5_CAMPAIGN_REPEATABILITY.json");
if (!firstPath || !secondPath) {
  process.stderr.write("usage: node scripts/r5/compare-r5-campaign-runs.mjs <run1.json> <run2.json>\n");
  process.exit(64);
}
...
const payload = {
  schemaVersion: "velmere.r5.local-current-execution-repeatability.v1",
  ...
  truthBoundary: "Repeatability is granted only to selected test set, summary, per-test classification and exit status. Some stdout payloads contain run-specific nonce/time/evidence values, so whole-campaign byte identity is explicitly not claimed.",
};
