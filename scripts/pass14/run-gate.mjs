...
const policyPath = path.join(root, "config", "pass14", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS14 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass14-gates");
fs.mkdirSync(receiptDirectory, { recursive: true });
const startedAt = new Date().toISOString();
const results = [];
let failed = false;

console.log(`PASS14 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass14.gate-execution.v1",
...
console.log(`PASS14 gate receipt: ${path.relative(root, receiptPath)}`);
