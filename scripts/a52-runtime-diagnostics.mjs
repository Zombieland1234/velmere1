...
check("contract:revision", contract.revisionId === "VELMERE_PASS35_A52_KILL_SWITCH_INCIDENT_CUSTOMER_COMMUNICATION_ACCEPTANCE", contract.revisionId);
check("contract:checks", contract.requiredChecks?.length === 30, contract.requiredChecks?.length);
check("contract:env", contract.requiredEnvironment?.length === 12, contract.requiredEnvironment?.length);
const regression = spawnSync(process.execPath, ["scripts/pass35/test-a52-kill-switch-incident-customer-communication-acceptance.mjs"], { cwd: root, encoding: "utf8", maxBuffer: 128 * 1024 * 1024 });
check("regression:a52_contract", regression.status === 0, { status: regression.status, stdout: regression.stdout?.trim(), stderr: regression.stderr?.trim() });
if (staticOnly) {
...
const report = { schemaVersion: "velmere.pass35.a52.runtime-diagnostics.v1", revisionId: contract.revisionId, generatedAt: new Date().toISOString(), truthBoundary: staticOnly ? "Static A52 diagnostics; no staging incident-response or customer-communication credit." : "Exact A52 preflight. Secret values are never printed.", runtime: { node: process.versions.node, npm: currentNpmVersion(), expectedNode: contract.runtime.node, expectedNpm: contract.runtime.npm }, summary: { checks: checks.length, passed: checks.length - failures.length, failed: failures.length, skipped: checks.filter((row) => row.skipped).length }, failures, checks };
if (write) { const target = path.join(root, "artifacts/pass35/a52/PASS35_A52_RUNTIME_DIAGNOSTICS.json"); fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, `${JSON.stringify(report, null, 2)}\n`); }
