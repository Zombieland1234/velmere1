...
const PASS_ID = "PASS4358";
const SUMMARY = path.join("artifacts", "live-receipts", "final-release-candidate", "final-public-release-candidate-summary.json");
const FAILURE = path.join("artifacts", "failure-artifacts", "final-release-candidate", "final-public-release-candidate-failure.json");
...
const REQUIRED_ROWS = [
  ...
  ["release_evidence_ledger_green", "artifacts/live-receipts/release-evidence-ledger/release-evidence-ledger-summary.json", ["ok", "decision", "requiredRowCount", "greenRowCount", "rootHash"], "executed_pass"],
];
function parseArgs(argv) {
  const out = { sourceRoot: ".", allowMissing: false, writeReceipt: true, maxAgeMs: 20 * 60 * 1000, shaIndex: "VELMERE_PASS4358_SHA256SUMS.txt" };
...
function freshness(value, maxAgeMs, lane) {
  const requires = lane.includes("hosted") || lane.includes("final") || lane.includes("signed_operator");
  if (!requires) return { ok: true, ageMs: null, reason: "not_a_freshness_lane" };
...
function row(root, spec, args) {
  ...
  const fresh = parsed.ok ? freshness(parsed.value, args.maxAgeMs, lane) : { ok: false, ageMs: null, reason: "missing" };
...
function shaIndexRow(root, shaIndex) {
  ...
  const text = read(abs);
  const exists = text.length > 0;
  const hasZip = /velmere_pass4358_.*\.zip/.test(text);
  const hasTxt = /VELMERE_PASS4358_.*\.txt/.test(text) || /VELMERE_PASS4358_MASTER_PLAN/.test(text);
  const green = exists && hasZip && hasTxt;
  return { schema: "velmere.pass4358.final_public_release_candidate_row.v1", passId: PASS_ID, lane: "release_sha256_index_present", canonicalPath: rel, exists, parsedOk: true, status: green ? "executed_pass" : exists ? "sha_index_incomplete" : "missing", decision: green ? "release_index_present" : "not_green", requiredDecision: "release_index_present", green, expectedFields: ["sha256_index"], missingFields: green ? [] : ["zip_or_txt_sha256_entry"], safePublicFieldsOnly: true, publicLeakTokens: [], freshness: { ok: true, ageMs: null, reason: "release_index_is_static_artifact" }, sha256: exists ? sha256(text) : null, bytes: exists ? Buffer.byteLength(text, "utf8") : 0, blocksPublicTopkaLive: true, blocksReleaseCandidateSeal: true };
}
function agreement(rows) {
  const byLane = new Map(rows.map((r) => [r.lane, r]));
  const problems = [];
  if (byLane.get("final_preflight_ready")?.green && !byLane.get("signed_operator_manifest_verified")?.green) problems.push("ready_preflight_without_signed_manifest");
  ...
  if (byLane.get("final_preflight_ready")?.green && !byLane.get("release_evidence_ledger_green")?.green) problems.push("ready_preflight_without_release_evidence_ledger");
  return problems;
}
const args = parseArgs(process.argv.slice(2));
const rows = [...REQUIRED_ROWS.map((spec) => row(args.sourceRoot, spec, args)), shaIndexRow(args.sourceRoot, args.shaIndex)];
const failedRows = rows.filter((r) => !r.green);
const agreementProblems = agreement(rows);
const releaseDecision = failedRows.length === 0 && agreementProblems.length === 0 ? "READY_TO_SEAL" : "BLOCKED";
const summary = {
  schema: "velmere.pass4358.final_public_release_candidate_summary.v1",
  passId: PASS_ID,
  generatedAtIso: new Date().toISOString(),
  ok: releaseDecision === "READY_TO_SEAL",
  releaseDecision,
  status: releaseDecision === "READY_TO_SEAL" ? "executed_pass" : "blocked_until_all_release_candidate_rows_green",
  noVisualChanges: true,
  publicTopkaLiveAllowed: false,
  claimAllowed: false,
  readyToSealPublicReleaseCandidate: releaseDecision === "READY_TO_SEAL",
  ...
  failedRows,
  agreementProblems,
  nextAction: releaseDecision === "READY_TO_SEAL" ? "Verify signed manifest one final time, publish redacted public proof status and attach SHA256 release index." : "Execute all Windows/hosted/domain/operator proof steps and generate the PASS4358 SHA256 release index, then rerun proof:final-preflight and proof:release-candidate."
};
if (args.writeReceipt || summary.ok) writeJson(path.join(args.sourceRoot, SUMMARY), summary);
if (!summary.ok) {
  const failure = { schema: "velmere.pass4358.final_public_release_candidate_failure.v1", passId: PASS_ID, generatedAtIso: new Date().toISOString(), ok: false, releaseDecision, summaryPath: SUMMARY, failureArtifact: FAILURE, failedRows, agreementProblems, publicTopkaLiveAllowed: false, claimAllowed: false, nextAction: summary.nextAction };
  writeJson(path.join(args.sourceRoot, FAILURE), failure);
  console.error(`${PASS_ID} final public release candidate BLOCKED: ${failedRows.length}/${rows.length} rows not green, ${agreementProblems.length} agreement problems.`);
  if (!args.allowMissing) process.exitCode = 1;
} else {
  console.log(`${PASS_ID} final public release candidate READY_TO_SEAL: ${SUMMARY}`);
}
