...
const modelPath = path.join(root, "lib/market-integrity/risk-engine-model.ts");
const errors = [];
...
for (const [needle, message] of forbidden) {
  if (riskDomain.includes(needle)) errors.push(`risk-engine domain: ${message}`);
}

if (/\[\s*\.\.\.\s*[^;\n]*(?:\.values\(\)|\.keys\(\)|\.entries\(\))/.test(riskDomain)) {
...
if (!/metaModel/.test(riskDomain) || !/limitations/.test(riskDomain)) {
  errors.push("risk-engine.ts: metaModel/limitations handling is missing.");
}
if (!/AssetMode/.test(riskDomain) || !/stablecoin/.test(riskDomain) || !/rwa/.test(riskDomain)) {
  errors.push("risk-engine.ts: asset profile handling for stablecoin/RWA is missing.");
}
if (!/confidence/.test(riskDomain) || !/computeDataConfidence/.test(riskDomain)) {
  errors.push("risk-engine.ts: confidence model is missing.");
}
if (!/manual review required/i.test(riskDomain)) {
  errors.push("risk-engine.ts: operator-grade manual review language is missing.");
}
if (/\b(buy now|safe buy|guaranteed profit|scam proven|fraud confirmed|moon|easy money)\b/i.test(riskDomain)) {
...
if (signalUnionMatch) {
  ...
  const arraySignalIds = Array.from(
    riskDomain.matchAll(/const\s+[A-Z_]+_SIGNAL_IDS:\s*RiskSignalId\[\]\s*=\s*\[([\s\S]*?)\];/g),
  ).flatMap((match) => Array.from(match[1].matchAll(/"([^"]+)"/g)).map((m) => m[1]));

  const addSignalIds = Array.from(
    riskDomain.matchAll(/addSignal\(\s*signals,\s*\{[\s\S]*?id:\s*"([^"]+)"/g),
  ).map((m) => m[1]);
