...
export const DIAGNOSTICS_DIR = path.join(ROOT, ".velmere", "pass25-diagnostics");
export const BUILDS_DIR = path.join(ROOT, ".velmere", "pass25-builds");
...
const EXCLUDED_FILES = new Set([
  ...
  "CLEAN_SAFE_PAYLOAD_MANIFEST.json",
]);
...
export function sourceTreeDigest(root = ROOT) {
  ...
  const visit = (directory) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (entry.isDirectory() && (relativeParent === "" || relativeParent === ".") && EXCLUDED_DIRECTORY_NAMES.has(entry.name)) continue;
...
export function relativeToRoot(filePath, root = ROOT) {
