...
if (!new Set(["webpack", "turbopack"]).has(mode)) throw new Error("PASS25 build mode must be webpack or turbopack");

const root = process.cwd();
const checkpoint = sourceTreeDigest(root);
const buildId = `vlm-pass25-${mode}-${checkpoint.sha256.slice(0, 16)}`;
...
function outputContractCheck() {
  ...
  const exists = (label, filePath, kind = "file") => {
    let ok = false;
    try { const stat = fs.statSync(filePath); ok = kind === "directory" ? stat.isDirectory() : stat.isFile(); } catch (ignoredError) { void ignoredError; }
...
if (preflightErrors.length > 0) {
  const receipt = {
    schemaVersion: "velmere.pass25.build-watchdog.v2",
...
const receipt = {
  ...
  sourceBefore, sourceAfter, sourceImmutable,
