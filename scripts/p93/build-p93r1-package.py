...
IDENTITY_REL = "artifacts/closure/p93r1/P93R1_TREE_IDENTITY_EXCLUDING_SELF.json"
...
def clean_unpack(zip_path: Path, rows: list[dict[str, Any]]) -> str:
    with tempfile.TemporaryDirectory(prefix="p93-clean-") as temporary:
...
def main() -> None:
    ...
    current_binaries = [
        row["path"]
        for row in rows
        if row["path"].startswith(("receipts/p93/", "artifacts/p93/", "scripts/p93/", "artifacts/closure/p93r1/"))
    ...
    with tempfile.TemporaryDirectory(prefix="p93-package-") as temporary:
    ...
    payload = {
        "schemaVersion": "velmere.p93r1.package-verification.v1",
        "status": "PASS",
        "root": str(root),
        "output": output.name,
        ...
        "packageContentManifest": {
            "path": MANIFEST_REL,
            "bytes": (root / MANIFEST_REL).stat().st_size,
            "sha256": sha(root / MANIFEST_REL),
            "listedFiles": len(manifest),
            "verifiedExact": True,
        },
