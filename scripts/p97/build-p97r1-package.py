...
IDENTITY_REL='artifacts/closure/p97r1/P97R1_TREE_IDENTITY_EXCLUDING_SELF.json'
...
def clean_unpack(zp,rows):
 with tempfile.TemporaryDirectory(prefix='p97-clean-') as td:
  t=Path(td)
...
def main():
 ...
 current_binaries=[r['path'] for r in rows if r['path'].startswith(('receipts/p97/','artifacts/p97/','scripts/p97/','artifacts/closure/p97r1/')) and Path(r['path']).suffix.lower() in {'.pdf','.zip','.woff','.woff2','.ttf','.otf','.exe','.dll','.bin','.pyc','.pyo'}]
 if current_binaries:raise RuntimeError(f'unexpected_current_binary:{current_binaries}')
 with tempfile.TemporaryDirectory(prefix='p97-package-') as td:
