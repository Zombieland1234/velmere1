...
const root=process.cwd(); const baseline=JSON.parse(fs.readFileSync(path.join(root,"config/pass27-visual-freeze-baseline.json"),"utf8"));
...
console.log(JSON.stringify({schemaVersion:"velmere.pass27.visual-freeze-verification.v1",status:changed.length?"FAIL":"STATIC-PROVEN",fileCount:rows.length,aggregateSha256,changed,truthBoundary:"Hash parity for UI source/static assets. Browser screenshot comparison was NOT EXECUTED."},null,2)); if(changed.length) process.exit(1);
