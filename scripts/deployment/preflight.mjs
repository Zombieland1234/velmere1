...
if (vercel.buildCommand !== "corepack npm run build:deployment") blockers.push("vercel_build_command_mismatch");
if (vercel.installCommand !== "corepack npm ci --engine-strict=true --strict-allow-scripts=true --ignore-scripts=false --no-audit --no-fund --progress=false") blockers.push("vercel_install_not_fail_closed");
...
const receipt = {
  ...
  blockers,
  truthBoundary: "Source-controlled Corepack npm selection and bounded hosting runtime wiring only. Exact Windows runtime authority remains a separate gate.",
};
