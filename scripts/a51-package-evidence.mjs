...
const source = path.join(root, "artifacts/pass35/a51");
const out = path.join(root, "artifacts/pass35/PASS35_A51_BACKUP_RESTORE_ROLLBACK_PROVIDER_LOSS_ACCEPTANCE_EVIDENCE.zip");
...
fs.writeFileSync(path.join(source, "PASS35_A51_EVIDENCE_MANIFEST.json"), `${JSON.stringify({ schemaVersion: "velmere.pass35.a51.evidence-manifest.v1", revisionId: "VELMERE_PASS35_A51_BACKUP_RESTORE_ROLLBACK_PROVIDER_LOSS_ACCEPTANCE", generatedAt: new Date().toISOString(), files: manifest }, null, 2)}\n`);
