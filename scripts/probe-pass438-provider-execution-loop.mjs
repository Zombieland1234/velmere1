...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "Velmere-PASS438-Provider-Execution-Loop/1.0" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
...
function loop(rows) {
  ...
  const critical = allMissing.filter((field) => ["price", "secondProvider", "candles"].includes(field));
  const mode = critical.includes("price") || critical.includes("secondProvider") ? "dry_run_facts_only" : rows.some((row) => row.status === "error") ? "execute_with_retry" : "execute_ready";
  const next = allMissing.includes("price") ? "execute CoinGecko/Yahoo price fallback" : allMissing.includes("secondProvider") ? "execute second independent provider" : allMissing.includes("liquidity") ? "execute DEX liquidity lane" : "record provider health";
  return { mode, next, allMissing, providerCount };
}
console.log("PASS438 provider execution loop · execute/read/trace provider lanes before PDF/chat\n");
for (const query of targets) {
  const rows = await Promise.all([probeCoinGecko, probeDexScreener, probeBinance, probeYahoo].map((fn) => settle(fn, query)));
  const l = loop(rows);
  console.log(`--- ${query} ---`);
  console.log(`execution: ${l.mode} · next: ${l.next} · provider lanes: ${l.providerCount}`);
  for (const row of rows) console.log(`${row.provider}: ${row.status} · ${price(row.price)} · observed ${fields(row).join("/") || "none"} · missing ${missing(row, l.providerCount).join("/") || "none"}${row.error ? ` · error ${row.error}` : ""}`);
  console.log(`provider-execution-ledger: ${l.allMissing.join(" · ") || "clean"}\n`);
}
console.log("PASS438 invariant: record provider attempts, classify errors, retry only idempotent reads, keep one PDF/chat payload, never fake live data.");
