...
def clean_unpack(zp,rows):
 with tempfile.TemporaryDirectory(prefix='p89-clean-') as td:
  t=Path(td)
...
def main():
 ...
 current_binary=[x['path'] for x in rows if (x['path'].startswith('receipts/p89/') or x['path'].startswith('artifacts/p89/') or x['path'].startswith('scripts/p89/')) and Path(x['path']).suffix.lower() in {'.pdf','.zip','.woff','.woff2','.ttf','.otf','.exe','.dll','.bin','.pyc'}]
 if current_binary:raise RuntimeError(f'unexpected_current_binary:{current_binary}')
 with tempfile.TemporaryDirectory(prefix='p89-package-') as td:
