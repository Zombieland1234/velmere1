...
const segmentId = String(process.env.A28_SEGMENT_ID ?? "").trim();
const LOG_PATH = segmentId ? `artifacts/pass35/PASS35_A28_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}.log` : "artifacts/pass35/PASS35_A28_NONDESTRUCTIVE_REGRESSION.log";
const RECEIPT_PATH = segmentId ? `artifacts/pass35/PASS35_A28_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}_RECEIPT.json` : "artifacts/pass35/PASS35_A28_NONDESTRUCTIVE_REGRESSION_RECEIPT.json";
...
const startIndex = Number.parseInt(process.env.A28_START_INDEX ?? "0", 10);
const endIndex = Number.parseInt(process.env.A28_END_INDEX ?? String(cases.length), 10);
if (!Number.isSafeInteger(startIndex) || !Number.isSafeInteger(endIndex) || startIndex < 0 || endIndex > cases.length || startIndex >= endIndex) throw new Error(`a28_segment_range_invalid:${startIndex}:${endIndex}:${cases.length}`);
...
for (const testCase of selectedCases) {
  ...
  console.log(`[A28 regression] ${testCase.id}`);
  const caseOutputPath = `artifacts/pass35/.a28-regression-${testCase.id}.tmp.log`;
  ...
  const result = spawnSync("timeout", ["--signal=TERM", "--kill-after=5s", "90s", testCase.command, ...testCase.args], {
    cwd: process.cwd(),
    env: { ...process.env, FORCE_COLOR: "0", A28_REGRESSION_BOOTSTRAP: "1" },
...
const receiptCore = {
  schemaVersion: "velmere.pass35.a28-nondestructive-regression-receipt.v1",
  startedAt,
  ...
  truthBoundary: "This runner executes non-mutating current-source runtime, security, market, PDF and A4-A28 regression tests. It does not grant exact Node/npm, staging, LIVE, customer, legal, provider-rights or independent-assurance credit.",
};
