...
const segmentId = String(process.env.A32_SEGMENT_ID ?? "").trim();
const LOG_PATH = segmentId ? `artifacts/pass35/PASS35_A32_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}.log` : "artifacts/pass35/PASS35_A32_NONDESTRUCTIVE_REGRESSION.log";
const RECEIPT_PATH = segmentId ? `artifacts/pass35/PASS35_A32_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}_RECEIPT.json` : "artifacts/pass35/PASS35_A32_NONDESTRUCTIVE_REGRESSION_RECEIPT.json";
...
const cases = [
  ...
  node("a27_fork_replay_evidence", "scripts/pass35/test-a27-fork-replay-evidence.mjs"),
  node("a27_control_plane", "scripts/pass35/test-a27-control-plane.mjs"),
  node("a28_economic_adversarial_evidence", "scripts/pass35/test-a28-economic-adversarial-evidence.mjs"),
  node("a28_control_plane", "scripts/pass35/test-a28-control-plane.mjs"),
  node("a29_upgrade_deployment_operations", "scripts/pass35/test-a29-upgrade-deployment-operations.mjs"),
  node("a29_control_plane", "scripts/pass35/test-a29-control-plane.mjs"),
  node("a30_threat_model", "scripts/pass35/test-a30-threat-model.mjs"),
  node("a30_control_plane", "scripts/pass35/test-a30-control-plane.mjs"),
  node("a31_privilege_control", "scripts/pass35/test-a31-privilege-control.mjs"),
  node("a31_control_plane", "scripts/pass35/test-a31-control-plane.mjs"),
  node("a32_report_delivery", "scripts/pass35/test-a32-report-delivery.mjs"),
  node("a32_control_plane", "scripts/pass35/test-a32-control-plane.mjs"),
...
const startIndex = Number.parseInt(process.env.A32_START_INDEX ?? "0", 10);
const endIndex = Number.parseInt(process.env.A32_END_INDEX ?? String(cases.length), 10);
if (!Number.isSafeInteger(startIndex) || !Number.isSafeInteger(endIndex) || startIndex < 0 || endIndex > cases.length || startIndex >= endIndex) throw new Error(`a32_segment_range_invalid:${startIndex}:${endIndex}:${cases.length}`);
...
for (const testCase of selectedCases) {
  ...
  console.log(`[A32 regression] ${testCase.id}`);
  const caseOutputPath = `artifacts/pass35/.a31-regression-${testCase.id}.tmp.log`;
  ...
  const result = spawnSync("timeout", ["--signal=TERM", "--kill-after=5s", "90s", testCase.command, ...testCase.args], {
    cwd: process.cwd(),
    env: { ...process.env, FORCE_COLOR: "0", A32_REGRESSION_BOOTSTRAP: "1" },
...
const receiptCore = {
  schemaVersion: "velmere.pass35.a32-nondestructive-regression-receipt.v1",
  startedAt,
  ...
  truthBoundary: "This runner executes non-mutating current-source runtime, security, market, PDF and A4-A32 regression tests. It does not grant exact Node/npm, staging, LIVE, customer, legal, provider-rights or independent-assurance credit.",
};
