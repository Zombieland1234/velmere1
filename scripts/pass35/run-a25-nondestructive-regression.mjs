...
const segmentId = String(process.env.A25_SEGMENT_ID ?? "").trim();
const LOG_PATH = segmentId ? `artifacts/pass35/PASS35_A25_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}.log` : "artifacts/pass35/PASS35_A25_NONDESTRUCTIVE_REGRESSION.log";
const RECEIPT_PATH = segmentId ? `artifacts/pass35/PASS35_A25_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}_RECEIPT.json` : "artifacts/pass35/PASS35_A25_NONDESTRUCTIVE_REGRESSION_RECEIPT.json";
...
const startIndex = Number.parseInt(process.env.A25_START_INDEX ?? "0", 10);
const endIndex = Number.parseInt(process.env.A25_END_INDEX ?? String(cases.length), 10);
if (!Number.isSafeInteger(startIndex) || !Number.isSafeInteger(endIndex) || startIndex < 0 || endIndex > cases.length || startIndex >= endIndex) throw new Error(`a25_segment_range_invalid:${startIndex}:${endIndex}:${cases.length}`);
...
for (const testCase of selectedCases) {
  ...
  console.log(`[A25 regression] ${testCase.id}`);
  const caseOutputPath = `artifacts/pass35/.a25-regression-${testCase.id}.tmp.log`;
...
const receiptCore = {
  schemaVersion: "velmere.pass35.a25-nondestructive-regression-receipt.v1",
  startedAt,
  ...
  truthBoundary: "This runner executes non-mutating current-source runtime, security, market, PDF and A4-A25 regression tests. It does not grant exact Node/npm, staging, LIVE, customer, legal, provider-rights or independent-assurance credit.",
};
