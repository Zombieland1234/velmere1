...
const segmentId = String(process.env.A30_SEGMENT_ID ?? "").trim();
const LOG_PATH = segmentId ? `artifacts/pass35/PASS35_A30_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}.log` : "artifacts/pass35/PASS35_A30_NONDESTRUCTIVE_REGRESSION.log";
const RECEIPT_PATH = segmentId ? `artifacts/pass35/PASS35_A30_NONDESTRUCTIVE_REGRESSION_SEGMENT_${segmentId}_RECEIPT.json` : "artifacts/pass35/PASS35_A30_NONDESTRUCTIVE_REGRESSION_RECEIPT.json";
...
const startIndex = Number.parseInt(process.env.A30_START_INDEX ?? "0", 10);
const endIndex = Number.parseInt(process.env.A30_END_INDEX ?? String(cases.length), 10);
...
for (const testCase of selectedCases) {
  ...
  console.log(`[A30 regression] ${testCase.id}`);
  const caseOutputPath = `artifacts/pass35/.a30-regression-${testCase.id}.tmp.log`;
  ...
  const result = spawnSync("timeout", ["--signal=TERM", "--kill-after=5s", "90s", testCase.command, ...testCase.args], {
    cwd: process.cwd(),
    env: { ...process.env, FORCE_COLOR: "0", A30_REGRESSION_BOOTSTRAP: "1" },
...
const receiptCore = {
  schemaVersion: "velmere.pass35.a30-nondestructive-regression-receipt.v1",
  startedAt,
  ...
  truthBoundary: "This runner executes non-mutating current-source runtime, security, market, PDF and A4-A30 regression tests. It does not grant exact Node/npm, staging, LIVE, customer, legal, provider-rights or independent-assurance credit.",
};
