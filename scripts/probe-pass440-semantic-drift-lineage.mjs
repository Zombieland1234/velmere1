...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "Velmere-PASS440-Semantic-Drift-Lineage/1.0" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
function observed(row) { return [row.price !== null ? "price" : null, row.change24h !== null ? "change24h" : null, row.volume24h ? "volume24h" : null, row.marketCap ? "marketCap" : null, row.liquidity ? "liquidity" : null, row.candles ? "candles" : null, row.security ? "security" : null].filter(Boolean); }
...
function displayPrice(value) { const n = num(value); return n === null ? "missing" : n >= 1 ? `$${n.toLocaleString("en-US", { maximumFractionDigits: 2 })}` : `$${n.toLocaleString("en-US", { maximumSignificantDigits: 5 })}`; }
...
function lineage(rows) {
  ...
  const unsupported = ["price", "change24h", "volume24h"].filter((field) => allMissing.includes(field)).length;
  const high = allMissing.includes("price") || providerCount === 0;
  const drift = Math.max(0, Math.min(100, 18 + allMissing.length * 6 + unsupported * 12 + (providerCount < 2 ? 18 : 0) + (high ? 22 : 0)));
  const state = high ? "semantic_sealed" : drift >= 68 ? "semantic_drift_warning" : drift >= 38 ? "semantic_guarded" : "semantic_aligned";
  const mode = state === "semantic_aligned" ? "normal_source_bound" : state === "semantic_guarded" ? "guarded_rewrite" : state === "semantic_drift_warning" ? "facts_only_rewrite" : "operator_lineage_review";
  return { providerCount, allMissing, drift, state, mode };
}
console.log("PASS440 semantic drift lineage · bind every customer sentence to source evidence before PDF/chat\n");
for (const query of targets) {
  const rows = await Promise.all([probeCoinGecko, probeDexScreener, probeBinance, probeYahoo].map((fn) => settle(fn, query)));
  const l = lineage(rows);
  console.log(`--- ${query} ---`);
  console.log(`semantic-lineage: ${l.state} · response ${l.mode} · drift ${l.drift}/100 · provider lanes ${l.providerCount}`);
  for (const row of rows) console.log(`${row.provider}: ${row.status} · ${displayPrice(row.price)} · observed ${observed(row).join("/") || "none"} · missing ${missing(row, l.providerCount).join("/") || "none"}${row.error ? ` · error ${row.error}` : ""}`);
  console.log(`lineage repair: ${l.allMissing.length ? l.allMissing.join(" · ") : "none"}\n`);
}
console.log("PASS440 invariant: memory cannot invent a source, tone cannot upgrade confidence, and unsupported live wording is rewritten or sealed.");
