...
const supplementalManifest=JSON.parse(fs.readFileSync(path.join(root,"config/pass23/i18n-supplemental-value-map.json"),"utf8"));
...
for(const field of ["sourceSubjectSha","sourceReceiptMergeSha","reviewStatus","expectedKeyCount","expectedSourceValueCount","sortedKeySha256","sourceRowsSha256"]){if(supplementalManifest[field]!==supplementalTranslationMeta[field])errors.push(`supplemental_manifest_meta_mismatch:${field}`);}
if(!Array.isArray(supplementalManifest.keyModules)||supplementalManifest.keyModules.length!==2)errors.push("supplemental_manifest_key_modules");
if(!Array.isArray(supplementalManifest.valueModules)||supplementalManifest.valueModules.length!==4)errors.push("supplemental_manifest_value_modules");
...
if(supplementalOverflow.caseKeysSource!=="config/pass23/i18n-supplemental-value-map.json#keyModules")errors.push("supplemental_overflow_key_source");
...
const report={schemaVersion:"velmere.pass23.native-review-overflow-preparation.v2",generatedAt:new Date().toISOString(),ok:errors.length===0,primaryTranslations:primaryCount,supplementalTranslations:supplementalCount,totalTranslations,identicalNonNeutral:i18n.summary.identicalNonNeutral,nativeReviewStatus:{primary:checklist.status,supplemental:supplementalChecklist.status},overflowPlanned:totalOverflowPlanned,overflowExecuted:totalOverflowExecuted,errors,truthBoundary:"Static draft completion across 659 values and 659 planned browser cases is not native-language, legal, brand-voice or browser proof. Both review waves remain PENDING and browser execution remains zero."};
