...
const knownFixtureSecretCandidates = new Set(["scripts/pass36/test-a63-staging-program-orchestrator.mjs\0stripe_live_secret"]);
  ...
  if (text) {
    ...
    for (const [kind, pattern] of secretPatterns) {
      ...
      if (pattern.test(source)) {
        ...
        if (knownFixtureSecretCandidates.has(`${relative}\0${kind}`)) fixtureSecretCandidates.push({ ...row, reason: "explicit hostile credential fixture literal" });
        ...
        if (specifier.startsWith("./") || specifier.startsWith("../") || specifier.startsWith("@/")) {
          if (!resolveLocal(absolute, specifier)) {
            if (
              ...
              (relative === "next-env.d.ts" || relative === "scripts/deployment/test-next-env-build-contract.mjs")
            ) {
              optionalGeneratedImports.push({
                path: relative,
                specifier,
                reason: relative === "next-env.d.ts" ? "next-build-generated" : "next-build-generated-fixture-literal",
              });
            } else if (
              relative === "scripts/pass35/test-a42-dev-runtime-cache-recovery.mjs"
              && /^\.\/messages\/(?:pl|en|de)\.json$/u.test(specifier)
            ) {
          ...
          if (relative === "lib/security/pass35-a20-case-bound-dependency-runtime.mjs" && specifier === "$1") continue;
