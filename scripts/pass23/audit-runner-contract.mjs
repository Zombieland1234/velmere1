...
const policy=JSON.parse(fs.readFileSync(path.join(root,"config/pass23/gate-policy.json"),"utf8"));
...
const expectedRunnerGate=scripts["gate:quick"]?.match(/scripts\/(pass\d+)\/run-gate\.mjs/u)?.[0]??null;
if(!expectedRunnerGate||!runner.includes(expectedRunnerGate))issues.push(`runner_not_bound_to_current_gate:${expectedRunnerGate??"missing_package_gate"}`);
...
const result={schemaVersion:"velmere.pass25.runner-contract.v1",generatedAt:"2026-07-20T18:00:00.000Z",ok:issues.length===0,runner:{path:"VELMERE_RUN_FULL_CHECK.ps1",lineCount,heavyExecutionGuard:runner.includes("AllowHeavy")},packageScriptCount:Object.keys(scripts).length,policyCommandCount:commands.length,issues};
