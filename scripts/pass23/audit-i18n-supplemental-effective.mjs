...
const supplementalManifest = JSON.parse(supplementalRaw);
...
for (const field of ["sourceSubjectSha","sourceReceiptMergeSha","reviewStatus","expectedKeyCount","expectedSourceValueCount","sortedKeySha256","sourceRowsSha256"]) {
  if (supplementalManifest[field] !== supplementalTranslationMeta[field]) failures.push(`supplemental_manifest_meta_mismatch:${field}`);
}
if (!Array.isArray(supplementalManifest.keyModules) || supplementalManifest.keyModules.length !== 2) failures.push("supplemental_manifest_key_modules");
if (!Array.isArray(supplementalManifest.valueModules) || supplementalManifest.valueModules.length !== 4) failures.push("supplemental_manifest_value_modules");
...
for (const key of keys) {
  ...
  if (!source || !mapped || typeof mapped.pl !== "string" || typeof mapped.de !== "string") {
    failures.push(`supplemental_source_unmapped:${key}`);
    continue;
  }
...
if (failures.length === 0) {
  ...
  receipt.summary = {
    ...receipt.summary,
    primaryDraftTranslationCount: 300,
    supplementalDraftTranslationCount: 359,
    supplementalSourceValueCount: 211,
    supplementalSourceSubjectSha: supplemental.sourceSubjectSha,
    draftTranslationCount: 659,
    draftTranslationReviewStatus: "PENDING_NATIVE_REVIEW",
    preSupplementalIdenticalNonNeutral: 359,
    identicalNonNeutral: 0,
  };
  for (const locale of ["pl", "de"]) {
    if (receipt.effectiveCatalogSources?.[locale]) {
      receipt.effectiveCatalogSources[locale].supplementalTranslationPath = "config/pass23/i18n-supplemental-value-map.json";
      receipt.effectiveCatalogSources[locale].supplementalTranslationStorage = "2 key modules + 4 source-value modules";
...
const report = {
  schemaVersion: "velmere.pass23.i18n-supplemental-effective-verification.v1",
  sourceSubjectSha: supplemental.sourceSubjectSha ?? null,
  primaryRedDenominator: rows.length,
  supplementalKeyDenominator: keys.length,
  supplementalSourceValueDenominator: Object.keys(valueMap).length,
  postSupplementalIdenticalNonNeutral: failures.length === 0 ? 0 : null,
  reviewStatus: supplemental.reviewStatus ?? null,
  failures,
  ok: failures.length === 0,
  truthBoundary: "PASS proves exact set equality between the preserved 359-row red receipt and the source-bound supplemental draft map, plus non-empty PL/DE replacements that break each identical triple. It does not prove native-language quality, legal approval, brand voice, browser rendering or commercial readiness."
};
