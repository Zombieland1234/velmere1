...
const policy = JSON.parse(fs.readFileSync(path.join(root, "config/pass20/gate-policy.json"), "utf8"));
...
if(!runner.includes("scripts/pass20/run-gate.mjs"))issues.push("runner_not_bound_to_pass20_gate");
...
const result={schemaVersion:"velmere.pass20.runner-contract.v1",generatedAt:"2026-07-20T12:00:00.000Z",ok:issues.length===0,runner:{path:"VELMERE_RUN_FULL_CHECK.ps1",lineCount,heavyExecutionGuard:runner.includes("AllowHeavy")},packageScriptCount:Object.keys(scripts).length,policyCommandCount:commands.length,issues};
