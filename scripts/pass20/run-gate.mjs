...
const policyPath = path.join(root, "config", "pass20", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS20 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass20-gates");
...
console.log(`PASS20 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass20.gate-execution.v1",
...
console.log(`PASS20 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS20 gate receipt: ${path.relative(root, receiptPath)}`);
