...
const fail = (message, extra = {}) => {
  console.log(JSON.stringify({ status: "FAIL_R44P38_SOURCE_AUTHORITY", message, ...extra }, null, 2));
  process.exit(1);
};
...
if (manifest.pathOrderAlgorithm !== "UTF8_BYTEWISE_ASCENDING_V1") fail("PATH_ORDER_ALGORITHM_MISMATCH", { actual: manifest.pathOrderAlgorithm });
...
for (let index = 0; index < actual.length; index += 1) {
  if (actual[index].path !== expected[index].path || actual[index].byteLength !== expected[index].byteLength || actual[index].sha256 !== expected[index].sha256) {
    fail("FILE_BINDING_MISMATCH", { index, actual: actual[index], expected: expected[index] });
  }
}
...
if (manifest.fileCount !== actual.length || manifest.payloadBytes !== payloadBytes) fail("MANIFEST_TOTAL_MISMATCH", { fileCount: actual.length, payloadBytes });
console.log(JSON.stringify({ status: "PASS_R44P38_SOURCE_AUTHORITY", revisionId: REV, fileCount: actual.length, payloadBytes, sourceAggregateSha256, pathSetSha256, sourceImmutable: true }, null, 2));
