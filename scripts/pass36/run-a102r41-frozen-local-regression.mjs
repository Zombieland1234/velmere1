...
import { REV } from "./a102r41-source-boundary.mjs";
...
const a60TypeScriptPolicy = JSON.parse(fs.readFileSync(path.join(process.cwd(), "config/pass36/a60-exact-final-byte-build-browser-acceptance.json"), "utf8")).typescript;

function argumentsExact(argv) {
  invariant(argv.length === 2 && argv[0] === "--receipt-dir" && typeof argv[1] === "string" && argv[1].length > 0, "a102r41_regression_arguments");
...
function strictJson(text, label) {
  ...
  catch (error) { throw new Error(`a102r41_regression_strict_json:${label}:${error instanceof Error ? error.message : String(error)}`, { cause: error }); }
}
...
function readArtifact(filePath, id) {
  const observed = readDescriptorBoundRegularFile(filePath, { maxBytes: 128 * 1024 * 1024, errorPrefix: `a102r41_regression_artifact_${safe(id)}` });
...
function validateStage(id, value, artifact, source) {
  switch (id) {
    case "current-authority": return value.schemaVersion === "velmere.pass36.a102r41.action-required-authority-verification.v1" && value.status === "PASS_A102R41_ACTION_REQUIRED_AUTHORITY_SECURITY_EVIDENCE_EXACT_WINDOWS_PREFLIGHT_AND_FAIL_CLOSED_RELEASE_PACKAGING_NO_LIVE_CREDIT" && value.revisionId === REV && value.checks === 46 && value.passed === 46 && value.failed === 0 && noGo(value);
    case "descendant": return value.schemaVersion === "velmere.pass36.a102r41.current-root-descendant-verification.v1" && value.status === "PASS_A102R41_CURRENT_ROOT_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION" && value.revisionId === REV && value.checks === 20 && value.passed === 20 && value.failed === 0 && value.historicalChainChecks === 415 && value.manifestDigestSha256 === source.descendantManifestDigestSha256 && value.payload?.aggregateSha256 === source.aggregateSha256 && value.globalDecision === "NO_GO" && value.live === false && value.saleEnabled === false;
    case "approved-changes": return value.schemaVersion === "velmere.pass36.a102r41.approved-current-source-changes-verification.v1" && value.status === "PASS_A102R41_APPROVED_CURRENT_SOURCE_CHANGES_COMPLETE_NO_PROMOTION" && value.revisionId === REV && Number.isInteger(value.fileCount) && value.fileCount > 0 && value.checks === 11 + (4 * value.fileCount) && value.passed === value.checks && value.failed === 0 && value.deletedFiles === 0 && value.addedFiles + value.modifiedFiles === value.fileCount && value.globalDecision === "NO_GO" && value.live === false && value.saleEnabled === false;
    case "authority-harness": return value.schemaVersion === "velmere.pass36.a102r41.current-source-authority-preflight-test.v1" && value.status === "PASS_A102R41_CURRENT_SOURCE_AUTHORITY_PREFLIGHT_LOCAL_ONLY_NO_EXACT_FINAL_BYTE_BUILD_BROWSER_CREDIT" && value.checks === 46 && value.passed === 46 && value.failed === 0 && value.retainedChecks === 37 && value.addedChecks === 9 && value.removedChecks === 0 && value.releaseSourceBindingSurfaces === 6 && noGo(value);
    case "historical-chain": return value.schemaVersion === "velmere.pass36.a80.current-root-descendant-verification.v3" && value.status === "PASS_A80_HISTORICAL_DESCENDANT_CHAIN_TO_CURRENT" && value.checks === 426 && value.passed === 426 && value.failed === 0 && value.currentChildPayload?.aggregateSha256 === source.aggregateSha256 && value.liveProven === false && value.saleEnabled === false;
    case "a42-critical": return value.schemaVersion === "velmere.pass36.a102r41.a42-critical-rebaseline-verification.v1" && value.revisionId === REV && value.status === "PASS_A102R41_A42_CRITICAL_REBASELINE_76_OF_76_NO_WINDOWS_BROWSER_STAGING_OR_SALE_CREDIT" && value.checks === 38 && value.passed === 38 && value.failed === 0 && value.criticalFilesPassed === 76 && value.rebaselinedRows === 5 && value.rowDenominatorMigration?.old === 4 && value.rowDenominatorMigration?.new === 5 && value.rowDenominatorMigration?.removed === 0 && value.verifierDenominatorMigration?.old === 33 && value.verifierDenominatorMigration?.new === 38 && value.verifierDenominatorMigration?.removedAssertions === 0 && noGo(value);
    ...
    case "account-operation": return value.schemaVersion === "velmere.pass36.a102r41.account-operation-privacy-containment-test.v1" && value.revisionId === REV && value.sensitiveOperationDenominator === 8 && value.total === 55 && value.passed === 55 && value.failed === 0 && value.durableServerOwnedWorkflowsImplemented === 0 && value.authorityCredit === false && value.live === false && value.saleEnabled === false;
    case "checkout": return value.schemaVersion === "velmere.pass36.a102r41.paid-checkout-containment-and-browser-storage-test.v1" && value.revisionId === REV && value.total === 15 && value.passed === 15 && value.failed === 0 && value.durableOpaqueCheckoutFlowImplemented === false && value.realStripeTestLifecycles === 0 && value.live === false && value.saleEnabled === false;
    ...
    case "package-boundary": return value.schemaVersion === "velmere.pass36.a102r41.package-portability-and-parser-test.v1" && value.status === "PASS_A102R41_PACKAGE_PORTABILITY_AND_STRICT_MANIFEST_BOUNDARY_NO_PROMOTION" && value.required === 36 && value.executed === 36 && value.passed === 36 && value.failed === 0 && value.denominatorMigration?.old === 17 && value.denominatorMigration?.retained === 17 && value.denominatorMigration?.added === 19 && value.denominatorMigration?.removed === 0 && value.denominatorMigration?.verifierChecks === 20 && noGo(value);
    ...
    case "a80r1": return value.schemaVersion === "velmere.pass36.a102r41.a80r1-two-phase-release-controller-test.v1" && value.status === "PASS_A102R41_A80R1_TWO_PHASE_CONTROLLER_MECHANISM_NO_PROMOTION" && value.required === 36 && value.executed === 36 && value.passed === 36 && value.failed === 0 && value.denominatorMigration?.old === 35 && value.denominatorMigration?.new === 36 && value.denominatorMigration?.removedAssertions === 0 && noGo(value);
...
invariant(process.version === "v24.18.0", `a102r41_regression_exact_node:${process.version}`);
invariant(!fs.existsSync(receiptDir), "a102r41_regression_receipt_dir_must_be_new");
...
const stages = [
  { id: "current-authority", command: [node, "scripts/pass36/verify-a102r41-action-required-authority.mjs"] },
  { id: "descendant", command: [node, "scripts/pass36/verify-a102r41-current-root-descendant.mjs"] },
  { id: "approved-changes", command: [node, "scripts/pass36/verify-a102r41-approved-current-source-changes.mjs"] },
  { id: "authority-harness", command: [node, "scripts/pass36/test-a102r41-current-source-authority-preflight.mjs"] },
  { id: "historical-chain", command: [node, "scripts/pass36/verify-a80-current-root-descendant.mjs"] },
  { id: "a42-critical", command: [node, "scripts/pass36/verify-a102r41-a42-critical-rebaseline.mjs"] },
  ...
  { id: "package-boundary", command: [node, "scripts/pass36/test-a102r41-package-portability-and-parser.mjs"] },
  { id: "a80", command: [node, "scripts/pass36/test-a80-frozen-local-release-candidate-admission.mjs"] },
  { id: "a80r1", command: [node, "scripts/pass36/test-a102r41-a80r1-two-phase-release-controller.mjs"] },
...
for (const [index, stage] of stages.entries()) {
  ...
  const artifactBefore = artifactFile && fs.existsSync(artifactFile)
    ? {
        binding: readDescriptorBoundRegularFile(artifactFile, { maxBytes: 128 * 1024 * 1024, errorPrefix: "a102r41_regression_prior_artifact" }).binding,
...
const receipt = {
  ...
  revisionId: REV,
  status: passed ? "PASS_A102R41_FROZEN_SOURCE_FULL_LOCAL_REGRESSION_NO_PROMOTION" : "FAIL_A102R41_FROZEN_SOURCE_LOCAL_REGRESSION",
...
fs.writeFileSync(path.join(receiptDir, "A102R41_FULL_REGRESSION_RECEIPT.json"), `${JSON.stringify(receipt, null, 2)}\n`, { encoding: "utf8", flag: "wx" });
for (const [stageId, fileName] of [
  ["current-authority", "A102R41_CURRENT_AUTHORITY_RECEIPT.json"],
  ["descendant", "A102R41_DESCENDANT_RECEIPT.json"],
  ["approved-changes", "A102R41_APPROVED_CHANGES_RECEIPT.json"],
]) {
  const stage = results.find((row) => row.id === stageId);
  if (!stage?.passed) continue;
  const wrapper = {
    schemaVersion: "velmere.pass36.a102r41.physical-command-receipt-wrapper.v1", revisionId: REV,
