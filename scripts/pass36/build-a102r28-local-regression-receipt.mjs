...
const output=path.join(root,'config/pass36/a102r28-local-regression-receipt.json');
const logDir=path.join(root,'.velmere/artifacts/a102r28-local-regression');
...
const definitions=[
 ['a102r28',['node',...loader,'scripts/pass36/test-a102r28-chart-abort-reopen-and-cache-reset-race-boundary.mjs']],
...
const auditText=fs.readFileSync(path.join(logDir,`${String(definitions.length).padStart(2,'0')}-source-audit.stdout.txt`),'utf8');
...
const receipt={
 schemaVersion:'velmere.pass36.a102r28.local-regression-receipt.v1',revisionId:REV,parentRevisionId:PARENT,generatedAt:'2026-07-31T01:34:00.000Z',status:'PASS_A102R28_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION',checkpointClass:'ACTION_REQUIRED_NON_PASS',requiredStages:definitions.length,executedStages:definitions.length,passedStages:definitions.length,failedStages:0,stages,
 keyDenominators:{a102r28BoundaryChecks:14,a102r27ParentBoundaryChecks:20,a102r26ParentBoundaryChecks:33,devBootstrapCriticalHashes:71,rapidReopenStartsFreshRequest:true,abortedResponseCacheWrites:0,deferredIdentityReset:false,orphanInflightEntriesAfterSettlement:0,routeDispatchChecks:1480,routeRoutes:160,routeDispatchTamperChecks:7,lazyRouteChecks:176,a57Checks:1138,protectedIntegrityRows:1073,apiBodyChecks:37,malformedJsonChecks:112,mega4800Checks:61,a59Checks:77,productTierChecks:186,zeroBudgetChecks:439},
 sourceAudit,
 localClosure:{rapidReopenStartsFreshRequest:true,abortedResponseCacheWrites:0,deferredIdentityReset:false,orphanInflightEntriesAfterSettlement:0,parentProviderIdentityBoundaryRetained:true,productionSyntheticChartCredit:false},
 ...
 realEvidence:{providerRights:0,browserRows:0,realObservationRuns:0,stagingStages:0,legalDecisions:0,customerCohorts:0},promotion:{globalDecision:'NO_GO',live:false,saleEnabled:false,productionApproved:false,worldClassProven:false},truthBoundary:'Local Asset Detail abort/reopen, aborted-response cache-write and deferred identity-reset race recovery only. No exact build/browser, staging, rights, legal, customer, LIVE or sale credit.'
};
