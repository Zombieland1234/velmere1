...
const MANIFEST = "_velmere/PASS36_A102R44P34_SOURCE_ONLY_MANIFEST.json";
...
const STATE = "config/pass36/a102r44p34-action-required-current-state.json";
const TOPOLOGY = "config/pass36/a102r44p34-canonical-product-topology.json";
const POINTER = "config/pass36/current-release-authority.json";

const sha256 = (bytes) => crypto.createHash("sha256").update(bytes).digest("hex");
const forbiddenTopLevel = new Set([
  ".cache", ".git", ".turbo", ".velmere", "__pycache__", "artifacts", "build",
  "cache", "coverage", "dist", "node_modules", "out", "playwright-report",
  "temp", "test-results", "tmp",
]);

function rejected(rel) {
  const parts = rel.split("/");
  const top = parts[0] ?? "";
  return forbiddenTopLevel.has(top)
    || top.startsWith(".next")
    || top === ".env"
    || top.startsWith(".env.")
...
function collectSource() {
  ...
  function walk(directory, prefix = "") {
    ...
    for (const entry of entries) {
      ...
      if (stat.isSymbolicLink()) throw new Error(`SYMLINK_FORBIDDEN:${rel}`);
      if (entry.isDirectory()) {
        if (!rejected(rel)) walk(full, rel);
        continue;
      }
      if (!entry.isFile() || rejected(rel)) continue;
...
const topology = JSON.parse(fs.readFileSync(path.join(ROOT, TOPOLOGY), "utf8"));
const pointer = JSON.parse(fs.readFileSync(path.join(ROOT, POINTER), "utf8"));

const checks = [];
const add = (id, passed, detail = null) => checks.push({ id, passed: Boolean(passed), detail });
add("schema", manifest.schemaVersion === "velmere.pass36.a102r44p34.source-manifest.v1");
...
add("entries", manifest.entries.length === rows.length && manifest.entries.every((row, index) => {
  const actual = rows[index];
  return row.path === actual.path
    && row.byteLength === actual.byteLength
    && row.sha256 === actual.sha256
    && row.mode === actual.mode;
}));
...
add("topology-tier-boundary", JSON.stringify(topology.tieredFamilies) === JSON.stringify(["audit", "pdf", "browser"]) && topology.products.filter((item) => item.productClass === "STANDALONE_PRODUCT").every((item) => item.tier === null));
add("psychology", state.psychologyMatrix.personas === 30 && state.psychologyMatrix.stepsPerPersona === 18 && state.psychologyMatrix.rows === 540 && state.psychologyMatrix.realParticipants === 0);
add("test-cycle", state.testCycle.current === 1 && state.testCycle.total === 3 && state.testCycle.fullRegressionCurrentPassExecuted === false);
...
add("dynamic-score-contract", state.dynamicScoring.scoreRows === 17 && state.dynamicScoring.copyPreviousPercentagesForbidden === true && state.dynamicScoring.newGateMayLowerScore === true);
...
add("patch", fs.readFileSync(path.join(ROOT, "VELMERE_A102R44P34_PATCH.txt"), "utf8").startsWith("VELMÈRE PASS36 A102R44P34"));

const failures = checks.filter((check) => !check.passed);
console.log(JSON.stringify({
  schemaVersion: "velmere.pass36.a102r44p34.source-authority-verification.v1",
  status: failures.length ? "FAIL" : "PASS_R44P34_SOURCE_AUTHORITY",
  revisionId: REVISION,
