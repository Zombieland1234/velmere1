#!/usr/bin/env node
import { ESLint } from "eslint";
import {
  discoverLintFiles,
  partitionFiles,
  generatedIgnoreMatch,
  ESLINT_FILE_CHUNK_SIZE,
  GENERATED_IGNORE_ALLOWLIST,
} from "../pass13/eslint-partition-plan.mjs";
import { resolveInstalledEslintCli } from "../pass13/eslint-cli-runtime.mjs";
import { treeDigest, writeJson, PASS13_DIR, now, rel } from "../pass13/common.mjs";
import path from "node:path";
import process from "node:process";

const root = process.cwd();
const args = process.argv.slice(2);
function valueOf(name) {
  const prefix = `${name}=`;
  const match = args.find((value) => value.startsWith(prefix));
  return match ? match.slice(prefix.length) : null;
}
function positiveInteger(name, fallback = null) {
  const raw = valueOf(name);
  if (raw === null) return fallback;
  const value = Number(raw);
  if (!Number.isInteger(value) || value < 1) throw new Error(`${name.slice(2)}_invalid`);
  return value;
}

const files = discoverLintFiles(root);
const chunks = partitionFiles(files, ESLINT_FILE_CHUNK_SIZE);
const partition = positiveInteger("--partition");
if (partition === null || partition > chunks.length) throw new Error("persistent_eslint_partition_invalid");
const batchSize = positiveInteger("--batch-size", 25);
if (batchSize > 50) throw new Error("persistent_eslint_batch_size_too_large");
const output = path.resolve(root, valueOf("--output") || path.join(PASS13_DIR, `R44P30_PERSISTENT_ESLINT_PARTITION_${String(partition).padStart(3, "0")}.json`));
const expectedFiles = chunks[partition - 1];
const before = treeDigest({ sourceOnly: true });
const eslintIdentity = resolveInstalledEslintCli(root);
const eslint = new ESLint({ cwd: root });
const invocationRows = [];
const messages = [];
const processFailures = [];
const resultFiles = [];
let lintErrors = 0;
let lintWarnings = 0;
let generatedIgnored = 0;
const started = Date.now();

for (let index = 0; index < expectedFiles.length; index += batchSize) {
  const batch = expectedFiles.slice(index, index + batchSize);
  const batchStarted = Date.now();
  let results = null;
  let failure = null;
  try {
    results = await eslint.lintFiles(batch);
  } catch (error) {
    failure = {
      code: "persistent_eslint_api_failed",
      partition,
      batch: Math.floor(index / batchSize) + 1,
      message: error instanceof Error ? error.message : String(error),
    };
    processFailures.push(failure);
  }
  const batchResultFiles = [];
  let batchErrors = 0;
  let batchWarnings = 0;
  let batchGeneratedIgnored = 0;
  if (results !== null) {
    for (const resultRow of results) {
      const file = rel(resultRow.filePath);
      batchResultFiles.push(file);
      resultFiles.push(file);
      const rowMessages = Array.isArray(resultRow.messages) ? resultRow.messages : [];
      const allowlistedIgnored = rowMessages.length === 1
        && Number(resultRow.errorCount || 0) === 0
        && Number(resultRow.warningCount || 0) === 1
        && rowMessages[0]?.ruleId === null
        && Number(rowMessages[0]?.severity || 0) === 1
        && generatedIgnoreMatch(file, rowMessages[0]?.message);
      if (allowlistedIgnored) {
        batchGeneratedIgnored += 1;
        generatedIgnored += 1;
        continue;
      }
      batchErrors += Number(resultRow.errorCount || 0);
      batchWarnings += Number(resultRow.warningCount || 0);
      lintErrors += Number(resultRow.errorCount || 0);
      lintWarnings += Number(resultRow.warningCount || 0);
      for (const message of rowMessages) messages.push({ file, ...message });
    }
  }
  const expectedSorted = [...batch].sort();
  const resultSorted = [...batchResultFiles].sort();
  const duplicateResultFiles = resultSorted.filter((file, rowIndex) => rowIndex > 0 && file === resultSorted[rowIndex - 1]);
  const resultPathSetMatch = duplicateResultFiles.length === 0
    && expectedSorted.length === resultSorted.length
    && expectedSorted.every((file, rowIndex) => file === resultSorted[rowIndex]);
  if (failure === null && !resultPathSetMatch) {
    failure = {
      code: "persistent_eslint_result_path_set_mismatch",
      partition,
      batch: Math.floor(index / batchSize) + 1,
      expectedFileCount: expectedSorted.length,
      resultFileCount: resultSorted.length,
      missing: expectedSorted.filter((file) => !resultSorted.includes(file)).slice(0, 100),
      extra: resultSorted.filter((file) => !expectedSorted.includes(file)).slice(0, 100),
      duplicateResultFiles,
    };
    processFailures.push(failure);
  }
  const memory = process.memoryUsage();
  invocationRows.push({
    index: Math.floor(index / batchSize) + 1,
    fileCount: batch.length,
    sourceBytes: batch.reduce((sum, file) => sum + Number(path.extname(file) ? 0 : 0), 0),
    resultFileCount: batchResultFiles.length,
    resultPathSetMatch,
    exitCode: failure === null ? (batchErrors > 0 ? 1 : 0) : 2,
    signal: null,
    timedOut: false,
    lintErrors: batchErrors,
    lintWarnings: batchWarnings,
    generatedIgnored: batchGeneratedIgnored,
    processFailure: failure,
    stderrTail: "",
    durationMs: Date.now() - batchStarted,
    rssBytes: memory.rss,
    heapUsedBytes: memory.heapUsed,
  });
  if (failure !== null) break;
}

const expectedSorted = [...expectedFiles].sort();
const resultSorted = [...resultFiles].sort();
const duplicateResultFiles = resultSorted.filter((file, rowIndex) => rowIndex > 0 && file === resultSorted[rowIndex - 1]);
const resultPathSetMatch = duplicateResultFiles.length === 0
  && expectedSorted.length === resultSorted.length
  && expectedSorted.every((file, rowIndex) => file === resultSorted[rowIndex]);
if (!resultPathSetMatch && processFailures.length === 0) {
  processFailures.push({
    code: "persistent_eslint_partition_path_set_mismatch",
    partition,
    expectedFileCount: expectedSorted.length,
    resultFileCount: resultSorted.length,
    missing: expectedSorted.filter((file) => !resultSorted.includes(file)).slice(0, 100),
    extra: resultSorted.filter((file) => !expectedSorted.includes(file)).slice(0, 100),
    duplicateResultFiles,
  });
}
const after = treeDigest({ sourceOnly: true });
const passed = processFailures.length === 0
  && lintErrors === 0
  && lintWarnings === 0
  && resultPathSetMatch
  && before.sha256 === after.sha256;
const part = {
  index: partition,
  expectedFiles,
  fileCount: expectedFiles.length,
  resultFileCount: resultFiles.length,
  resultPathSetMatch,
  plannedInvocations: Math.ceil(expectedFiles.length / batchSize),
  completedInvocations: invocationRows.length,
  lintErrors,
  lintWarnings,
  generatedIgnored,
  processFailure: processFailures[0] ?? null,
  invocations: invocationRows,
  durationMs: Date.now() - started,
  passed,
};
const out = {
  schemaVersion: "velmere.pass13.partitioned-eslint.v4",
  runnerSchemaVersion: "velmere.pass36.a102r44p30.persistent-eslint-api.v1",
  generatedAt: now(),
  ok: passed,
  requestedPartitionStart: partition,
  requestedPartitionEnd: partition,
  fileCount: files.length,
  partitionCount: chunks.length,
  completedPartitions: 1,
  passedPartitions: passed ? 1 : 0,
  filesCovered: resultFiles.length,
  lintErrors,
  lintWarnings,
  processFailureCount: processFailures.length,
  firstProcessFailure: processFailures[0] ?? null,
  messages,
  processFailures,
  parts: [part],
  invocationPolicy: {
    fileChunkSize: ESLINT_FILE_CHUNK_SIZE,
    persistentProcess: true,
    persistentBatchSize: batchSize,
    maxOldSpaceSizeRequiredMiB: 4096,
    generatedIgnoreAllowlist: GENERATED_IGNORE_ALLOWLIST,
  },
  eslintIdentity,
  nodeExecutable: process.execPath,
  sourceBefore: before.sha256,
  sourceAfter: after.sha256,
  sourceImmutable: before.sha256 === after.sha256,
};
writeJson(output, out);
console.log(`R44P30 persistent ESLint ${String(partition).padStart(3, "0")}/${chunks.length}: ${passed ? "PASS" : "FAIL"} · ${expectedFiles.length} files · ${lintErrors} errors · ${lintWarnings} warnings · ${processFailures.length} process failures · ${invocationRows.length} in-process batches`);
if (!passed) process.exit(1);
