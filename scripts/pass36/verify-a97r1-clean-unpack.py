...
REV = "VELMERE_PASS36_A97R1_SCOPED_PAYMENT_OPERATOR_ASSERTIONS_DUAL_CONTROL_AND_SINGLE_USE_REQUEUE"
MANIFEST_REL = "_velmere/PASS36_A97R1_SOURCE_ONLY_MANIFEST.json"
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA97R1PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def main():
    ...
    steps=[
      ...
      {'id':'a97_authority_descendant','cmd':['node','scripts/pass36/verify-a97r1-action-required-authority.mjs'],'expectedStatus':'PASS_A97R1_ACTION_REQUIRED_AUTHORITY_NO_STAGING_CREDIT','timeout':360},
      {'id':'a97r1_payment_operator_assertion_control','cmd':['node','scripts/pass36/verify-a97r1-payment-operator-assertions.mjs'],'timeout':360},
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A97R1_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a97r1.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
    ...
    out=receipt_dir/'PASS36_A97R1_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
