...
if (state.revisionId !== REV || state.parentRevisionId !== PARENT) {
  throw new Error("a102r3_state_identity");
}
if (parent.revisionId !== PARENT || !parent.manifestDigestSha256) {
  throw new Error("a102r3_parent_manifest_identity");
}
if (receipt.revisionId !== REV || receipt.status !== "PASS_A102R3_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION") {
  throw new Error("a102r3_local_regression_receipt_identity");
}
const inventory = collect(root);
if (inventory.rejected.length) {
  throw new Error(`a102r3_rejected:${JSON.stringify(inventory.rejected)}`);
}
...
const core = {
  schemaVersion:
    "velmere.pass36.a102r3.action-required-current-root-descendant-manifest.v1",
  revisionId: REV,
  ...
  completedThrough: 89,
  coveredWorkRange:
    "A90-A102_LOCAL_FULL_REGRESSION_PRODUCT_PDF_AND_RELEASE_INTEGRITY_REBASELINE_NO_FORMAL_PASS_CREDIT",
...
console.log(JSON.stringify({
  status: "BUILT_A102R3_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
