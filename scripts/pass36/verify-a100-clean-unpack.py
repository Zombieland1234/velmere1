...
REV = "VELMERE_PASS36_A100R0_INCIDENT_KILL_SWITCH_ALERT_ACK_AND_CUSTOMER_COMMUNICATION_TRUTH_BOUNDARY"
MANIFEST_REL = "_velmere/PASS36_A100R0_SOURCE_ONLY_MANIFEST.json"
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA100PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def run_step(root: pathlib.Path, receipt_dir: pathlib.Path, index: int, step: dict):
    start=time.monotonic(); env={'PATH':os.environ.get('PATH',''),'LANG':'C.UTF-8','LC_ALL':'C.UTF-8','TZ':'UTC','CI':'1','NO_COLOR':'1','NODE_ENV':'test','VELMERE_A100_CLEAN_UNPACK':'1','VELMERE_A95_NO_WRITE':'1'}
...
def main():
    ...
    steps=[
      ...
      {'id':'a100_authority_descendant','cmd':['node','scripts/pass36/verify-a100-action-required-authority.mjs'],'expectedStatus':'PASS_A100_ACTION_REQUIRED_AUTHORITY_NO_STAGING_CREDIT','timeout':360},
      {'id':'a100_incident_kill_switch_customer_communication_boundary','cmd':['node','scripts/pass36/verify-a100-incident-kill-switch-customer-communication-boundaries.mjs'],'expectedStatus':'PASS_A100_INCIDENT_KILL_SWITCH_CUSTOMER_COMMUNICATION_BOUNDARY_LOCAL_ONLY','timeout':600},
      ...
      {'id':'a99_historical_descendant','cmd':['node','scripts/pass36/verify-a99-current-root-descendant.mjs'],'expectedStatus':'PASS_A99_HISTORICAL_DESCENDANT_BOUND_TO_CURRENT_NO_PROMOTION','timeout':360},
      {'id':'a98_email_storage_kms_historical','cmd':['node','scripts/pass36/verify-a98-email-storage-kms-boundaries.mjs'],'expectedStatus':'PASS_A98_EMAIL_STORAGE_KMS_BOUNDARY_LOCAL_ONLY','timeout':360},
      {'id':'a97r1_payment_operator_assertion_historical','cmd':['node','scripts/pass36/verify-a97r1-payment-operator-assertions.mjs'],'timeout':360},
      {'id':'a96_rls_local_implementation_historical','cmd':['node','scripts/pass36/verify-a96-rls-tenant-isolation.mjs'],'timeout':360},
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A100R0_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a100.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
      ...
      {'gate':'real_a95_a100_staging_lifecycles','status':'0_OF_REQUIRED_REAL_STAGES'},
    ...
    out=receipt_dir/'PASS36_A100R0_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
