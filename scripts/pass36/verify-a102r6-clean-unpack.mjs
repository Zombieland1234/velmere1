...
import {
  ...
  REVISION_ID,
  validateA102R6ParsedArchive,
} from "./package-a102r6-deterministic.mjs";
import { collect, payload } from "./a102r6-source-boundary.mjs";
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    invariant(allowed.has(token), `a102r6_clean_argument_unknown:${token}`);
    invariant(!values.has(token), `a102r6_clean_argument_duplicate:${token}`);
    const value = argv[index + 1];
    invariant(typeof value === "string" && value.length > 0 && !value.startsWith("--"), `a102r6_clean_argument_value:${token}`);
    values.set(token, value);
    index += 1;
  }
  for (const required of allowed) invariant(values.has(required), `a102r6_clean_argument_required:${required}`);
  return {
...
function writeEntry(root, entry) {
  ...
  invariant(absolute.startsWith(`${root}${path.sep}`), `a102r6_clean_extract_escape:${entry.path}`);
...
function main() {
  ...
  invariant(sourceMeta.isFile() && !sourceMeta.isSymbolicLink(), "a102r6_clean_source_zip_not_regular");
  fs.mkdirSync(receiptDir, { recursive: true });
  const parsed = parseDeterministicZip(sourceZip);
  const packageValidation = validateA102R6ParsedArchive(parsed, "source");
  ...
  invariant(fs.existsSync(path.join(cleanRoot, SOURCE_MANIFEST_PATH)), "a102r6_clean_source_manifest_missing");

  const beforeInventory = collect(cleanRoot);
  invariant(beforeInventory.rejected.length === 0, `a102r6_clean_rejected_before:${JSON.stringify(beforeInventory.rejected)}`);
  ...
  const steps = [
    ...
    { id: "a102r6_authority", command: [node, "scripts/pass36/verify-a102r6-action-required-authority.mjs"], expected: "PASS_A102R6_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT" },
    ...
    { id: "a102r6_private_account_browser_state", command: [node, ...loader, "scripts/pass36/test-a102r6-private-account-browser-state-boundary.ts"], expected: "PASS_A102R6_PRIVATE_ACCOUNT_EPHEMERAL_SERVER_CONFIRMED_BOUNDARY_NO_PROMOTION" },
  ...
  const receipt = {
    schemaVersion: "velmere.pass36.a102r6.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: localContractPassed
      ? "ACTION_REQUIRED_A102R6_CLEAN_UNPACK_LOCAL_CONTRACT_NO_EXACT_RUNTIME_BROWSER_OR_REAL_CREDIT"
      : "FAIL_A102R6_CLEAN_UNPACK_LOCAL_CONTRACT",
    sourceZip: {
    ...
    notExecutedOrNotCredited: [
      ["fresh_exact_npm_ci_lint_typecheck_builds_next_start", "NOT_EXECUTED_ON_A102R6_FINAL_BYTES"],
  ...
  const receiptPath = path.join(receiptDir, "PASS36_A102R6_CLEAN_UNPACK_RECEIPT.json");
...
try { main(); }
  ...
  process.stderr.write(`${JSON.stringify({
    schemaVersion: "velmere.pass36.a102r6.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: "FAIL_A102R6_CLEAN_UNPACK_LOCAL_CONTRACT",
