...
def main() -> int:
    ...
    for row in manifest["rows"]:
        ...
        result = {
            ...
            "labelsOk": labels_ok, "boundaryOk": boundary_ok, "advancedMarkersOk": advanced_markers_ok, "pdfSha256": pdf_sha, "bytes": len(data),
        }
        rows.append(result)
        if not ok:
    ...
    if args.sample_dir:
        ...
        samples = []
    receipt = {
        "schemaVersion": "velmere.pass36.a102r44p7.audit-pdf-corpus-verification.v1",
        "status": "PASS_A102R44P7_ADVANCED_EVIDENCE_PDF_CORPUS_NO_REAL_CUSTOMER_CREDIT" if not failures else "FAIL_A102R44P7_ADVANCED_EVIDENCE_PDF_CORPUS",
        ...
        "rows": rows,
        "truthBoundary": "This verifies local deterministic R44P7 Basic/Pro/Advanced PDF files, active-content safety, and Advanced-only evidence markers. It is not independent adjudication, browser delivery, real customer comprehension, willingness-to-pay, staging, LIVE or paid-release proof.",
    }
    Path(args.receipt).write_text(json.dumps(receipt, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({k: receipt[k] for k in ["status", "documentsExecuted", "documentsPassed", "pagesExecuted", "failed", "byTier", "activeContentDocuments", "blankPages", "edgeTouchPages", "fontEmbeddedDocuments", "toUnicodeDocuments", "advancedMarkerDocuments", "realCustomerPdfCredit", "paidReleaseCredit"]}, indent=2, ensure_ascii=False))
