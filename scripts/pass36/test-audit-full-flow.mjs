...
async function run() {
  ...
  console.log("Submitting Basic Prescreen with authenticated session...");
  await submitBtn.click();
  await page.waitForTimeout(3000);
  
  const metaText = await page.locator('.audit-v4609-intake-meta').innerText();
  console.log("INTAKE RESULT:", metaText);
  
  await page.screenshot({ path: "artifacts/forensic/screen_investigation/audit_basic_case_created_auth.png", fullPage: true });
  console.log("Screenshot saved to artifacts/forensic/screen_investigation/audit_basic_case_created_auth.png");
