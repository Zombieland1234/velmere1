...
const REV='VELMERE_PASS36_A102R31_ACTION_REQUIRED_ROUTE_TRANSITION_HASH_FOCUS_PREFETCH_BOUND_AND_ACCESSIBILITY_CONTINUITY_RECOVERY_NO_REAL_CREDIT';
...
const output=path.join(root,'config/pass36/a102r31-local-regression-receipt.json');const logDir=path.join(root,'.velmere/artifacts/a102r31-local-regression');fs.rmSync(logDir,{recursive:true,force:true});fs.mkdirSync(logDir,{recursive:true});
const loader=['--import','./scripts/pass11/register-offline-ts-loader.mjs'];
const definitions=[
 ['a102r31',['node',...loader,'scripts/pass36/test-a102r31-route-transition-hash-focus-prefetch-accessibility-boundary.mjs']],
...
const receipt={schemaVersion:'velmere.pass36.a102r31.local-regression-receipt.v1',revisionId:REV,parentRevisionId:PARENT,generatedAt:'2026-07-31T04:30:00.000Z',status:'PASS_A102R31_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION',checkpointClass:'ACTION_REQUIRED_NON_PASS',requiredStages:definitions.length,executedStages:definitions.length,passedStages:definitions.length,failedStages:0,stages,keyDenominators:{a102r31BoundaryChecks:29,a102r30ParentBoundaryChecks:26,a102r29ParentBoundaryChecks:23,a102r28ParentBoundaryChecks:14,a102r27ParentBoundaryChecks:20,a102r26ParentBoundaryChecks:33,devBootstrapCriticalHashes:71,localeVariantsVerified:3,localizedLabelsInfluenceSessionPolicy:false,realMarkets4hCadenceMinutes:60,shield4hCadenceMinutes:240,realMarkets1mCadenceMinutes:43200,shield1mCadenceMinutes:1440,referenceCanClaimVerifiedSource:false,visiblePass4540Jargon:false,canonicalActionLedgerSurface:true,routeDispatchChecks:1480,routeRoutes:160,routeDispatchTamperChecks:7,lazyRouteChecks:176,a57Checks:1138,protectedIntegrityRows:1073,apiBodyChecks:37,malformedJsonChecks:112,mega4800Checks:61,a59Checks:77,productTierChecks:186,zeroBudgetChecks:439},sourceAudit,localClosure:{crossRouteHashTargetReadiness:true,crossRouteHashFocusTransfer:true,nonHashMainFocusTransfer:true,prefetchCacheLimit:48,pointerIntentCancellation:true,focusIntentCancellation:true,nonSelfTargetsIntercepted:false,screenReaderStatusBound:true,productionSyntheticChartCredit:false},environmentBlockers:[{id:'exact_node_npm_project_dependencies',credit:false,observedNode:process.versions.node,requiredNode:'24.18.0',requiredNpm:'11.16.0'},{id:'exact_playwright_chromium',credit:false},{id:'real_route_transition_hash_focus_accessibility_browser_matrix_0_of_18',credit:false},{id:'a102_real_observation_0_of_3',credit:false},{id:'staging_rights_legal_customers',credit:false}],realEvidence:{providerRights:0,browserRows:0,realObservationRuns:0,stagingStages:0,legalDecisions:0,customerCohorts:0},promotion:{globalDecision:'NO_GO',live:false,saleEnabled:false,productionApproved:false,worldClassProven:false},truthBoundary:'Local route-transition hash/focus/prefetch/accessibility continuity only. No exact build/browser, screenshot, performance, staging, rights, legal, customer, LIVE or sale credit.'};fs.writeFileSync(output,JSON.stringify(receipt,null,2)+'\n');console.log(JSON.stringify({status:receipt.status,stages:receipt.passedStages,sourceAudit,output:path.relative(root,output)},null,2));
