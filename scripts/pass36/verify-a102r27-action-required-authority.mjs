...
const state=read('config/pass36/a102r27-action-required-current-state.json');
const program=read('config/pass36/a102r27-world-class-completion-program.json');
...
ok(auth.claims?.a102r27BoundaryChecks===20,'boundary');
ok(auth.claims?.a102r27ProviderSymbolRequestAuthority===true,'provider_symbol_authority');
ok(auth.claims?.a102r27ProviderVenueCacheIdentity===true,'provider_venue_identity');
ok(auth.claims?.a102r27AliasCacheCollisions===0,'alias_collisions');
ok(auth.claims?.a102r27RealBrowserRows===0,'browser_zero');
ok(auth.claims?.a102r27ExactReleaseCredit===false,'release_false');

const out={
  status:'PASS_A102R27_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_STAGING_RIGHTS_LEGAL_OR_SALE_CREDIT',
