...
} from "./a102r7-source-boundary.mjs";
...
if (state.revisionId !== REV || state.parentRevisionId !== PARENT) {
  throw new Error("a102r7_state_identity");
}
if (parent.revisionId !== PARENT || !parent.manifestDigestSha256) {
  throw new Error("a102r7_parent_manifest_identity");
}
if (receipt.revisionId !== REV || receipt.status !== "PASS_A102R7_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION") {
  throw new Error("a102r7_local_regression_receipt_identity");
}
const inventory = collect(root);
if (inventory.rejected.length) {
  throw new Error(`a102r7_rejected:${JSON.stringify(inventory.rejected)}`);
}
...
const core = {
  schemaVersion:
    "velmere.pass36.a102r7.action-required-current-root-descendant-manifest.v1",
  revisionId: REV,
  ...
  completedThrough: 89,
  coveredWorkRange:
    "A90-A102_LOCAL_MOBILE_WALLET_DEEPLINK_QUERY_HASH_PRIVATE_PATH_EXTERNAL_PROVIDER_PRIVACY_AND_VERIFIER_HARDENING_NO_FORMAL_PASS_CREDIT",
...
console.log(JSON.stringify({
  status: "BUILT_A102R7_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
