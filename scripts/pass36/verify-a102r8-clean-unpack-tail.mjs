...
const REVISION_ID = "VELMERE_PASS36_A102R8_ACTION_REQUIRED_CHECKOUT_PII_AUDIT_CASE_AND_PDF_ACTIVITY_BROWSER_PERSISTENCE_FAIL_CLOSED_NO_REAL_CREDIT";
...
const parseGroup = (argv) => {
  if (argv.length !== 2 || argv[0] !== "--group") throw new Error("a102r8_clean_group_argument");
  return argv[1];
};
...
const groups = {
  core: [
    { id: "checkout_customer_browser_privacy", command: [node, ...loader, "scripts/pass36/test-a102r8-checkout-customer-browser-privacy-boundary.ts"], verify: (p) => statusIs("PASS_A102R8_CHECKOUT_PII_AUDIT_CASE_AND_PDF_METADATA_BROWSER_PERSISTENCE_FAIL_CLOSED_NO_PROMOTION")(p) && p?.assertions === 41 },
    { id: "approved_changes", command: [node, "scripts/pass36/verify-a102r8-approved-checkout-customer-browser-privacy-changes.mjs"], verify: (p) => statusIs("PASS_A102R8_APPROVED_CHECKOUT_CUSTOMER_BROWSER_PRIVACY_CHANGES_NO_PROMOTION")(p) && p?.checks === 23 },
    { id: "local_regression_receipt", command: [node, "scripts/pass36/verify-a102r8-local-regression-receipt.mjs"], verify: (p) => statusIs("PASS_A102R8_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION")(p) && p?.checks === 15 },
    { id: "descendant", command: [node, "scripts/pass36/verify-a102r8-current-root-descendant.mjs"], verify: (p) => statusIs("PASS_A102R8_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION")(p) && p?.checks === 12 },
    { id: "authority", command: [node, "scripts/pass36/verify-a102r8-action-required-authority.mjs"], verify: (p) => statusIs("PASS_A102R8_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT")(p) && p?.checks === 19 },
  ],
  history: [
...
if (!stages) throw new Error(`a102r8_clean_group_unknown:${group}`);
const results = [];
...
const output = {
  schemaVersion: "velmere.pass36.a102r8.clean-unpack-group-verification.v1",
  revisionId: REVISION_ID,
  group,
  status: failed.length || results.length !== stages.length
    ? `FAIL_A102R8_CLEAN_UNPACK_GROUP_${group.toUpperCase()}`
    : `PASS_A102R8_CLEAN_UNPACK_GROUP_${group.toUpperCase()}_NO_PROMOTION`,
