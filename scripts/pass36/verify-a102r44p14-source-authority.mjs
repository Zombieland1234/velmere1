...
const REVISION = "VELMERE_PASS36_A102R44P14_ACTION_REQUIRED_WEAK_CATEGORY_RECALL_CONTROL_FALSE_ALERT_SUPPRESSION_AND_PRODUCT_ANALYZER_INTEGRATION_NO_LIVE_CREDIT";
const MANIFEST_REL = "_velmere/PASS36_A102R44P14_SOURCE_ONLY_MANIFEST.json";
const manifest = JSON.parse(fs.readFileSync(path.join(ROOT, MANIFEST_REL), "utf8"));
const excludedTop = new Set(["node_modules", ".git", "artifacts", "coverage", "test-results", "playwright-report", ".cache", "tmp", "temp", "__pycache__"]);
const forbiddenParts = new Set(["node_modules", ".git", "artifacts", "coverage", "test-results", "playwright-report", ".cache", "__pycache__"]);
...
check("schema", manifest.schemaVersion === "velmere.pass36.a102r44p14.source-manifest.v1");
...
const missing = expected.filter((row) => !currentMap.has(row.path)).map((row) => row.path);
...
const state = JSON.parse(fs.readFileSync(path.join(ROOT, "config/pass36/a102r44p14-action-required-current-state.json"), "utf8"));
...
const result = { schemaVersion: "velmere.pass36.a102r44p14.source-authority-receipt.v1", status: failed.length === 0 ? "PASS" : "FAIL", checks: checks.length, passed: checks.length - failed.length, failed: failed.length, revisionId: REVISION, manifestSha256: sha(fs.readFileSync(path.join(ROOT, MANIFEST_REL))), aggregateSha256: aggregate, fileCount: rows.length, sourceImmutable: true, rows: checks };
