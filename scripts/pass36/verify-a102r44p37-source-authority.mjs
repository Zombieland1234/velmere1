...
const manifestRelative = "_velmere/PASS36_A102R44P37_SOURCE_ONLY_MANIFEST.json";
const manifestPath = path.join(root, ...manifestRelative.split("/"));
const fail = (message, extra = {}) => {
  process.stdout.write(`${JSON.stringify({ status: "FAIL_R44P37_SOURCE_AUTHORITY", message, ...extra }, null, 2)}\n`);
  process.exit(1);
};
...
for (let index = 0; index < actual.length; index += 1) {
  ...
  if (current.path !== bound.path || current.byteLength !== bound.byteLength || current.sha256 !== bound.sha256) {
    fail("FILE_BINDING_MISMATCH", { index, actual: current, expected: bound });
  }
}
...
process.stdout.write(`${JSON.stringify({
  status: "PASS_R44P37_SOURCE_AUTHORITY",
  revisionId: REV,
  pathOrderAlgorithm: manifest.pathOrderAlgorithm,
