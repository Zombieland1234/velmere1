...
import { REV, PARENT, RECEIPT, STATE } from "./a102r8-source-boundary.mjs";
...
add("status",receipt.phase==="FINAL"&&receipt.status==="PASS_A102R8_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION",receipt.status);
...
add("checkout-customer-privacy",receipt.keyDenominators?.a102r8CheckoutCustomerBrowserPrivacyChecks===41);
...
add("state",state.revisionId===REV&&state.parentRevisionId===PARENT&&state.localImplementation?.checkoutPiiPersistentBrowserStorageRemoved===true&&state.localImplementation?.legacyCheckoutPiiDraftsPurgedWithoutReadOrMigration===true&&state.localImplementation?.auditCaseReferencePersistentBrowserStorageRemoved===true&&state.localImplementation?.pdfActivityMetadataPersistentBrowserStorageRemoved===true&&state.localImplementation?.legacyAuditAndPdfBrowserRowsPurgedWithoutReadOrMigration===true&&state.runtimeTruth?.freshWebpackBuildOnA102R8BytesExecuted===false&&state.runtimeTruth?.freshBrowserStoragePrivacyRowsExecuted===0);
const failed=checks.filter((r)=>!r.passed);
console.log(JSON.stringify({status:failed.length?"FAIL_A102R8_LOCAL_REGRESSION_RECEIPT":"PASS_A102R8_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION",revisionId:REV,checks:checks.length,passed:checks.length-failed.length,failed:failed.length,results:checks,live:false,saleEnabled:false},null,2));
