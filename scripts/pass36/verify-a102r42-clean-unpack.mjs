...
import { collect, payload } from "./a102r42-source-boundary.mjs";
...
const CLEAN_RECEIPT_FILE_NAME = "PASS36_A102R42_CLEAN_UNPACK_RECEIPT.json";
const FROZEN_REGRESSION_STAGE_IDS = parseStrictJsonCli(fs.readFileSync("config/pass36/a102r42-frozen-regression-denominator-migration.json", "utf8"), { maxBytes: 1024 * 1024, maxDepth: 32, maxNodes: 10000, requireObject: true }).currentStageIds;
...
export function writeFailureReceipt(receiptDir, error) {
  ...
  const receipt = {
    schemaVersion: "velmere.pass36.a102r42.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: "FAIL_A102R42_CLEAN_UNPACK",
    passed: false,
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    invariant(allowed.has(name), `a102r42_clean_argument_unknown:${name}`);
    invariant(!values.has(name), `a102r42_clean_argument_duplicate:${name}`);
    const value = argv[++index];
    invariant(typeof value === "string" && value.length > 0 && !value.startsWith("--"), `a102r42_clean_argument_value:${name}`);
    values.set(name, value);
  }
  for (const name of allowed) invariant(values.has(name), `a102r42_clean_argument_required:${name}`);
...
function writeEntry(cleanRoot, entry) {
  ...
  invariant(absolute.startsWith(`${cleanRoot}${path.sep}`), `a102r42_clean_extract_escape:${entry.path}`);
...
function completeTreeSnapshot(root) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current.absolute, { withFileTypes: true }).sort((left, right) => left.name < right.name ? -1 : left.name > right.name ? 1 : 0)) {
      ...
      invariant(!metadata.isSymbolicLink(), `a102r42_clean_tree_symlink:${relative}`);
      if (entry.isDirectory()) { stack.push({ absolute, prefix: relative }); continue; }
      invariant(entry.isFile(), `a102r42_clean_tree_special:${relative}`);
      const observed = readDescriptorBoundRegularFile(absolute, { errorPrefix: `a102r42_clean_tree_${safeId(relative)}` });
...
export function verifyExecutionPackagedBytes(parsedEntries, executionRoot) {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      const absolute = path.join(current.absolute, entry.name);
      const metadata = fs.lstatSync(absolute);
      invariant(!metadata.isSymbolicLink(), `a102r42_clean_execution_symlink_or_reparse:${relative}`);
      if (entry.isDirectory()) {
        stack.push({ absolute, prefix: relative });
        continue;
      }
      invariant(entry.isFile(), `a102r42_clean_execution_special:${relative}`);
      if (packaged.has(relative)) {
        const observed = readDescriptorBoundRegularFile(absolute, { errorPrefix: `a102r42_clean_execution_${safeId(relative)}` });
...
export function nestedReceiptContract(fileName, value) {
  ...
  if (fileName === "A102R42_FULL_REGRESSION_RECEIPT.json") {
    const stageIds = Array.isArray(value.stages) ? value.stages.map((row) => row?.id) : [];
    return value.schemaVersion === "velmere.pass36.a102r42.frozen-local-regression-receipt.v1"
      && value.status === "PASS_A102R42_FROZEN_SOURCE_FULL_LOCAL_REGRESSION_NO_PROMOTION"
      && value.requiredStages === 29 && value.executedStages === 29 && value.passedStages === 29
      && Array.isArray(value.failedStages) && value.failedStages.length === 0
      && Array.isArray(value.stages) && value.stages.length === 29 && new Set(stageIds).size === 29
      && JSON.stringify(stageIds) === JSON.stringify(FROZEN_REGRESSION_STAGE_IDS)
      && value.stages.every((row) => row?.passed === true)
      && value.executedStages === value.stages.length && value.passedStages === value.stages.filter((row) => row?.passed === true).length
      && value.sourceImmutable === true && value.exactBuildBrowserCredit === false && value.a77r1ToA80r1Credit === false;
  }
  const expectedStatuses = {
    "A102R42_CURRENT_AUTHORITY_RECEIPT.json": "PASS_A102R42_ACTION_REQUIRED_AUTHORITY_A60_TOTAL_FAIL_CLOSED_RECEIPT_AND_LOG_CLASSIFIER_BOUNDARY_EXACT_WINDOWS_NO_LIVE_CREDIT",
    "A102R42_DESCENDANT_RECEIPT.json": "PASS_A102R42_CURRENT_ROOT_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
    "A102R42_APPROVED_CHANGES_RECEIPT.json": "PASS_A102R42_APPROVED_CURRENT_SOURCE_CHANGES_COMPLETE_NO_PROMOTION",
  };
  return value.schemaVersion === "velmere.pass36.a102r42.physical-command-receipt-wrapper.v1"
...
function main() {
  ...
  invariant(process.version === "v24.18.0", `a102r42_clean_exact_node_required:${process.version}`);
  const sourceObserved = readDescriptorBoundRegularFile(sourceZip, { errorPrefix: "a102r42_clean_source_zip" });
  invariant(sourceObserved.binding.fileName === SOURCE_FILE_NAME, "a102r42_clean_source_zip_filename");
  invariant(!fs.existsSync(receiptDir), "a102r42_clean_receipt_dir_must_be_new");
  ...
  const packageValidation = validateA102R42ParsedArchive(parsed, "source");
  const cleanRoot = `${receiptDir}.clean-source`;
  invariant(!fs.existsSync(cleanRoot), "a102r42_clean_work_root_must_be_new");
  fs.mkdirSync(cleanRoot, { recursive: false });
  for (const entry of parsed.entries) writeEntry(cleanRoot, entry);
  invariant(fs.existsSync(path.join(cleanRoot, SOURCE_MANIFEST_PATH)), "a102r42_clean_source_manifest_missing");
  const beforeInventory = collect(cleanRoot, { platform: process.platform });
  invariant(beforeInventory.rejected.length === 0, `a102r42_clean_rejected_before:${JSON.stringify(beforeInventory.rejected)}`);
  const beforeAuthority = payload(beforeInventory.rows);
  const before = completeTreeSnapshot(cleanRoot);
  const descendantBytes = fs.readFileSync(path.join(cleanRoot, "config/pass36/a102r42-current-root-descendant-manifest.json"));
  ...
  invariant(!fs.existsSync(executionRoot), "a102r42_clean_execution_root_must_be_new");
  ...
  const npmCliBefore = readDescriptorBoundRegularFile(npmCliPath, { maxBytes: 4 * 1024 * 1024, errorPrefix: "a102r42_clean_exact_npm_cli_before" });
  const npmPackageObserved = readDescriptorBoundRegularFile(path.join(runtimeRoot, "node_modules/npm/package.json"), { maxBytes: 1024 * 1024, errorPrefix: "a102r42_clean_exact_npm_package" });
  ...
  invariant(npmPackage.version === "11.16.0", `a102r42_clean_exact_npm_version:${npmPackage.version}`);
  const packageLockPath = path.join(executionRoot, "package-lock.json");
  const packageLockBefore = readDescriptorBoundRegularFile(packageLockPath, { maxBytes: 32 * 1024 * 1024, errorPrefix: "a102r42_clean_package_lock_before" });
  const nodeModulesPath = path.join(executionRoot, "node_modules");
  invariant(!fs.existsSync(nodeModulesPath), "a102r42_clean_node_modules_must_be_absent_before_install");
  const systemRootValue = process.env.SystemRoot ?? process.env.SYSTEMROOT;
  invariant(typeof systemRootValue === "string" && path.isAbsolute(systemRootValue) && !systemRootValue.includes(";"), "a102r42_clean_system_root_invalid");
  const systemRoot = fs.realpathSync(systemRootValue);
  invariant(fs.lstatSync(systemRoot).isDirectory(), "a102r42_clean_system_root_not_directory");
  const system32 = path.join(systemRoot, "System32");
  invariant(fs.lstatSync(system32).isDirectory(), "a102r42_clean_system32_not_directory");
  invariant(!runtimeRoot.includes(";"), "a102r42_clean_runtime_root_path_separator");
  ...
  const steps = [
    ...
    { id: "a102r42_authority", command: [node, "scripts/pass36/verify-a102r42-action-required-authority.mjs"], expected: "PASS_A102R42_ACTION_REQUIRED_AUTHORITY_A60_TOTAL_FAIL_CLOSED_RECEIPT_AND_LOG_CLASSIFIER_BOUNDARY_EXACT_WINDOWS_NO_LIVE_CREDIT", exact: [["checks", 47], ["passed", 47], ["failed", 0]], noGo: true, falseFields: ["live", "saleEnabled", "productionApproved", "worldClassProven"] },
    { id: "a102r42_descendant", command: [node, "scripts/pass36/verify-a102r42-current-root-descendant.mjs"], expected: "PASS_A102R42_CURRENT_ROOT_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION", exact: [["checks", 28], ["passed", 28], ["failed", 0], ["historicalChainChecks", 422]], noGo: true, falseFields: ["live", "saleEnabled"] },
    { id: "a102r42_approved_changes", command: [node, "scripts/pass36/verify-a102r42-approved-current-source-changes.mjs"], expected: "PASS_A102R42_APPROVED_CURRENT_SOURCE_CHANGES_COMPLETE_NO_PROMOTION", exact: [["failed", 0]], approvedLedgerFormula: true, noGo: true, falseFields: ["live", "saleEnabled"] },
    { id: "a102r42_authority_denominator", command: [node, "scripts/pass36/verify-a102r42-current-source-authority-denominator-migration.mjs"], expected: "PASS_A102R42_CURRENT_SOURCE_AUTHORITY_DENOMINATOR_MIGRATION_46_TO_60_NO_PROMOTION", exact: [["checks", 68], ["passed", 68], ["failed", 0]], noGo: true, falseFields: ["live", "saleEnabled"] },
    ...
    { id: "a102r42_package_boundary", command: [node, "scripts/pass36/test-a102r42-package-portability-and-parser.mjs"], expected: "PASS_A102R42_PACKAGE_PORTABILITY_AND_STRICT_MANIFEST_BOUNDARY_NO_PROMOTION", exact: [["required", 38], ["executed", 38], ["passed", 38], ["failed", 0], ["denominatorMigration.old", 36], ["denominatorMigration.current", 38], ["denominatorMigration.retained", 36], ["denominatorMigration.added", 2], ["denominatorMigration.removed", 0], ["denominatorMigration.verifierChecks", 16]], noGo: true, falseFields: ["live", "saleEnabled", "productionApproved", "worldClassProven"] },
    { id: "a80r1_mechanism", command: [node, "scripts/pass36/test-a102r42-a80r1-two-phase-release-controller.mjs"], expected: "PASS_A102R42_A80R1_TWO_PHASE_CONTROLLER_MECHANISM_NO_PROMOTION", exact: [["required", 48], ["executed", 48], ["passed", 48], ["failed", 0]], noGo: true, falseFields: ["live", "saleEnabled", "productionApproved", "worldClassProven"] },
    { id: "a102r42_frozen_local_regression", command: [node, "scripts/pass36/run-a102r42-frozen-local-regression.mjs", "--receipt-dir", path.join(receiptDir, "full-regression")], expected: "PASS_A102R42_FROZEN_SOURCE_FULL_LOCAL_REGRESSION_NO_PROMOTION", exact: [["requiredStages", 29], ["executedStages", 29], ["passedStages", 29], ["sourceImmutable", true], ["exactBuildBrowserCredit", false], ["a77r1ToA80r1Credit", false]], noGo: true, falseFields: ["live", "saleEnabled", "productionApproved", "worldClassProven"], timeoutMs: 3600000 },
  ];
  const results = [];
  ...
  if (results[1]?.passed) {
    const packageLockAfter = readDescriptorBoundRegularFile(packageLockPath, { maxBytes: 32 * 1024 * 1024, errorPrefix: "a102r42_clean_package_lock_after" });
    const npmCliAfter = readDescriptorBoundRegularFile(npmCliPath, { maxBytes: 4 * 1024 * 1024, errorPrefix: "a102r42_clean_exact_npm_cli_after" });
  ...
  const regressionStep = results.find((row) => row.id === "a102r42_frozen_local_regression");
  if (regressionStep?.passed) {
    for (const fileName of [
      "A102R42_CURRENT_AUTHORITY_RECEIPT.json",
      "A102R42_DESCENDANT_RECEIPT.json",
      "A102R42_APPROVED_CHANGES_RECEIPT.json",
      "A102R42_FULL_REGRESSION_RECEIPT.json",
    ]) {
      const relativeSource = `full-regression/${fileName}`;
      const sourceReceipt = readDescriptorBoundRegularFile(path.join(receiptDir, ...relativeSource.split("/")), { maxBytes: 16 * 1024 * 1024, errorPrefix: "a102r42_clean_nested_receipt" });
      ...
      invariant(value.revisionId === REVISION_ID && JSON.stringify(value.sourceBinding) === JSON.stringify(expectedSourceBinding), `a102r42_clean_nested_receipt_source_binding:${fileName}`);
      invariant(nestedReceiptContract(fileName, value), `a102r42_clean_nested_receipt_contract:${fileName}`);
      ...
      const copied = readDescriptorBoundRegularFile(destination, { maxBytes: 16 * 1024 * 1024, errorPrefix: "a102r42_clean_copied_receipt" });
      invariant(copied.binding.byteLength === sourceReceipt.binding.byteLength && copied.binding.sha256 === sourceReceipt.binding.sha256, `a102r42_clean_nested_receipt_copy_binding:${fileName}`);
  ...
  const receipt = {
    schemaVersion: "velmere.pass36.a102r42.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: passed ? "PASS_A102R42_ACTION_REQUIRED_CLEAN_UNPACK_A58_FIRST_NO_PROMOTION" : "FAIL_A102R42_CLEAN_UNPACK",
...
function runCli() {
  try { main(); }
    ...
    process.stderr.write(`${JSON.stringify({ schemaVersion: "velmere.pass36.a102r42.clean-unpack-verification.v1", revisionId: REVISION_ID, status: "FAIL_A102R42_CLEAN_UNPACK", passed: false, failure, a77r1ToA80r1Credit: false, globalDecision: "NO_GO", live: false, saleEnabled: false, productionApproved: false, worldClassProven: false })}\n`);
    process.exitCode = 1;
  }
}
