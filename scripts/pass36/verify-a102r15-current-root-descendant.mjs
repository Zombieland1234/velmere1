...
import { REV, PARENT, MANIFEST, PARENT_MANIFEST, STATE, PROGRAM, RECEIPT, sha256, canonicalJson, readJson, collect, payload } from "./a102r15-source-boundary.mjs";
...
add("claims", claims.a102PassCredit === false && claims.a116PassCredit === false && claims.assetAnalysisClipboardBoundaryChecks === 54 && claims.rawAssetClipboardSinksRemaining === 0 && claims.redactedSummaryOnly === true && claims.receiptPacketIdentifiersIncluded === false && claims.sourceLabelsTimestampsClaimsIncluded === false && claims.realAssetClipboardBrowserRows === 0 && claims.liveProven === false && claims.saleEnabled === false && claims.productionApproved === false && claims.worldClassProven === false, claims);
const failed = checks.filter((row) => !row.passed);
console.log(JSON.stringify({ status: failed.length ? "FAIL_A102R15_DESCENDANT" : "PASS_A102R15_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION", checks: checks.length, passed: checks.length - failed.length, failed: failed.length, results: checks }, null, 2));
