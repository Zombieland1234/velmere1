...
const checks = [
  ...
  ["parent-archive", manifest.parentSourceArchive?.byteLength === 56103275 && manifest.parentSourceArchive?.sha256 === "8078edb40b7bfa1d4eda450e1be4bb57107563abf9b586c0655a56bc59bbd120"],
...
checks.push(["provider-diagnostic-bound", state.localEvidence?.providerDiagnostic?.realNetworkObservedAssets === 15 && state.localEvidence?.providerDiagnostic?.realNetworkObservedProviderRows === 30 && state.localEvidence?.providerDiagnostic?.rightsApprovedAssets === 0]);
checks.push(["browser-process-isolation-bound", state.localEvidence?.browserClosure?.processIsolationMode === "SIX_ROUTE_BROWSER_PROCESS_BATCH" && state.localEvidence?.browserClosure?.verifierChecksDenominator === 584 && state.localEvidence?.browserClosure?.exactWindowsCredit === false]);
checks.push(["typescript-denominator-bound", state.localEvidence?.typescriptDenominatorMigration?.currentPreBuildPhysical === 160 && state.localEvidence?.typescriptDenominatorMigration?.currentPostBuildPhysical === 301]);
const failed = checks.filter(([, ok]) => !ok);
const result = {
  schemaVersion: "velmere.pass36.a102r44p12.source-authority-verification.v1",
  status: failed.length ? "FAIL_R44P12_SOURCE_AUTHORITY" : "PASS_R44P12_SOURCE_AUTHORITY_NO_PROMOTION",
