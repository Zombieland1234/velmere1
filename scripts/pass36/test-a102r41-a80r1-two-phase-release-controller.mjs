...
import {
  PHASE1_BLOCKED, PHASE1_VERIFIED, RECEIPT_SPECS,
...
import { collectA102R41Inventory, MATERIALS_FILE_NAME, ROADMAP_FILE_NAME, SOURCE_FILE_NAME } from "./package-a102r41-deterministic.mjs";
import { createPortableRejectedSymlink } from "./portable-symlink-negative-fixture.mjs";

const digest = (character) => character.repeat(64);
const source = {
  revisionId: REV,
  descendantManifestPath: "config/pass36/a102r41-current-root-descendant-manifest.json",
...
const mode = JSON.parse(fs.readFileSync("config/pass36/a102r41-cross-platform-source-mode-policy.json", "utf8"));
const frozenMode = JSON.parse(fs.readFileSync("config/pass36/a102r40-cross-platform-source-mode-policy.json", "utf8"));
const modeMigration = JSON.parse(fs.readFileSync("config/pass36/a102r41-source-mode-denominator-migration.json", "utf8"));
const mechanismMigration = JSON.parse(fs.readFileSync("config/pass36/a102r41-a80r1-mechanism-denominator-migration.json", "utf8"));
const parentManifest = JSON.parse(fs.readFileSync("config/pass36/a102r41-parent-source-package-manifest.json", "utf8"));
const parentPaths = new Set(parentManifest.entries.map((row) => row.path));
const currentRows = collectA102R41Inventory(process.cwd(), "source", { allowReservedManifestInput: true }).rows;
...
const validateModeMigration = (document, observedShebangPaths = observedNewShebangPaths) => {
  ...
  return document.migrationDigestSha256 === crypto.createHash("sha256").update(canonical(core)).digest("hex")
    && document.oldDenominator === 35 && document.newDenominator === 58
    && document.retainedPaths.length === 35 && document.addedPaths.length === 23 && document.removedPaths.length === 0
    && new Set(document.addedPaths).size === 23
...
const validateMechanismMigration = (document) => {
  const core = { ...document }; delete core.migrationDigestSha256;
  return document.migrationDigestSha256 === crypto.createHash("sha256").update(canonical(core)).digest("hex")
    && document.oldDenominator === 35 && document.newDenominator === 36
    && document.retainedAssertionCount === 35 && document.addedAssertionIds.length === 1
    && document.removedAssertionIds.length === 0 && document.denominatorCollapseNegativeTest === true
    && document.scoreImprovementClaimed === false;
};
...
check("mode-migration:baseline-35-to-58", validateModeMigration(modeMigration) && validateMechanismMigration(mechanismMigration));
for (const [id, mutate, resign = true] of [
  ...
  ["mode-migration:duplicate-addition-rejected", (row) => { row.addedPaths[22] = row.addedPaths[0]; }],
  ["mode-migration:wildcard-addition-rejected", (row) => { row.addedPaths[22] = "scripts/pass36/*"; }],
...
collapsedMechanismMigration.newDenominator = 35;
...
function receipt(spec) {
  return {
    schemaVersion: `fixture.${spec.id}.v1`, revisionId: REV,
    status: spec.statuses[0], passed: true, verified: spec.id === "dual_control",
    sourceBinding,
    ...(spec.id === "clean_unpack" ? { sourceImmutable: true, a58LiteralFirstChild: true } : {}),
...
try {
  ...
  fs.writeFileSync(finalVerification, `${JSON.stringify({
    ...
    revisionId: REV,
    status: "PASS_A102R41_FINAL_ACTION_REQUIRED_PACKAGES_NO_PROMOTION",
...
const result = {
  schemaVersion: "velmere.pass36.a102r41.a80r1-two-phase-release-controller-test.v1",
  revisionId: REV,
  status: failed.length === 0 ? "PASS_A102R41_A80R1_TWO_PHASE_CONTROLLER_MECHANISM_NO_PROMOTION" : "FAIL_A102R41_A80R1_TWO_PHASE_CONTROLLER_MECHANISM",
  required: 36, executed: checks.length, passed: checks.length - failed.length, failed: failed.length,
  denominatorMigration: { old: 35, new: 36, removedAssertions: 0 },
  globalDecision: "NO_GO", live: false, saleEnabled: false,
  productionApproved: false, worldClassProven: false,
  failures: failed,
};
