...
} from "./a102r6-source-boundary.mjs";
...
if (state.revisionId !== REV || state.parentRevisionId !== PARENT) {
  throw new Error("a102r6_state_identity");
}
if (parent.revisionId !== PARENT || !parent.manifestDigestSha256) {
  throw new Error("a102r6_parent_manifest_identity");
}
if (receipt.revisionId !== REV || receipt.status !== "PASS_A102R6_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION") {
  throw new Error("a102r6_local_regression_receipt_identity");
}
const inventory = collect(root);
if (inventory.rejected.length) {
  throw new Error(`a102r6_rejected:${JSON.stringify(inventory.rejected)}`);
}
...
const core = {
  schemaVersion:
    "velmere.pass36.a102r6.action-required-current-root-descendant-manifest.v1",
  revisionId: REV,
  ...
  completedThrough: 89,
  coveredWorkRange:
    "A90-A102_LOCAL_PRIVATE_ACCOUNT_BROWSER_STATE_CLIENT_FALLBACK_DURABLE_VAULT_PRIVACY_AND_VERIFIER_HARDENING_NO_FORMAL_PASS_CREDIT",
...
console.log(JSON.stringify({
  status: "BUILT_A102R6_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
