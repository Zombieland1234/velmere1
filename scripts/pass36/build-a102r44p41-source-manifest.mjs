...
const PARENT_SOURCE="VELMERE_PASS36_A102R44P39_ACTION_REQUIRED_AUDIT_PACKET_PARITY_BYTECODE_PROXY_BINDING_AND_EXTERNAL_ACCURACY_REGISTRY_TEST_CYCLE_2_OF_3_NO_LIVE_CREDIT";
const PARENT_CHECKPOINT="VELMERE_PASS36_A102R44P40_ACTION_REQUIRED_FULL_QA_OF_FROZEN_R44P39_SOURCE_TEST_CYCLE_3_OF_3_NO_LIVE_CREDIT";
const root=path.resolve(process.argv[2]??process.cwd());
const manifestRelative="_velmere/PASS36_A102R44P41_SOURCE_ONLY_MANIFEST.json";
...
const manifest={schemaVersion:"velmere.pass36.a102r44p41.source-manifest.v1",revisionId:REV,parentSourceRevisionId:PARENT_SOURCE,parentCheckpointRevisionId:PARENT_CHECKPOINT,pathOrderAlgorithm:"UTF8_BYTEWISE_ASCENDING_V1",fileCount:files.length,payloadBytes:files.reduce((s,r)=>s+r.byteLength,0),sourceAggregateSha256,pathSetSha256,files,excludedGenerated:["node_modules",".next*",".turbo",".cache","coverage","test-results","playwright-report","out","cache","failures"]};
fs.mkdirSync(path.dirname(manifestPath),{recursive:true});fs.writeFileSync(manifestPath,`${JSON.stringify(manifest,null,2)}\n`);
console.log(JSON.stringify({status:"BUILT_R44P41_SOURCE_MANIFEST",manifestPath:manifestRelative,fileCount:manifest.fileCount,payloadBytes:manifest.payloadBytes,sourceAggregateSha256,pathSetSha256},null,2));
