...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "verify-a89-clean-unpack-sequence.py");
const MAX_OUTPUT_BYTES = 16 * 1024 * 1024;
const BRIDGE_TIMEOUT_MS = 40 * 60 * 1000;
...
async function main() {
  if (process.argv.includes("--bridge-smoke")) {
    const result = await runBounded(process.execPath, ["-e", "process.stdout.write(JSON.stringify({status:'PASS_A89_ASYNC_BRIDGE_SMOKE'}));"], 10_000);
    const ok = result.code === 0 && !result.error && !result.overflow;
    console.log(`\n${JSON.stringify({ status: ok ? "PASS_A89_ASYNC_BRIDGE_PROCESS_EXIT" : "FAIL_A89_ASYNC_BRIDGE_PROCESS_EXIT", ...result, error: result.error?.message ?? null }, null, 2)}`);
    return ok ? 0 : 1;
  }
  ...
  for (const candidate of candidates) {
    ...
    if (result.error || result.overflow || result.signal || result.code !== 0) {
      console.error(JSON.stringify({
        status: "FAIL_A89_CLEAN_UNPACK_PYTHON_BRIDGE",
        command: candidate,
        code: result.code,
  ...
  console.error(JSON.stringify({ status: "FAIL_A89_CLEAN_UNPACK_PYTHON_BRIDGE", error: lastError?.message ?? "python_runtime_not_found" }, null, 2));
  return 1;
}
