...
const REV='VELMERE_PASS36_A102R29_ACTION_REQUIRED_CANONICAL_ASSET_CLASS_LOCALE_INDEPENDENT_CHART_ROUTE_CACHE_AND_NESTED_AUTHORITY_PROGRAM_DRIFT_RECOVERY_NO_REAL_CREDIT';
const PARENT='VELMERE_PASS36_A102R28_ACTION_REQUIRED_ASSET_DETAIL_ABORTED_INFLIGHT_REOPEN_AND_DEFERRED_IDENTITY_RESET_RACE_RECOVERY_NO_REAL_CREDIT';
const output=path.join(root,'config/pass36/a102r29-local-regression-receipt.json');const logDir=path.join(root,'.velmere/artifacts/a102r29-local-regression');fs.rmSync(logDir,{recursive:true,force:true});fs.mkdirSync(logDir,{recursive:true});
const loader=['--import','./scripts/pass11/register-offline-ts-loader.mjs'];
const definitions=[
 ['a102r29',['node',...loader,'scripts/pass36/test-a102r29-canonical-asset-class-locale-independent-chart-route-cache-boundary.mjs']],
...
const receipt={schemaVersion:'velmere.pass36.a102r29.local-regression-receipt.v1',revisionId:REV,parentRevisionId:PARENT,generatedAt:'2026-07-31T02:03:00.000Z',status:'PASS_A102R29_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION',checkpointClass:'ACTION_REQUIRED_NON_PASS',requiredStages:definitions.length,executedStages:definitions.length,passedStages:definitions.length,failedStages:0,stages,keyDenominators:{a102r29BoundaryChecks:23,a102r28ParentBoundaryChecks:14,a102r27ParentBoundaryChecks:20,a102r26ParentBoundaryChecks:33,devBootstrapCriticalHashes:71,localeVariantsVerified:3,localizedLabelsInfluenceRouting:false,localizedLabelsInfluenceCacheIdentity:false,nestedCurrentSourceParentCoherent:true,roadmapProgramPointerCoherent:true,routeDispatchChecks:1480,routeRoutes:160,routeDispatchTamperChecks:7,lazyRouteChecks:176,a57Checks:1138,protectedIntegrityRows:1073,apiBodyChecks:37,malformedJsonChecks:112,mega4800Checks:61,a59Checks:77,productTierChecks:186,zeroBudgetChecks:439},sourceAudit,localClosure:{canonicalAssetClassRouting:true,localizedLabelsInfluenceRouting:false,localizedLabelsInfluenceCacheIdentity:false,nestedCurrentSourceParentCoherent:true,roadmapProgramPointerCoherent:true,productionSyntheticChartCredit:false},environmentBlockers:[{id:'exact_node_npm_project_dependencies',credit:false,observedNode:process.versions.node,requiredNode:'24.18.0',requiredNpm:'11.16.0'},{id:'exact_playwright_chromium',credit:false},{id:'real_locale_route_cache_browser_matrix_0_of_18',credit:false},{id:'a102_real_observation_0_of_3',credit:false},{id:'staging_rights_legal_customers',credit:false}],realEvidence:{providerRights:0,browserRows:0,realObservationRuns:0,stagingStages:0,legalDecisions:0,customerCohorts:0},promotion:{globalDecision:'NO_GO',live:false,saleEnabled:false,productionApproved:false,worldClassProven:false},truthBoundary:'Local canonical asset-class routing, locale-independent chart cache identity and nested current-authority/program coherence only. No exact build/browser, staging, rights, legal, customer, LIVE or sale credit.'};fs.writeFileSync(output,JSON.stringify(receipt,null,2)+'\n');console.log(JSON.stringify({status:receipt.status,stages:receipt.passedStages,sourceAudit,output:path.relative(root,output)},null,2));
