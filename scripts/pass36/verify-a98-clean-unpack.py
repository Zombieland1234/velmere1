...
REV = "VELMERE_PASS36_A98R0_EMAIL_STORAGE_KMS_ORIGIN_CONTEXT_CLEANUP_AND_DELIVERY_TRUTH_BOUNDARY"
MANIFEST_REL = "_velmere/PASS36_A98R0_SOURCE_ONLY_MANIFEST.json"
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA98PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def run_step(root: pathlib.Path, receipt_dir: pathlib.Path, index: int, step: dict):
    start=time.monotonic(); env={'PATH':os.environ.get('PATH',''),'LANG':'C.UTF-8','LC_ALL':'C.UTF-8','TZ':'UTC','CI':'1','NO_COLOR':'1','NODE_ENV':'test','VELMERE_A97_CLEAN_UNPACK':'1','VELMERE_A95_NO_WRITE':'1'}
...
def main():
    ...
    steps=[
      ...
      {'id':'a98_authority_descendant','cmd':['node','scripts/pass36/verify-a98-action-required-authority.mjs'],'expectedStatus':'PASS_A98_ACTION_REQUIRED_AUTHORITY_NO_STAGING_CREDIT','timeout':360},
      {'id':'a98_email_storage_kms_boundary','cmd':['node','scripts/pass36/verify-a98-email-storage-kms-boundaries.mjs'],'expectedStatus':'PASS_A98_EMAIL_STORAGE_KMS_BOUNDARY_LOCAL_ONLY','timeout':600},
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A98R0_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a98.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
    ...
    out=receipt_dir/'PASS36_A98R0_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
