...
import { REV, PARENT, MANIFEST, STATE, PROGRAM } from "./a102r5-source-boundary.mjs";
...
const plane=authority.planes?.a102r5LocalClosure;
add("authority:local-plane",plane?.revisionId===REV&&plane?.parentRevisionId===PARENT&&plane?.browserBearerTokenPersistenceRemoved===true&&plane?.browserPaidHeaderAuthority===false&&plane?.serverAccountSessionRequired===true&&plane?.exactAccountContextEntitlementBinding===true&&plane?.browserBearerTokenIssued===false&&plane?.exactBuildBrowserCredit===false&&plane?.realCredit===false,plane);
...
add("mirror",mirror.sourceRevisionId===REV&&mirror.parentSourceRevisionId===PARENT&&mirror.currentRootDescendantManifestPath===MANIFEST&&mirror.a102r5BrowserBearerTokenPersistenceRemoved===true&&mirror.a102r5BrowserPaidHeaderAuthority===false&&mirror.a102r5ServerAccountEntitlementRequired===true&&mirror.a102r5RealObservationRuns===0&&mirror.a102r5ExactReleaseCredit===false);
...
add("state",state.revisionId===REV&&state.parentRevisionId===PARENT&&state.completedThrough===89&&state.localImplementation?.browserPaidBearerTokenPersistenceRemoved===true&&state.localImplementation?.browserPaidAccessHeaderAuthorityRemoved===true&&state.localImplementation?.serverAccountSessionAndEntitlementLedgerRequiredForPaidDepth===true&&state.runtimeTruth?.freshWebpackBuildOnA102R5BytesExecuted===false&&state.runtimeTruth?.freshExactBrowserRowsExecuted===0&&state.promotion?.globalDecision==="NO_GO");
add("program",program.revisionId===REV&&program.parentRevisionId===PARENT&&program.formalRemainingEntries===31&&program.passes?.map((r)=>r.id).join(",")==="A102,A103,A104,A105,A106,A107,A108,A109,A110,A111,A112,A113,A114,A115,A116"&&program.passes?.every((r)=>r.credit===false)&&program.closureTracks?.every((r)=>r.credit===false)&&["A102R5-GAP-01","A102R5-GAP-11"].every((id)=>program.newRoadmapGaps?.some((r)=>r.id===id)));
...
add("a58",a58.currentCheckpointRevisionId===REV&&a58.currentCheckpointParentRevisionId===PARENT&&a58.currentDescendantManifestPath===MANIFEST&&a58.currentAuthorityVerifierPath==="scripts/pass36/verify-a102r5-action-required-authority.mjs"&&a58.archiveManifestPath==="_velmere/PASS36_A102R5_SOURCE_ONLY_MANIFEST.json"&&a58.currentAuthorityVerifierExpectedStatus==="PASS_A102R5_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT");
...
run("descendant",["scripts/pass36/verify-a102r5-current-root-descendant.mjs"],"PASS_A102R5_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION");
run("receipt",["scripts/pass36/verify-a102r5-local-regression-receipt.mjs"],"PASS_A102R5_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION");
