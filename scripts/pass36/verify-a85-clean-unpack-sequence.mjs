...
const root=process.cwd(),REV="VELMERE_PASS36_A85R0_SHIELD_PRO_AND_SHIELD_MAP_FULL_DEPTH_IDENTITY_ENTITLEMENT_MATRIX";
if(!fs.existsSync(path.join(root,"_velmere/PASS35_SOURCE_ONLY_MANIFEST.json")))throw new Error("a85_clean_unpack_package_manifest_required");
const commands=[
...
const results=[];for(const command of commands){const run=spawnSync(process.execPath,command.args,{cwd:root,encoding:"utf8",maxBuffer:512*1024*1024,timeout:300000,env:{...process.env,VELMERE_A85_CLEAN_UNPACK_SEQUENCE:"1"}});let parsed=null;try{parsed=JSON.parse(run.stdout)}catch (ignoredError) { void ignoredError; }let passed=run.status===0&&!run.signal&&!run.error;if(command.parseA58)passed=passed&&parsed?.status==="PASS_RELEASE_INTEGRITY_NO_PROMOTION"&&parsed?.summary?.blockingFailed===0&&parsed?.historicalArtifactRecoveryComplete===false;results.push({id:command.id,passed,exitCode:run.status,signal:run.signal,timedOut:run.error?.code==="ETIMEDOUT",stdoutBytes:Buffer.byteLength(run.stdout??""),stderrBytes:Buffer.byteLength(run.stderr??""),status:parsed?.status??null,failureTail:passed?null:`${run.stderr??""}\n${run.stdout??""}`.slice(-5000)});if(!passed)break;}
const failures=results.filter(r=>!r.passed);const report={schemaVersion:"velmere.pass36.a85.clean-unpack-sequence.v1",revisionId:REV,status:failures.length?"FAIL_A85_CLEAN_UNPACK_SEQUENCE":"PASS_A85_CLEAN_UNPACK_SEQUENCE_NO_PROMOTION",requiredOrder:["A58_EXACT_PATH_SET_BEFORE_ANY_RECEIPT_GENERATING_TEST","A85_AND_RETAINED_REGRESSIONS_AFTER_A58"],checks:results.length,passed:results.length-failures.length,failed:failures.length,results,historicalArtifactsRecovered:false,exactBuildBrowserExecuted:false,realFullDepthCasesVerified:0,rightsApprovedAssets:0,productionBrowserAssets:0,realEntitlementsVerified:0,customerValueLabeledAssets:0,stagingProven:false,liveProven:false,saleEnabled:false,truthBoundary:"This sequence verifies an unpacked SOURCE_ONLY package in safe order. A58 runs first. It does not execute current providers, real account entitlements, rights, production browsers, customers, staging, LIVE or sale."};console.log(JSON.stringify(report,null,2));if(failures.length)process.exit(1);
