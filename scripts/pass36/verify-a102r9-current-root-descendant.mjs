...
add("claims:no-promotion", manifest.claims?.a102PassCredit === false
  ...
  && manifest.claims?.systemClipboardRedactedSummaryOnly === true
  && manifest.claims?.privateAccountIdentifiersCopiedToSystemClipboard === false
  && manifest.claims?.downloadSessionOrAccessAuthorityCopiedToSystemClipboard === false
  && manifest.claims?.adminOperationalIdentifiersCopiedToSystemClipboard === false
  && manifest.claims?.realSystemClipboardBrowserRows === 0
...
console.log(JSON.stringify({
  decision: failed.length ? "FAIL" : "PASS_A102R9_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
