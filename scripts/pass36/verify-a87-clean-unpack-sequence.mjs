...
if (!fs.existsSync(path.join(root, "_velmere/PASS35_SOURCE_ONLY_MANIFEST.json"))) throw new Error("a87_clean_unpack_package_manifest_required");
const commands = [
  ...
  { id: "a87_descendant", args: ["scripts/pass36/verify-a87-current-root-descendant.mjs"] },
  { id: "a87_matrix", args: ["--import", "./scripts/pass11/register-offline-ts-loader.mjs", "scripts/pass36/verify-a87-market-impact-whale-watch-matrix.ts"] },
...
for (const command of commands) {
  const run = spawnSync(process.execPath, command.args, { cwd: root, encoding: "utf8", maxBuffer: 512 * 1024 * 1024, timeout: 300000, env: { ...process.env, TERM: process.env.TERM || "xterm-256color", VELMERE_A87_CLEAN_UNPACK_SEQUENCE: "1" } });
  let parsed = null;
...
const report = {
  schemaVersion: "velmere.pass36.a87.clean-unpack-sequence.v1",
  revisionId: REV,
  status: failures.length ? "FAIL_A87_CLEAN_UNPACK_SEQUENCE" : "PASS_A87_CLEAN_UNPACK_SEQUENCE_NO_PROMOTION",
  requiredOrder: ["A58_EXACT_PATH_SET_BEFORE_ANY_RECEIPT_GENERATING_TEST", "A87_AND_RETAINED_REGRESSIONS_AFTER_A58"],
  ...
  liveProven: false,
  saleEnabled: false,
  truthBoundary: "This sequence verifies an unpacked SOURCE_ONLY package in safe order. A58 runs first. It does not execute current multi-venue books, realized fills, holder/transfer/label providers, provider rights, production browsers, customers, exact A80, staging, LIVE or sale.",
};
