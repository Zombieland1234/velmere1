...
const packageManifest = path.join(root, "_velmere/PASS35_SOURCE_ONLY_MANIFEST.json");
if (!fs.existsSync(packageManifest)) throw new Error("a81_clean_unpack_package_manifest_required");
...
for (const command of commands) {
  const run = spawnSync(process.execPath, command.args, {
    ...
    timeout: 120_000,
    env: { ...process.env, VELMERE_A81_CLEAN_UNPACK_SEQUENCE: "1" },
  });
  let parsed = null;
  ...
  results.push({
    ...
    failureTail: passed ? null : `${run.stderr ?? ""}\n${run.stdout ?? ""}`.slice(-4000),
  });
  if (!passed) break;
}
...
const report = {
  schemaVersion: "velmere.pass36.a81.clean-unpack-sequence.v1",
  revisionId: A81_REVISION,
  status: failures.length === 0 ? "PASS_A81_CLEAN_UNPACK_SEQUENCE_NO_PROMOTION" : "FAIL_A81_CLEAN_UNPACK_SEQUENCE",
  requiredOrder: ["A58_EXACT_PATH_SET_BEFORE_ANY_RECEIPT_GENERATING_TEST", "REMAINING_REGRESSIONS_AFTER_A58"],
  ...
  liveProven: false,
  saleEnabled: false,
  truthBoundary: "This sequence verifies an unpacked SOURCE_ONLY package in a safe order. A58 exact path-set runs before tests that may create deterministic receipts. It does not recover A61 bytes, install dependencies, execute exact Node/Chromium, prove staging, LIVE, sale or world-class status.",
};
