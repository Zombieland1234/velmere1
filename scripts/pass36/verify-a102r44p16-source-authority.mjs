...
const REVISION = "VELMERE_PASS36_A102R44P16_ACTION_REQUIRED_SINGLE_SKU_TRUTH_ANALYSIS_QUEUE_AND_CUSTOMER_CLAIM_CLOSURE_NO_LIVE_CREDIT";
const PARENT = "VELMERE_PASS36_A102R44P15_ACTION_REQUIRED_REENTRANCY_RECALL_SOURCE_AUDIT_DECLARATION_BOUNDARY_AND_FINAL_BYTE_REGRESSION_NO_LIVE_CREDIT";
const MANIFEST_REL = "_velmere/PASS36_A102R44P16_SOURCE_ONLY_MANIFEST.json";
...
check("schema",manifest.schemaVersion==="velmere.pass36.a102r44p16.source-manifest.v1");
...
const expected=new Map(manifest.entries.map((row)=>[row.path,row])); const current=new Map(rows.map((row)=>[row.path,row]));
...
const changed=rows.filter((row)=>{const old=expected.get(row.path);return old&&(old.byteLength!==row.byteLength||old.sha256!==row.sha256||old.mode!==row.mode);}).map((row)=>row.path);
check("missing-zero",missing.length===0,missing.slice(0,20)); check("extra-zero",extra.length===0,extra.slice(0,20)); check("changed-zero",changed.length===0,changed.slice(0,20));
...
check("approved-changes-bound",manifest.approvedChangesPath==="config/pass36/a102r44p16-approved-current-source-changes.json"&&/^[0-9a-f]{64}$/u.test(manifest.approvedChangesSha256));
check("no-forbidden-generated-paths",rows.every((row)=>!row.path.split("/").some((part)=>forbiddenParts.has(part)||part.startsWith(".next"))));
const failed=checks.filter((row)=>!row.ok);const result={schemaVersion:"velmere.pass36.a102r44p16.source-authority-receipt.v1",status:failed.length?"FAIL":"PASS_R44P16_SOURCE_AUTHORITY",checks:checks.length,passed:checks.length-failed.length,failed:failed.length,revisionId:REVISION,manifestSha256:sha(fs.readFileSync(path.join(ROOT,MANIFEST_REL))),aggregateSha256,fileCount:rows.length,sourceImmutable:true,rows:checks};
