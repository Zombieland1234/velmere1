...
const output=path.join(root,'config/pass36/a102r35-local-regression-receipt.json');const logDir=path.join(root,'.velmere/artifacts/a102r35-local-regression');fs.rmSync(logDir,{recursive:true,force:true});fs.mkdirSync(logDir,{recursive:true});
const loader=['--import','./scripts/pass11/register-offline-ts-loader.mjs'];
const definitions=[
 ['a102r35',['node',...loader,'scripts/pass36/test-a102r35-modal-scroll-lock-rapid-reopen-restore-race.mjs']],
 ['a102r34',['node',...loader,'scripts/pass36/test-a102r34-dialog-stack-top-layer-ownership-boundary.mjs']],
 ['a102r33-parent',['node',...loader,'scripts/pass36/test-a102r33-dialog-focus-return-race-boundary.mjs']],
 ['a102r32-parent',['node',...loader,'scripts/pass36/test-a102r32-route-hash-timeout-prefetch-reentry-boundary.mjs']],
...
let sourceAudit={filesRead:0,codeFiles:0,syntaxErrors:0,missingLocalImports:0,missingCssModuleClasses:0};const auditText=fs.readFileSync(path.join(logDir,`${String(definitions.length).padStart(2,'0')}-source-audit.stdout.txt`),'utf8');const m=auditText.match(/\{[\s\S]*\}\s*$/u);if(m){const p=JSON.parse(m[0]);sourceAudit={filesRead:p.filesRead??p.files??0,codeFiles:p.codeFiles??0,syntaxErrors:p.syntaxErrors??0,missingLocalImports:p.missingLocalImports??0,missingCssModuleClasses:p.missingCssModuleClasses??0};}
const receipt={schemaVersion:'velmere.pass36.a102r35.local-regression-receipt.v1',revisionId:REV,parentRevisionId:PARENT,generatedAt:'2026-07-31T07:45:00.000Z',status:'PASS_A102R35_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION',checkpointClass:'ACTION_REQUIRED_NON_PASS',requiredStages:definitions.length,executedStages:definitions.length,passedStages:definitions.length,failedStages:0,stages,keyDenominators:{a102r35BoundaryChecks:24,a102r34ParentBoundaryChecks:24,a102r33ParentBoundaryChecks:24,devBootstrapCriticalHashes:76,modalScrollRestoreGenerationBound:true,rapidReopenConsumesPendingCoordinates:true,routeDispatchChecks:1480,routeRoutes:160,routeDispatchTamperChecks:7,lazyRouteChecks:176,a57Checks:1138,protectedIntegrityRows:1073,apiBodyChecks:37,malformedJsonChecks:112,mega4800Checks:61,a59Checks:77,productTierChecks:186,zeroBudgetChecks:439},sourceAudit,localClosure:{generationBoundRestore:true,rapidReopenConsumesPendingCoordinates:true,staleFrameCannotScroll:true,activeLockBlocksRestore:true,referenceCountedNestedOwnershipRetained:true,productionPromotionCredit:false},environmentBlockers:[{id:'exact_node_npm_project_dependencies',credit:false,observedNode:process.versions.node,requiredNode:'24.18.0',requiredNpm:'11.16.0'},{id:'exact_playwright_chromium',credit:false},{id:'real_modal_scroll_lock_browser_matrix_0_of_18',credit:false},{id:'a102_real_observation_0_of_3',credit:false},{id:'staging_rights_legal_customers',credit:false}],realEvidence:{providerRights:0,browserRows:0,realObservationRuns:0,stagingStages:0,legalDecisions:0,customerCohorts:0},promotion:{globalDecision:'NO_GO',live:false,saleEnabled:false,productionApproved:false,worldClassProven:false},truthBoundary:'Local generation-bound modal scroll restore, rapid-reopen coordinate continuity, stale frame cancellation and nested owner reference counting only. No exact build/browser, staging, rights, legal, customer, LIVE or sale credit.'};fs.writeFileSync(output,JSON.stringify(receipt,null,2)+'\n');console.log(JSON.stringify({status:receipt.status,stages:receipt.passedStages,sourceAudit,output:path.relative(root,output)},null,2));
