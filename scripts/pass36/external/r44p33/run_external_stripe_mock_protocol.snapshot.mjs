...
const MOCK_API_KEY = 'sk_test_123';
const AMOUNT = 4900;
const CURRENCY = 'eur';
...
try {
  ...
  started = now();
  addCase('refund-schema', refund?.object === 'refund' && String(refund?.id ?? '').startsWith('re_') && refund?.amount === AMOUNT, { object: refund?.object ?? null, idSha256: sha256Text(refund?.id ?? ''), livemode: refund?.livemode ?? null, amount: refund?.amount ?? null, currency: refund?.currency ?? null, apiBaseClass: 'STRIPE_MOCK_OPENAPI' }, started);
