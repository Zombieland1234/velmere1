...
MANIFEST_REL = "_velmere/PASS36_A97R0_SOURCE_ONLY_MANIFEST.json"
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA97PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def main():
    ...
    steps=[
      ...
      {'id':'a97_authority_descendant','cmd':['node','scripts/pass36/verify-a97-action-required-authority.mjs'],'expectedStatus':'PASS_A97_ACTION_REQUIRED_AUTHORITY_NO_STRIPE_STAGING_CREDIT','timeout':360},
      {'id':'a97_stripe_runtime_receipt_control','cmd':['node','scripts/pass36/verify-a97-stripe-payment-boundaries.mjs'],'timeout':360},
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A97R0_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a97r0.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
    ...
    out=receipt_dir/'PASS36_A97R0_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
