...
import { collectA102R40Inventory } from "./package-a102r40-deterministic.mjs";
...
const ledger=JSON.parse(fs.readFileSync("config/pass36/a102r40-approved-current-source-changes.json","utf8"));
const parent=JSON.parse(fs.readFileSync("config/pass36/a102r40-parent-source-package-manifest.json","utf8"));
let checks=0;const ok=(v,id)=>{checks++;assert.ok(v,id)};
ok(ledger.schemaVersion==="velmere.pass36.a102r40.approved-current-source-changes.v1","schema");
ok(ledger.revisionId===REV&&ledger.parentRevisionId===PARENT,"identity");
ok(ledger.parentSourceArchiveSha256==="a855164ce042c9b0c9fe8a765eabbe06e165bc13f6dccb5db08f821e11be2995","parent-archive");
...
const excluded=new Set(ledger.selfExcludedPaths);ok(excluded.has("config/pass36/a102r40-approved-current-source-changes.json")&&excluded.has("config/pass36/a102r40-current-root-descendant-manifest.json")&&excluded.has("config/pass36/a102r40-local-regression-receipt.json"),"exclusions");
const parentMap=new Map(parent.entries.map((x)=>[x.path,x]));
const {rows}=collectA102R40Inventory(process.cwd(),"source");const currentMap=new Map(rows.map(({content,...x})=>[x.path,x]));
const observed=[];
...
console.log(JSON.stringify({status:"PASS_A102R40_APPROVED_SOURCE_MODE_CHANGES_COMPLETE_NO_PROMOTION",checksPassed:checks,checksFailed:0,fileCount:ledger.fileCount,addedFiles:ledger.addedFiles,modifiedFiles:ledger.modifiedFiles},null,2));
