...
REV = "VELMERE_PASS36_A101R0_MEASURED_SLO_ERROR_BUDGET_VENDOR_EXIT_AND_RECOVERY_TRUTH_BOUNDARY"
MANIFEST_REL = "_velmere/PASS36_A101R0_SOURCE_ONLY_MANIFEST.json"
...
def source_excluded(rel: str) -> bool:
    ...
    segments=rel.split('/'); base=rel.rsplit('/',1)[-1]
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA101PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def run_step(root: pathlib.Path, receipt_dir: pathlib.Path, index: int, step: dict):
    start=time.monotonic(); env={'PATH':os.environ.get('PATH',''),'LANG':'C.UTF-8','LC_ALL':'C.UTF-8','TZ':'UTC','CI':'1','NO_COLOR':'1','NODE_ENV':'test','VELMERE_A101_CLEAN_UNPACK':'1','VELMERE_A95_NO_WRITE':'1'}
    ...
    result['parsedStatus']=(parsed.get('status') or parsed.get('decision')) if isinstance(parsed,dict) else None
...
def main():
    ...
    steps=[
      ...
      {'id':'a101_authority_descendant','cmd':['node','scripts/pass36/verify-a101-action-required-authority.mjs'],'expectedStatus':'PASS_A101_ACTION_REQUIRED_AUTHORITY_NO_STAGING_CREDIT','timeout':420},
      {'id':'a101_slo_vendor_exit_boundary','cmd':['node','scripts/pass36/verify-a101-slo-vendor-exit-recovery-boundaries.mjs'],'expectedStatus':'PASS_A101_SLO_ERROR_BUDGET_VENDOR_EXIT_RECOVERY_BOUNDARY_LOCAL_ONLY','timeout':600},
      {'id':'a100_incident_boundary_historical','cmd':['node','scripts/pass36/verify-a100-incident-kill-switch-customer-communication-boundaries.mjs'],'expectedStatus':'PASS_A100_INCIDENT_KILL_SWITCH_CUSTOMER_COMMUNICATION_BOUNDARY_LOCAL_ONLY','timeout':600},
      {'id':'a100_historical_descendant','cmd':['node','scripts/pass36/verify-a100-current-root-descendant.mjs'],'expectedStatus':'PASS_A100_HISTORICAL_DESCENDANT_BOUND_TO_CURRENT_NO_PROMOTION','timeout':420},
      {'id':'a99_backup_restore_provider_loss_historical','cmd':['node','scripts/pass36/verify-a99-backup-restore-provider-loss-boundaries.mjs'],'expectedStatus':'PASS_A99_BACKUP_RESTORE_PROVIDER_LOSS_BOUNDARY_LOCAL_ONLY','timeout':600},
      {'id':'a99_historical_descendant','cmd':['node','scripts/pass36/verify-a99-current-root-descendant.mjs'],'expectedStatus':'PASS_A99_HISTORICAL_DESCENDANT_BOUND_TO_CURRENT_NO_PROMOTION','timeout':420},
      {'id':'a98_email_storage_kms_historical','cmd':['node','scripts/pass36/verify-a98-email-storage-kms-boundaries.mjs'],'expectedStatus':'PASS_A98_EMAIL_STORAGE_KMS_BOUNDARY_LOCAL_ONLY','timeout':420},
      {'id':'a97r1_payment_operator_assertion_historical','cmd':['node','scripts/pass36/verify-a97r1-payment-operator-assertions.mjs'],'timeout':420},
      {'id':'a96_rls_local_implementation_historical','cmd':['node','scripts/pass36/verify-a96-rls-tenant-isolation.mjs'],'timeout':420},
      {'id':'a95_staging_subject_admission_historical_precondition','cmd':['node','scripts/pass36/verify-a95-staging-subject-admission.mjs'],'timeout':300},
      {'id':'route_ast_static_replay','cmd':['node','scripts/pass15/verify-route-export-ast-registry.mjs'],'expectedStatus':'PASS_ROUTE_EXPORT_AST_REGISTRY_STATIC_REPLAY_NO_REPARSE_CREDIT','timeout':240},
      {'id':'route_dispatch_full_denominator','cmd':['node','scripts/pass15/verify-route-dispatch-consolidation.mjs','--output',str(receipt_dir/'route-dispatch-verification.json')],'timeout':300},
      {'id':'lazy_route_shells','cmd':['node','scripts/pass15/verify-lazy-route-shells.mjs'],'timeout':240},
      {'id':'cross_surface_value_verifier','cmd':['node','scripts/pass36/verify-a94r2-cross-surface-value-truth.mjs'],'expectedStatus':'PASS_LOCAL_CROSS_SURFACE_VALUE_TRUTH_NO_PAID_OR_REAL_CREDIT','timeout':240},
      {'id':'retained_pdf_summary','cmd':['node','scripts/pass36/verify-a94r2-retained-pdf-summary.mjs'],'expectedStatus':'PASS_A94R2_RETAINED_PDF_SUMMARY_SYNTHETIC_ONLY_NO_SALE_CREDIT','timeout':180},
      {'id':'product_tier_contract','cmd':['node','scripts/pass35/test-product-tier-content-contract.mjs'],'timeout':300},
      {'id':'zero_budget_contract','cmd':['node','scripts/pass35/test-zero-budget-functional-roadmap.mjs'],'timeout':300},
      {'id':'source_integrity_audit','cmd':['node','scripts/a44-source-integrity-audit.mjs'],'timeout':700},
    ]
    results=[]
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A101R0_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a101.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
      ...
      {'gate':'real_a95_a101_staging_lifecycles','status':'0_OF_REQUIRED_REAL_STAGES'},
      {'gate':'real_slo_vendor_exit_recovery','status':'0_OF_1_VERIFIED'},
    ...
    out=receipt_dir/'PASS36_A101R0_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
