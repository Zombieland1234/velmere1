...
const root=process.cwd(); const REV="VELMERE_PASS36_A83R0_BROWSER_LENS_PDF_REAL_PACKET_MATRIX_AND_SECURE_DELIVERY_PARITY"; const packageManifest=path.join(root,"_velmere/PASS35_SOURCE_ONLY_MANIFEST.json"); if(!fs.existsSync(packageManifest))throw new Error("a83_clean_unpack_package_manifest_required");
const commands=[
...
const results=[]; for(const command of commands){const run=spawnSync(process.execPath,command.args,{cwd:root,encoding:"utf8",maxBuffer:512*1024*1024,timeout:300000,env:{...process.env,VELMERE_A83_CLEAN_UNPACK_SEQUENCE:"1"}});let parsed=null;try{parsed=JSON.parse(run.stdout)}catch (ignoredError) { void ignoredError; }let passed=run.status===0&&!run.signal&&!run.error;if(command.parseA58)passed=passed&&parsed?.status==="PASS_RELEASE_INTEGRITY_NO_PROMOTION"&&parsed?.summary?.blockingFailed===0&&parsed?.historicalArtifactRecoveryComplete===false;results.push({id:command.id,passed,exitCode:run.status,signal:run.signal,timedOut:run.error?.code==="ETIMEDOUT",stdoutBytes:Buffer.byteLength(run.stdout??""),stderrBytes:Buffer.byteLength(run.stderr??""),status:parsed?.status??null,failureTail:passed?null:`${run.stderr??""}\n${run.stdout??""}`.slice(-4000)});if(!passed)break;}
const failures=results.filter(r=>!r.passed); const report={schemaVersion:"velmere.pass36.a83.clean-unpack-sequence.v1",revisionId:REV,status:failures.length?"FAIL_A83_CLEAN_UNPACK_SEQUENCE":"PASS_A83_CLEAN_UNPACK_SEQUENCE_NO_PROMOTION",requiredOrder:["A58_EXACT_PATH_SET_BEFORE_ANY_RECEIPT_GENERATING_TEST","A83_AND_RETAINED_REGRESSIONS_AFTER_A58"],checks:results.length,passed:results.length-failures.length,failed:failures.length,results,historicalArtifactsRecovered:false,exactBuildBrowserExecuted:false,realPacketsVerified:0,productionBrowserRuns:0,secureDeliveries:0,customerComprehensionLabels:0,stagingProven:false,liveProven:false,saleEnabled:false,truthBoundary:"This sequence verifies an unpacked SOURCE_ONLY package in safe order. A58 runs before receipt-generating tests. It does not recover A61, install exact dependencies, execute production browsers, verify current provider packets, prove secure customer delivery, customer value, staging, LIVE or sale."}; console.log(JSON.stringify(report,null,2)); if(failures.length)process.exit(1);
