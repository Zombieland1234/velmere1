...
import { REV, PARENT, MANIFEST, STATE, PROGRAM } from "./a102r7-source-boundary.mjs";
...
const plane=authority.planes?.a102r7LocalClosure;
add("authority:local-plane",plane?.revisionId===REV&&plane?.parentRevisionId===PARENT&&plane?.currentQueryForwardedToWalletProvider===false&&plane?.currentFragmentForwardedToWalletProvider===false&&plane?.privateDynamicPathForwardedToWalletProvider===false&&plane?.validatedOriginAndAllowlistedPublicPathOnly===true&&plane?.freshExactBuildBrowserCredit===false&&plane?.realMobileWalletBrowserCredit===false&&plane?.realCredit===false,plane);
...
add("mirror",mirror.sourceRevisionId===REV&&mirror.parentSourceRevisionId===PARENT&&mirror.currentRootDescendantManifestPath===MANIFEST&&mirror.a102r7CurrentQueryForwardedToWalletProvider===false&&mirror.a102r7CurrentFragmentForwardedToWalletProvider===false&&mirror.a102r7PrivateDynamicPathForwardedToWalletProvider===false&&mirror.a102r7ValidatedOriginAndAllowlistedPublicPathOnly===true&&mirror.a102r7RealMobileWalletBrowserRows===0&&mirror.a102r7ExactReleaseCredit===false);
add("legacy",legacy.notAuthoritativeCurrentSourcePointer===true&&legacy.authoritativeCurrentSourceRevisionId===REV&&legacy.authoritativeCurrentSourceParentRevisionId===PARENT&&legacy.realRepeatedObservationRuns===0&&legacy.a90ToA102PassCredit===false&&legacy.a102r7CurrentQueryForwardedToWalletProvider===false&&legacy.a102r7CurrentFragmentForwardedToWalletProvider===false&&legacy.a102r7PrivateDynamicPathForwardedToWalletProvider===false&&legacy.a102r7RealMobileWalletBrowserRows===0);
add("state",state.revisionId===REV&&state.parentRevisionId===PARENT&&state.completedThrough===89&&state.localImplementation?.mobileWalletDeeplinkCurrentQueryForwarded===false&&state.localImplementation?.mobileWalletDeeplinkCurrentFragmentForwarded===false&&state.localImplementation?.mobileWalletDeeplinkPrivateDynamicPathForwarded===false&&state.localImplementation?.mobileWalletDeeplinkValidatedOriginOnly===true&&state.localImplementation?.mobileWalletDeeplinkAllowlistedPublicPathOnly===true&&state.localImplementation?.unknownPendingMobileWalletMarkerRejected===true&&state.runtimeTruth?.freshWebpackBuildOnA102R7BytesExecuted===false&&state.runtimeTruth?.freshMobileWalletBrowserRowsExecuted===0&&state.promotion?.globalDecision==="NO_GO");
add("program",program.revisionId===REV&&program.parentRevisionId===PARENT&&program.formalRemainingEntries===31&&program.passes?.map((r)=>r.id).join(",")==="A102,A103,A104,A105,A106,A107,A108,A109,A110,A111,A112,A113,A114,A115,A116"&&program.passes?.every((r)=>r.credit===false)&&program.closureTracks?.every((r)=>r.credit===false)&&["A102R7-GAP-01","A102R7-GAP-10"].every((id)=>program.newRoadmapGaps?.some((r)=>r.id===id)));
...
add("readmes",readme.includes(`Current source revision: \`${REV}\``)&&cleanReadme.includes(`Current source revision: \`${REV}\``)&&readme.includes("mobile-wallet deeplink disclosure")&&cleanReadme.includes("mobile-wallet deeplink disclosure"));
add("a58",a58.currentCheckpointRevisionId===REV&&a58.currentCheckpointParentRevisionId===PARENT&&a58.currentDescendantManifestPath===MANIFEST&&a58.currentAuthorityVerifierPath==="scripts/pass36/verify-a102r7-action-required-authority.mjs"&&a58.archiveManifestPath==="_velmere/PASS36_A102R7_SOURCE_ONLY_MANIFEST.json"&&a58.currentAuthorityVerifierExpectedStatus==="PASS_A102R7_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT");
...
run("descendant",["scripts/pass36/verify-a102r7-current-root-descendant.mjs"],"PASS_A102R7_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION");
run("receipt",["scripts/pass36/verify-a102r7-local-regression-receipt.mjs"],"PASS_A102R7_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION");
run("wallet-privacy",["--import","./scripts/pass11/register-offline-ts-loader.mjs","scripts/pass36/test-a102r7-mobile-wallet-deeplink-privacy-boundary.ts"],"PASS_A102R7_MOBILE_WALLET_DEEPLINK_QUERY_HASH_PRIVATE_PATH_FAIL_CLOSED_NO_PROMOTION");
run("approved-changes",["scripts/pass36/verify-a102r7-approved-mobile-wallet-deeplink-privacy-changes.mjs"],"PASS_A102R7_APPROVED_MOBILE_WALLET_DEEPLINK_PRIVACY_CHANGES_NO_PROMOTION");
...
console.log(JSON.stringify({status:failed.length?"FAIL_A102R7_AUTHORITY":"PASS_A102R7_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT",revisionId:REV,checks:checks.length,passed:checks.length-failed.length,failed:failed.length,results:checks,globalDecision:"NO_GO",realObservationRuns:0,realMobileWalletBrowserRows:0,freshExactBuildBrowserCredit:false,stagingCredit:false,live:false,saleEnabled:false,productionApproved:false,worldClassProven:false},null,2));
