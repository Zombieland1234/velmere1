...
const REVISION_ID = "VELMERE_PASS36_A102R7_ACTION_REQUIRED_MOBILE_WALLET_DEEPLINK_QUERY_HASH_PRIVATE_PATH_AND_EXTERNAL_PROVIDER_PRIVACY_FAIL_CLOSED_NO_REAL_CREDIT";
...
const parseGroup = (argv) => {
  if (argv.length !== 2 || argv[0] !== "--group") throw new Error("a102r7_clean_group_argument");
  return argv[1];
};
...
const groups = {
  core: [
    { id: "mobile_wallet_privacy", command: [node, ...loader, "scripts/pass36/test-a102r7-mobile-wallet-deeplink-privacy-boundary.ts"], verify: (p) => statusIs("PASS_A102R7_MOBILE_WALLET_DEEPLINK_QUERY_HASH_PRIVATE_PATH_FAIL_CLOSED_NO_PROMOTION")(p) && p?.assertions === 43 },
    { id: "approved_changes", command: [node, "scripts/pass36/verify-a102r7-approved-mobile-wallet-deeplink-privacy-changes.mjs"], verify: (p) => statusIs("PASS_A102R7_APPROVED_MOBILE_WALLET_DEEPLINK_PRIVACY_CHANGES_NO_PROMOTION")(p) && p?.checks === 8 },
    { id: "local_regression_receipt", command: [node, "scripts/pass36/verify-a102r7-local-regression-receipt.mjs"], verify: (p) => statusIs("PASS_A102R7_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION")(p) && p?.checks === 15 },
    { id: "descendant", command: [node, "scripts/pass36/verify-a102r7-current-root-descendant.mjs"], verify: (p) => statusIs("PASS_A102R7_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION")(p) && p?.checks === 12 },
    { id: "authority", command: [node, "scripts/pass36/verify-a102r7-action-required-authority.mjs"], verify: (p) => statusIs("PASS_A102R7_ACTION_REQUIRED_AUTHORITY_NO_REAL_BUILD_BROWSER_OR_STAGING_CREDIT")(p) && p?.checks === 19 },
  ],
  history: [
  ...
  regression_core: [
    { id: "a102r7_mobile_wallet_deeplink_privacy", command: [node, ...loader, "scripts/pass36/test-a102r7-mobile-wallet-deeplink-privacy-boundary.ts"], verify: (p) => statusIs("PASS_A102R7_MOBILE_WALLET_DEEPLINK_QUERY_HASH_PRIVATE_PATH_FAIL_CLOSED_NO_PROMOTION")(p) && p?.assertions === 43 },
...
if (!stages) throw new Error(`a102r7_clean_group_unknown:${group}`);
const results = [];
...
const output = {
  schemaVersion: "velmere.pass36.a102r7.clean-unpack-group-verification.v1",
  revisionId: REVISION_ID,
  group,
  status: failed.length || results.length !== stages.length
    ? `FAIL_A102R7_CLEAN_UNPACK_GROUP_${group.toUpperCase()}`
    : `PASS_A102R7_CLEAN_UNPACK_GROUP_${group.toUpperCase()}_NO_PROMOTION`,
