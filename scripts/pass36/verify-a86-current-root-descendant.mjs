...
add("receipt:pass", receipt.status === "PASS_A86_LOCAL_CROSS_ASSET_MATRIX_NO_PROMOTION" && receipt.summary?.failed === 0, receipt.status);
add("claims:denominators", manifest.claims?.totalInstruments === 583 && manifest.claims?.fieldRows === 5830 && manifest.claims?.tierPackets === 1749 && manifest.claims?.channelProjections === 6996 && manifest.claims?.semanticMutations === 31482 && manifest.claims?.mutationKilled === 31482, { claims: manifest.claims, receipt: d });
add("claims:no-credit", manifest.claims?.realEvidenceRowsVerified === 0 && manifest.claims?.currentProviderEvidenceRows === 0 && manifest.claims?.rightsApprovedRows === 0 && manifest.claims?.productionBrowserRows === 0 && manifest.claims?.customerValueLabeledRows === 0 && manifest.claims?.paidDeliveredPackets === 0 && manifest.claims?.exactA80CandidateBound === false && manifest.claims?.paidGateEligible === false && manifest.claims?.liveProven === false && manifest.claims?.saleEnabled === false, manifest.claims);
for (const row of authority.checks) add(row.id, row.passed, row.detail);
const failed = checks.filter((row) => !row.passed);
console.log(JSON.stringify({ schemaVersion: "velmere.pass36.a86.current-root-descendant-verification.v2", revisionId: REV, status: failed.length ? "FAIL_A86_DESCENDANT" : "PASS_A86_HISTORICAL_OR_CURRENT_DESCENDANT_CHAIN", checks: checks.length, passed: checks.length - failed.length, failed: failed.length, failures: failed, currentRevisionId: authority.current.sourceRevisionId, historicalPayloadRecomputedAgainstCurrentTree: false, claims: manifest.claims, liveProven: false, saleEnabled: false }, null, 2));
