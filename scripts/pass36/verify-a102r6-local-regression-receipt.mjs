...
import { REV, PARENT, RECEIPT, STATE } from "./a102r6-source-boundary.mjs";
...
add("status",receipt.phase==="FINAL"&&receipt.status==="PASS_A102R6_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION",receipt.status);
add("rows",receipt.results?.length===16&&receipt.results.every((r)=>r.passed===true&&digest.test(r.stdoutSha256)&&digest.test(r.stderrSha256)),receipt.summary);
...
add("state",state.revisionId===REV&&state.parentRevisionId===PARENT&&state.localImplementation?.privateAccountPersistentBrowserStateRemoved===true&&state.localImplementation?.clientFallbackVaultReadyPromotionRemoved===true&&state.localImplementation?.durableVaultServerConfirmationRequired===true&&state.runtimeTruth?.freshWebpackBuildOnA102R6BytesExecuted===false&&state.runtimeTruth?.freshExactBrowserRowsExecuted===0);
const failed=checks.filter((r)=>!r.passed);
console.log(JSON.stringify({status:failed.length?"FAIL_A102R6_LOCAL_REGRESSION_RECEIPT":"PASS_A102R6_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION",revisionId:REV,checks:checks.length,passed:checks.length-failed.length,failed:failed.length,results:checks,live:false,saleEnabled:false},null,2));
