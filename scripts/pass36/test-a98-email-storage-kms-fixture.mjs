...
const anonKey = "anon_a98_fixture_key_abcdefghijklmnopqrstuvwxyz";
const serviceKey = "service_role_a98_fixture_key_abcdefghijklmnopqrstuvwxyz";
const kmsWrapSecret = "a98-kms-wrap-fixture-secret-abcdefghijklmnopqrstuvwxyz";
const kmsUnwrapSecret = "a98-kms-unwrap-fixture-secret-abcdefghijklmnopqrstuvwxyz";
const resendKey = "re_a98_fixture_abcdefghijklmnopqrstuvwxyz";
const bucket = "a98-private";
const tenantA = { email: "a98-a@example.invalid", password: "A98-a-password", id: "user-a98-a", token: "token-a98-a" };
const tenantB = { email: "a98-b@example.invalid", password: "A98-b-password", id: "user-a98-b", token: "token-a98-b" };
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === "/kms/wrap" && method === "POST") {
        if (bearer(req) !== (url.pathname === "/kms/wrap" ? kmsWrapSecret : kmsUnwrapSecret)) return json(res, 401, { error: "unauthorized" });
        const body = JSON.parse((await readBytes(req)).toString("utf8"));
        const wrapped = `wrapped:${crypto.randomBytes(12).toString("hex")}`;
        wrappedKeys.set(wrapped, body.plaintextKeyBase64);
        return json(res, 200, { wrappedKeyBase64: Buffer.from(wrapped).toString("base64"), keyId: "kms-fixture-a98", algorithm: "fixture-wrap-v1", contextDigest: body.contextDigest, requestId: body.requestId });
      }
      if (url.pathname === "/kms/unwrap" && method === "POST") {
        ...
        return json(res, 200, { plaintextKeyBase64: output, keyId: "kms-fixture-a98", algorithm: "fixture-wrap-v1", contextDigest: body.contextDigest, requestId: body.requestId });
      }
...
async function runScenario(name, options, expectedPass, expectedFailureId = null) {
  ...
  const child = spawn(process.execPath, ["scripts/a98-email-storage-kms-acceptance.mjs", "--fixture"], {
    cwd: root,
    ...
    env: {
      ...
      VELMERE_A98_STAGING_BASE_URL: base,
      VELMERE_A98_TENANT_A_EMAIL: tenantA.email,
      VELMERE_A98_TENANT_A_PASSWORD: tenantA.password,
      VELMERE_A98_TENANT_B_EMAIL: tenantB.email,
      VELMERE_A98_TENANT_B_PASSWORD: tenantB.password,
      SUPABASE_URL: base,
      SUPABASE_ANON_KEY: anonKey,
      SUPABASE_SERVICE_ROLE_KEY: serviceKey,
      VELMERE_A98_SUPABASE_PROJECT_CLASS: "disposable_staging",
      VELMERE_A98_STORAGE_BUCKET: bucket,
      VELMERE_A98_KMS_WRAP_URL: `${base}/kms/wrap`,
      VELMERE_A98_KMS_UNWRAP_URL: `${base}/kms/unwrap`,
      VELMERE_A98_KMS_WRAP_BEARER_SECRET: kmsWrapSecret,
      VELMERE_A98_KMS_UNWRAP_BEARER_SECRET: kmsUnwrapSecret,
      VELMERE_A98_KMS_TRUSTED_ORIGIN: base,
      RESEND_API_KEY: resendKey,
      VELMERE_A98_RESEND_API_BASE: base,
      VELMERE_A98_EMAIL_FROM: "Velmere Staging <staging@example.invalid>",
      VELMERE_A98_EMAIL_TO: "a98-inbox@example.invalid",
      VELMERE_A98_EMAIL_TO_SHA256: crypto.createHash("sha256").update("a98-inbox@example.invalid").digest("hex"),
      VELMERE_A98_CONFIRM: "fixture",
    },
  });
  let stdout = "", stderr = "";
  ...
  const report = JSON.parse(fs.readFileSync(path.join(root, "artifacts/pass36/a98/PASS36_A98_EMAIL_STORAGE_KMS_ORIGIN_CONTEXT_CLEANUP_AND_DELIVERY_TRUTH_BOUNDARY.json"), "utf8"));
...
fs.mkdirSync(path.join(root, "artifacts/pass36/a98"), { recursive: true });
fs.writeFileSync(path.join(root, "artifacts/pass36/a98/PASS36_A98_ADVERSARIAL_FIXTURE_SUITE.json"), `${JSON.stringify({ schemaVersion: "velmere.pass36.a98.adversarial-fixture-suite.v1", revisionId: "VELMERE_PASS36_A98R0_EMAIL_STORAGE_KMS_ORIGIN_CONTEXT_CLEANUP_AND_DELIVERY_TRUTH_BOUNDARY", generatedAt: new Date().toISOString(), summary: { checks: checks.length, passed: checks.length - failures.length, failed: failures.length }, checks }, null, 2)}\n`);
