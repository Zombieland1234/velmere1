...
import { REV, PARENT, RECEIPT } from "./a102r9-source-boundary.mjs";
...
const groups = [
  { name: "regression_core", expected: "PASS_A102R9_CLEAN_UNPACK_GROUP_REGRESSION_CORE_NO_PROMOTION", checks: 10 },
  { name: "regression_extended_a", expected: "PASS_A102R9_CLEAN_UNPACK_GROUP_REGRESSION_EXTENDED_A_NO_PROMOTION", checks: 4 },
  { name: "regression_extended_b1", expected: "PASS_A102R9_CLEAN_UNPACK_GROUP_REGRESSION_EXTENDED_B1_NO_PROMOTION", checks: 4 },
];
const groupRuns = [];
const results = [];
for (const group of groups) {
  const command = [node, "scripts/pass36/verify-a102r9-clean-unpack-tail.mjs", "--group", group.name];
...
const groupsPassed = groupRuns.length === groups.length && groupRuns.every((row) => row.groupPassed);
const sourceAudit = results.find((row) => row.id === "source_audit")?.metrics ?? {};
const receipt = {
  schemaVersion: "velmere.pass36.a102r9.local-regression-receipt.v2",
  revisionId: REV,
  parentRevisionId: PARENT,
  generatedAt: "2026-07-30T01:30:00.000Z",
  phase: "FINAL",
  status: groupsPassed && failed.length === 0 && results.length === 18
    ? "PASS_A102R9_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION"
    : "FAIL_A102R9_LOCAL_REGRESSION",
  summary: {
    executedPassRows: results.length,
    passedRows: results.filter((row) => row.passed).length,
    failedRows: failed.length + (results.length === 18 ? 0 : 18 - results.length),
  ...
  environmentBlockers: [
    ...
    { id: "real_system_clipboard_privacy_and_staging_inputs", classification: "BLOCKED_EXTERNAL", required: "real Chrome/Edge/Firefox/Android operating-system clipboard payload observation, user warning, staging, rights, legal and observers", available: false, credit: false },
  ],
  keyDenominators: {
    a102r9SystemClipboardBoundaryChecks: 38, a102r8CheckoutCustomerBrowserPrivacyChecks: 41, a102r6PrivateAccountBrowserStateChecks: 30,
  ...
  realEvidence: {
    a102ObservationRuns: 0, browserRows: 0, systemClipboardBrowserRows: 0, browserStoragePrivacyRows: 0,
