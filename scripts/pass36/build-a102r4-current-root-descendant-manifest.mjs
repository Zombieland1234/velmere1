...
} from "./a102r4-source-boundary.mjs";
...
if (state.revisionId !== REV || state.parentRevisionId !== PARENT) {
  throw new Error("a102r4_state_identity");
}
if (parent.revisionId !== PARENT || !parent.manifestDigestSha256) {
  throw new Error("a102r4_parent_manifest_identity");
}
if (receipt.revisionId !== REV || receipt.status !== "PASS_A102R4_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION") {
  throw new Error("a102r4_local_regression_receipt_identity");
}
const inventory = collect(root);
if (inventory.rejected.length) {
  throw new Error(`a102r4_rejected:${JSON.stringify(inventory.rejected)}`);
}
...
const core = {
  schemaVersion:
    "velmere.pass36.a102r4.action-required-current-root-descendant-manifest.v1",
  revisionId: REV,
  ...
  completedThrough: 89,
  coveredWorkRange:
    "A90-A102_LOCAL_CLIENT_AUTH_PREVIEW_HEADER_LOCAL_STORAGE_ACCOUNT_DATA_AND_VERIFIER_HARDENING_NO_FORMAL_PASS_CREDIT",
...
console.log(JSON.stringify({
  status: "BUILT_A102R4_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
