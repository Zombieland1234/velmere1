...
const ledger=JSON.parse(fs.readFileSync(path.join(root,'config/pass36/a102r33-approved-route-cache-authority-changes.json'),'utf8'));
const sha=(b)=>crypto.createHash('sha256').update(b).digest('hex');
let checks=0; const ok=(v,id)=>{checks++;assert.ok(v,id)};
ok(ledger.schemaVersion==='velmere.pass36.a102r33.approved-route-cache-authority-changes.v1','schema');
...
console.log(JSON.stringify({status:'PASS_A102R33_APPROVED_ROUTE_CACHE_AUTHORITY_CHANGES_NO_PROMOTION',checksPassed:checks,checksFailed:0,fileCount:ledger.fileCount},null,2));
