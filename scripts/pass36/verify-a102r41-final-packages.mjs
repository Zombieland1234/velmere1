...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    invariant(allowed.has(name), `a102r41_final_argument_unknown:${name}`);
    invariant(!values.has(name), `a102r41_final_argument_duplicate:${name}`);
    const value = argv[++index];
    invariant(typeof value === "string" && value.length > 0 && !value.startsWith("--"), `a102r41_final_argument_value:${name}`);
    values.set(name, value);
  }
  for (const name of ["--source-zip", "--materials-zip", "--roadmap"]) invariant(values.has(name), `a102r41_final_argument_required:${name}`);
  ...
  invariant(sourceZip !== materialsZip, "a102r41_final_archive_collision");
  invariant(roadmap !== sourceZip && roadmap !== materialsZip, "a102r41_final_roadmap_collision");
  invariant(output === null || (output !== sourceZip && output !== materialsZip && output !== roadmap), "a102r41_final_output_collision");
  return { sourceZip, materialsZip, roadmap, output };
}

function regularIdentity(archivePath, expectedName, kind) {
  const observed = readDescriptorBoundRegularFile(archivePath, { errorPrefix: `a102r41_final_${kind}` });
  invariant(observed.binding.fileName === expectedName, `a102r41_final_${kind}_filename`);
  const parsed = parseDeterministicZipBytes(observed.bytes);
  invariant(parsed.byteLength === observed.binding.byteLength && parsed.archiveSha256 === observed.binding.sha256, `a102r41_final_${kind}_descriptor_parse_binding`);
  return { parsed, identity: observed.binding };
}

const PARENT_ROADMAP_BYTES = 1_825_900;
const PARENT_ROADMAP_SHA256 = "0921fdc04fc9ea108ab8732251f1a515aa9473962cdce3205b8c6090be5fc705";
const PARENT_OPENING = Buffer.from("====================================================================================================\nPASS36 A102R40 — CURRENT SOURCE AUTHORITY + EXACT BUILD PREFLIGHT + STALE RELEASE POINTER RECONCILIATION", "utf8");
...
function inspectRoadmap(roadmapPath, internalRoadmap) {
  try {
    const observed = readDescriptorBoundRegularFile(roadmapPath, { maxBytes: 4 * 1024 * 1024, errorPrefix: "a102r41_final_roadmap" });
    invariant(observed.binding.fileName === ROADMAP_FILE_NAME, "a102r41_final_roadmap_filename");
    const boundary = observed.bytes.length - PARENT_ROADMAP_BYTES;
    invariant(boundary > PRESERVATION_MARKER.length && observed.bytes.subarray(boundary, boundary + PARENT_OPENING.length).equals(PARENT_OPENING), "a102r41_final_roadmap_parent_boundary");
    const prefix = observed.bytes.subarray(0, boundary);
    const suffix = observed.bytes.subarray(boundary);
    invariant(suffix.length === PARENT_ROADMAP_BYTES && cryptoSha256(suffix) === PARENT_ROADMAP_SHA256, "a102r41_final_roadmap_parent_suffix_binding");
    invariant(prefix.subarray(prefix.length - PRESERVATION_MARKER.length).equals(PRESERVATION_MARKER), "a102r41_final_roadmap_preservation_marker");
    const prefixText = prefix.toString("utf8");
    invariant(Buffer.from(prefixText, "utf8").equals(prefix), "a102r41_final_roadmap_prefix_utf8");
    invariant(prefixText.includes(`Revision ID: ${REVISION_ID}`) && prefixText.includes("Parent: VELMERE_PASS36_A102R40_ACTION_REQUIRED_CURRENT_SOURCE_AUTHORITY_EXACT_BUILD_PREFLIGHT_AND_STALE_RELEASE_POINTER_RECONCILIATION_NO_LIVE_CREDIT") && prefixText.includes("Klasa: ACTION_REQUIRED_NON_PASS") && prefixText.includes("GLOBAL DECISION: NO_GO") && prefixText.includes("LIVE=false") && prefixText.includes("saleEnabled=false") && prefixText.includes("productionApproved=false") && prefixText.includes("worldClassProven=false"), "a102r41_final_roadmap_r41_truth_prefix");
    invariant(internalRoadmap && internalRoadmap.byteLength === observed.binding.byteLength && internalRoadmap.sha256 === observed.binding.sha256 && Buffer.isBuffer(internalRoadmap.content) && internalRoadmap.content.equals(observed.bytes), "a102r41_final_roadmap_source_archive_binding");
    return { passed: true, ...observed.binding, sourceEntryPath: internalRoadmap.path, sourceEntryByteLength: internalRoadmap.byteLength, sourceEntrySha256: internalRoadmap.sha256, sourceByteIdentical: true, prefixByteLength: prefix.length, prefixSha256: cryptoSha256(prefix), sourceArchiveBindingPassed: true, lineagePassed: true, canonicalNameVerified: true, utf8Verified: true, r41PrefixVerified: true, parentSuffixVerified: true, parentSuffixByteLength: suffix.length, parentSuffixSha256: cryptoSha256(suffix), failure: null };
  } catch (error) {
    return { passed: false, fileName: path.basename(roadmapPath), byteLength: null, sha256: null, sourceEntryPath: SOURCE_ROADMAP_PATH, sourceEntryByteLength: internalRoadmap?.byteLength ?? null, sourceEntrySha256: internalRoadmap?.sha256 ?? null, sourceByteIdentical: false, prefixByteLength: null, prefixSha256: null, sourceArchiveBindingPassed: false, lineagePassed: false, canonicalNameVerified: false, utf8Verified: false, r41PrefixVerified: false, parentSuffixVerified: false, parentSuffixByteLength: null, parentSuffixSha256: null, failure: error instanceof Error ? error.message : String(error) };
  }
}
...
function inspect(archivePath, kind, expectedSource = null) {
  ...
  try {
    ...
    const verified = validateA102R41ParsedArchive(parsed, kind, expectedSource);
    const sourceRoadmap = kind === "source" ? parsed.entries.find((entry) => entry.path === SOURCE_ROADMAP_PATH) : null;
    if (kind === "source") invariant(sourceRoadmap && sourceRoadmap.mode === 0o100644, "a102r41_final_source_roadmap_entry");
    const result = {
...
export function verifyA102R41FinalPackages({ sourceZip, materialsZip, roadmap }) {
  ...
  return {
    schemaVersion: "velmere.pass36.a102r41.final-packages-verification.v1",
    revisionId: REVISION_ID,
    status: canonicalArtifactSetPassed ? "PASS_A102R41_FINAL_ACTION_REQUIRED_PACKAGES_NO_PROMOTION" : "FAIL_A102R41_FINAL_PACKAGES",
...
function writeNoClobber(filePath, text) {
  invariant(!fs.existsSync(filePath), "a102r41_final_output_exists_no_overwrite");
...
function main() {
  ...
  const result = verifyA102R41FinalPackages(options);
...
if (invoked === import.meta.url) {
  try { main(); }
    ...
    process.stderr.write(`${JSON.stringify({ schemaVersion: "velmere.pass36.a102r41.final-packages-verification.v1", revisionId: REVISION_ID, status: "FAIL_A102R41_FINAL_PACKAGES", error: error instanceof Error ? error.message : String(error), globalDecision: "NO_GO", live: false, saleEnabled: false, productionApproved: false, worldClassProven: false })}\n`);
    process.exitCode = 1;
  }
}
