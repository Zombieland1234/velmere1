...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    invariant(allowed.has(token), `a102r8_verify_argument_unknown:${token}`);
    invariant(
      !values.has(token),
      `a102r8_verify_argument_duplicate:${token}`,
    );
    const value = argv[index + 1];
    invariant(
      typeof value === "string" && value.length > 0 && !value.startsWith("--"),
      `a102r8_verify_argument_value_missing:${token}`,
    );
  ...
  for (const required of ["--source-zip", "--materials-zip"]) {
    invariant(
      values.has(required),
      `a102r8_verify_argument_required:${required}`,
    );
  }
  ...
  invariant(
    ...
    "a102r8_verify_archive_paths_must_differ",
  );
  if (output !== null) {
    invariant(
      output !== sourceZip && output !== materialsZip,
      "a102r8_verify_output_archive_collision",
    );
  }
...
function assertRegularArchive(archivePath, kind) {
  ...
  invariant(
    ...
    `a102r8_verify_${kind}_archive_not_regular:${archivePath}`,
  );
}
...
function rawArchiveIdentity(archivePath) {
  ...
  invariant(
    !fileName.includes("/"),
    "a102r8_verify_source_filename_invalid",
  );
  return {
    fileName,
...
function inspectArchive(archivePath, kind, expectedSourceArchive = null) {
  ...
  try {
    ...
    const validation = validateA102R8ParsedArchive(
      parsed,
      kind,
...
export function verifyA102R8FinalPackages({
  sourceZip,
  materialsZip,
}) {
  ...
  return {
    schemaVersion: "velmere.pass36.a102r8.final-packages-verification.v1",
    revisionId: REVISION_ID,
    status:
      failures.length === 0
        ? "PASS_A102R8_FINAL_PACKAGES_ACTION_REQUIRED_NO_PROMOTION"
        : "FAIL_A102R8_FINAL_PACKAGES",
    source,
    materials,
...
function main() {
  ...
  const result = verifyA102R8FinalPackages(options);
...
if (invokedPath === import.meta.url) {
  try {
    ...
    process.stderr.write(
      `${JSON.stringify({
        schemaVersion:
          "velmere.pass36.a102r8.final-packages-verification.v1",
        revisionId: REVISION_ID,
        status: "FAIL_A102R8_FINAL_PACKAGES",
        failed: 1,
        failures: [
          {
            id: "verifier",
