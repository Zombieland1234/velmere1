...
import {
  ...
  REVISION_ID,
  validateA102R7ParsedArchive,
} from "./package-a102r7-deterministic.mjs";
import { collect, payload } from "./a102r7-source-boundary.mjs";
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    invariant(allowed.has(token), `a102r7_clean_argument_unknown:${token}`);
    invariant(!values.has(token), `a102r7_clean_argument_duplicate:${token}`);
    const value = argv[index + 1];
    invariant(typeof value === "string" && value.length > 0 && !value.startsWith("--"), `a102r7_clean_argument_value:${token}`);
    values.set(token, value);
    index += 1;
  }
  for (const required of allowed) invariant(values.has(required), `a102r7_clean_argument_required:${required}`);
  return {
...
function writeEntry(root, entry) {
  ...
  invariant(absolute.startsWith(`${root}${path.sep}`), `a102r7_clean_extract_escape:${entry.path}`);
...
function main() {
  ...
  invariant(sourceMeta.isFile() && !sourceMeta.isSymbolicLink(), "a102r7_clean_source_zip_not_regular");
  fs.mkdirSync(receiptDir, { recursive: true });
  const parsed = parseDeterministicZip(sourceZip);
  const packageValidation = validateA102R7ParsedArchive(parsed, "source");
  ...
  invariant(fs.existsSync(path.join(cleanRoot, SOURCE_MANIFEST_PATH)), "a102r7_clean_source_manifest_missing");

  const beforeInventory = collect(cleanRoot);
  invariant(beforeInventory.rejected.length === 0, `a102r7_clean_rejected_before:${JSON.stringify(beforeInventory.rejected)}`);
  ...
  const node = process.execPath;
    ...
    { id: "a102r7_group_core", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "core"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_CORE_NO_PROMOTION", checks: 5 },
    { id: "a102r7_group_history", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "history"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_HISTORY_NO_PROMOTION", checks: 5 },
    { id: "a102r7_group_route_product", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "route_product"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_ROUTE_PRODUCT_NO_PROMOTION", checks: 5 },
    { id: "a102r7_group_operational", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "operational"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_OPERATIONAL_NO_PROMOTION", checks: 5 },
    { id: "a102r7_group_final_a", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "final_a"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_FINAL_A_NO_PROMOTION", checks: 4 },
    { id: "a102r7_group_final_b1", command: [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", "final_b1"], expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_FINAL_B1_NO_PROMOTION", checks: 2 },
    { id: "a102r7_source_audit_direct", command: [node, "scripts/a44-source-integrity-audit.mjs"], sourceAudit: true },
  ];
  const results = [];
  ...
  const receipt = {
    schemaVersion: "velmere.pass36.a102r7.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: localContractPassed
      ? "ACTION_REQUIRED_A102R7_CLEAN_UNPACK_LOCAL_CONTRACT_NO_EXACT_RUNTIME_BROWSER_OR_REAL_CREDIT"
      : "FAIL_A102R7_CLEAN_UNPACK_LOCAL_CONTRACT",
    sourceZip: {
    ...
    environmentTruth: {
      ...
      realMobileWalletBrowserRows: 0,
      realMobileWalletProviderPrivacyObservations: 0,
    },
    notExecutedOrNotCredited: [
      ["fresh_exact_npm_ci_lint_typecheck_builds_next_start", "NOT_EXECUTED_ON_A102R7_FINAL_BYTES"],
  ...
  const receiptPath = path.join(receiptDir, "PASS36_A102R7_CLEAN_UNPACK_RECEIPT.json");
...
try { main(); }
  ...
  process.stderr.write(`${JSON.stringify({
    schemaVersion: "velmere.pass36.a102r7.clean-unpack-verification.v1",
    revisionId: REVISION_ID,
    status: "FAIL_A102R7_CLEAN_UNPACK_LOCAL_CONTRACT",
