...
import { REV, PARENT, RECEIPT, STATE } from "./a102r5-source-boundary.mjs";
...
add("status",receipt.phase==="FINAL"&&receipt.status==="PASS_A102R5_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION",receipt.status);
add("rows",receipt.results?.length===15&&receipt.results.every((r)=>r.passed===true&&digest.test(r.stdoutSha256)&&digest.test(r.stderrSha256)),receipt.summary);
add("auth",receipt.keyDenominators?.a102r5PaidBoundaryChecks===26&&receipt.keyDenominators?.vlmVerifyPositiveCases===1&&receipt.keyDenominators?.vlmVerifyNegativeCases===5&&receipt.keyDenominators?.a73Checks===57&&receipt.keyDenominators?.a73VerifierChecks===73);
...
add("deferred-lineage",receipt.deferredFinalLineageVerifiers?.length===2&&receipt.deferredFinalLineageVerifiers.every((r)=>r.requiredAfterDescendantFreeze===true&&r.credit===false),receipt.deferredFinalLineageVerifiers);
...
add("state",state.revisionId===REV&&state.parentRevisionId===PARENT&&state.localImplementation?.browserPaidBearerTokenPersistenceRemoved===true&&state.localImplementation?.serverAccountSessionAndEntitlementLedgerRequiredForPaidDepth===true&&state.runtimeTruth?.freshWebpackBuildOnA102R5BytesExecuted===false&&state.runtimeTruth?.freshExactBrowserRowsExecuted===0);
const failed=checks.filter((r)=>!r.passed);
console.log(JSON.stringify({status:failed.length?"FAIL_A102R5_LOCAL_REGRESSION_RECEIPT":"PASS_A102R5_LOCAL_REGRESSION_RECEIPT_ACTION_REQUIRED_NO_PROMOTION",revisionId:REV,checks:checks.length,passed:checks.length-failed.length,failed:failed.length,results:checks,live:false,saleEnabled:false},null,2));
