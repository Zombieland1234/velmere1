...
REVISION_ID = "VELMERE_PASS36_A89R0_ACCOUNT_AUTH_TENANT_PRIVACY_RED_TEAM_AND_TRUST_CENTER_INTAKE"
...
COMMANDS: tuple[tuple[str, tuple[str, ...], bool], ...] = (
    ...
    ("a89_local_red_team", ("--import", "./scripts/pass11/register-offline-ts-loader.mjs", "scripts/pass36/test-a89-account-auth-tenant-privacy-red-team.ts"), False),
    ("a89_descendant", ("scripts/pass36/verify-a89-current-root-descendant.mjs",), False),
    ("a89_verifier", ("--import", "./scripts/pass11/register-offline-ts-loader.mjs", "scripts/pass36/verify-a89-account-auth-tenant-privacy-red-team.ts"), False),
    ("a78_historical_descendant_chain", ("scripts/pass36/verify-a78-current-root-descendant.mjs",), False),
    ("a73_cookie_session_boundary", ("--experimental-strip-types", "--import", "./scripts/pass11/register-offline-ts-loader.mjs", "scripts/pass36/test-a73-cookie-session-boundary.mjs"), False),
...
def run_command(node: str, identifier: str, args: tuple[str, ...], parse_a58: bool) -> dict[str, Any]:
    started = time.monotonic(); environment = os.environ.copy(); environment["VELMERE_A89_CLEAN_UNPACK_SEQUENCE"] = "1"
    try:
...
def create_state_file() -> Path:
    fd, raw = tempfile.mkstemp(prefix="velmere-a89-clean-state-", suffix=".json"); os.close(fd); p=Path(raw); os.chmod(p,0o600); p.write_text(json.dumps({"next":0,"results":[]}),"utf-8"); return p

def read_state(p:Path):
    v=json.loads(p.read_text("utf-8"))
    if not isinstance(v,dict) or v.get("next") != len(v.get("results",[])): raise RuntimeError("a89_clean_state_invalid")
    return v
...
def report(results):
    ...
    return {
        "schemaVersion":"velmere.pass36.a89.clean-unpack-sequence.v1",
        "revisionId":REVISION_ID,
        "status":"FAIL_A89_CLEAN_UNPACK_SEQUENCE" if failures else "PASS_A89_CLEAN_UNPACK_SEQUENCE_NO_PROMOTION",
        "requiredOrder":["A58_EXACT_PATH_SET_FIRST","A89_AND_RETAINED_REGRESSIONS_AFTER_A58"],
        "processBoundary":{"nodeBridge":"ASYNC_SPAWN_BOUNDED_OUTPUT_TIMEOUT_EXPLICIT_EXIT","pythonBatches":"SELF_REEXECUTING_BOUNDED_SUBPROCESS_RUN","shell":False,"batchSize":BATCH_SIZE,"commandTimeoutSeconds":COMMAND_TIMEOUT_SECONDS},
        "checks":len(results),
        "passed":sum(r["passed"] for r in results),
        "failed":len(failures),
        "results":results,
        "currentRoot":{"passed":24,"denominator":30,"blocked":6,"exactRuntimeCredited":False},
        "realOAuthRuns":0,
        ...
        "liveProven":False,
        "saleEnabled":False,
        "truthBoundary":"This verifies safe clean-unpack ordering and local regressions only. It does not establish exact runtime, real OAuth, production proxy semantics, two-tenant RLS, DSAR execution, legal approval, provider rights, customer value, LIVE or sale readiness."
    }

def main():
    if not (ROOT/"_velmere/PASS35_SOURCE_ONLY_MANIFEST.json").is_file(): raise RuntimeError("a89_clean_unpack_package_manifest_required")
    node=shutil.which("node")
    if not node: raise RuntimeError("a89_clean_unpack_node_not_found")
    if len(sys.argv)==3 and sys.argv[1]=="--state":
        state_path=Path(sys.argv[2]).resolve()
        if not state_path.is_file() or ROOT in state_path.parents: raise RuntimeError("a89_clean_state_path_invalid")
    elif len(sys.argv)==1: state_path=create_state_file()
    else: raise RuntimeError("a89_clean_arguments_invalid")
