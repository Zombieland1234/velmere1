...
export const SOURCE_MANIFEST_PATH =
  "_velmere/PASS36_A102R28_SOURCE_ONLY_MANIFEST.json";
export const MATERIALS_MANIFEST_PATH =
  "MANIFESTS/PASS36_A102R28_MATERIALS_MANIFEST.json";
export const SOURCE_MANIFEST_SCHEMA =
  "velmere.pass36.a102r28.source-only-package-manifest.v1";
export const MATERIALS_MANIFEST_SCHEMA =
  "velmere.pass36.a102r28.materials-package-manifest.v1";
...
export function validatePortableArchivePath(value) {
  ...
  invariant(
    ...
    `a102r28_archive_path_control_character:${JSON.stringify(value)}`,
  );
  return value;
}
...
export function validatePortablePathSet(paths) {
  ...
  for (const [label, transform] of transforms) {
    ...
    for (const entryPath of paths) {
      ...
      invariant(
        !seen.has(key),
        `a102r28_${label}_path_collision:${seen.get(key)}:${entryPath}`,
      );
...
function assertRegularDirectory(root) {
  ...
  invariant(metadata.isDirectory(), `a102r28_root_not_directory:${root}`);
  invariant(!metadata.isSymbolicLink(), `a102r28_root_symlink_forbidden:${root}`);
}
...
function projectedRealPath(targetPath) {
  ...
  while (!fs.existsSync(cursor)) {
    ...
    invariant(parent !== cursor, `a102r28_output_ancestor_missing:${targetPath}`);
...
function assertOutputOutsideRoot(root, output) {
  ...
  invariant(
    ...
    `a102r28_output_inside_input_root:${resolvedOutput}`,
  );
}

export function collectA102R28Inventory(rootPath, kind) {
  invariant(
    kind === "source" || kind === "materials",
    `a102r28_package_kind_invalid:${kind}`,
  );
    ...
    for (const directoryEntry of directoryEntries) {
      ...
      if (relativePath === manifestPath) {
        ...
        invariant(
          ...
          `a102r28_reserved_manifest_not_regular:${relativePath}`,
        );
        invariant(
          metadata.nlink === 1,
          `a102r28_hardlink_forbidden:${relativePath}`,
        );
        continue;
      }
      ...
      invariant(
        ...
        `a102r28_symlink_forbidden:${relativePath}`,
      );
      if (metadata.isDirectory()) {
        ...
        if (
          ...
          throw new Error(`a102r28_materials_forbidden_path:${relativePath}`);
        }
      ...
      invariant(
        metadata.isFile(),
        `a102r28_special_file_forbidden:${relativePath}`,
      );
      invariant(
        metadata.nlink === 1,
        `a102r28_hardlink_forbidden:${relativePath}`,
  ...
  invariant(rows.length > 0, `a102r28_${kind}_inventory_empty`);
...
export function buildA102R28Manifest(
  kind,
  inventory,
  ...
  if (kind === "source") {
    invariant(
      sourceArchiveBinding === null,
      "a102r28_source_manifest_binding_forbidden",
    );
  } else {
    invariant(
      exactKeys(sourceArchiveBinding, ["fileName", "byteLength", "sha256"]),
      "a102r28_materials_source_binding_shape",
    );
  }
...
function parseManifestEntry(entry, kind) {
  try {
    ...
    invariant(isRecord(manifest), `a102r28_${kind}_manifest_not_object`);
    return manifest;
  } catch (error) {
    if (
      error instanceof Error
      && error.message.startsWith(`a102r28_${kind}_manifest_`)
    ) {
      throw error;
    }
    throw new Error(
      `a102r28_${kind}_manifest_parse:${
...
function validateManifestShape(manifest, kind) {
  ...
  invariant(
    ...
    `a102r28_${kind}_manifest_top_level_fields`,
  );
  invariant(
    manifest.schemaVersion === schemaVersion,
    `a102r28_${kind}_manifest_schema`,
  );
  invariant(
    manifest.revisionId === REVISION_ID,
    `a102r28_${kind}_manifest_revision`,
  );
  invariant(
    manifest.parentRevisionId === PARENT_REVISION_ID,
    `a102r28_${kind}_manifest_parent`,
  );
  invariant(
    manifest.normalizedTimestamp === NORMALIZED_TIMESTAMP,
    `a102r28_${kind}_manifest_timestamp`,
  );
  invariant(
    manifest.manifestPath === manifestPath
      && manifest.manifestExcludedFromOwnInventory === true,
    `a102r28_${kind}_manifest_self_boundary`,
  );
  invariant(
  ...
  invariant(
    ...
    `a102r28_${kind}_manifest_entry_order`,
  );
  invariant(
    manifest.fileCount === manifest.entries.length,
    `a102r28_${kind}_manifest_file_count`,
  );
  invariant(
    manifest.byteLength
  ...
  invariant(
    ...
    `a102r28_${kind}_manifest_aggregate`,
  );
  ...
  if (kind === "materials") {
    ...
    invariant(
      ...
      "a102r28_materials_source_binding_fields",
    );
    validatePortableArchivePath(binding.fileName);
    invariant(
      !binding.fileName.includes("/"),
      "a102r28_materials_source_binding_filename",
    );
    invariant(
      Number.isSafeInteger(binding.byteLength) && binding.byteLength > 0,
      "a102r28_materials_source_binding_bytes",
    );
    invariant(
      typeof binding.sha256 === "string" && DIGEST.test(binding.sha256),
      "a102r28_materials_source_binding_digest",
    );
  }
}

export function validateA102R28ParsedArchive(
  parsed,
  kind,
  ...
  invariant(
    kind === "source" || kind === "materials",
    `a102r28_archive_kind_invalid:${kind}`,
  );
  const manifestPath =
    kind === "source" ? SOURCE_MANIFEST_PATH : MATERIALS_MANIFEST_PATH;
  ...
  invariant(
    ...
    `a102r28_${kind}_archive_manifest_count`,
  );
  invariant(
    manifestEntries[0].mode === 0o100644,
    `a102r28_${kind}_archive_manifest_mode`,
  );
  ...
  invariant(
    ...
    `a102r28_${kind}_archive_payload_manifest_mismatch`,
  );
  if (kind === "source") {
    for (const entry of payload) {
      invariant(
        sourceExclusionReason(entry.path) === null,
        `a102r28_source_archive_hygiene:${entry.path}`,
      );
    }
  }
  if (kind === "materials" && expectedSourceArchive !== null) {
    invariant(
      ...
      "a102r28_materials_source_archive_binding_mismatch",
    );
  }
  return {
    manifest,
...
function assertRegularArchive(archivePath, label) {
  ...
  invariant(
    metadata.isFile() && !metadata.isSymbolicLink(),
    `a102r28_${label}_archive_not_regular:${archivePath}`,
  );
}
...
export function sourceArchiveIdentity(sourceZipPath) {
  ...
  invariant(
    !fileName.includes("/"),
    "a102r28_source_archive_filename_invalid",
  );
  const parsed = parseDeterministicZip(sourceZip);
  validateA102R28ParsedArchive(parsed, "source");
  return {
    fileName,
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === "--overwrite") {
      invariant(!overwrite, "a102r28_argument_duplicate:--overwrite");
      overwrite = true;
      continue;
    }
    invariant(valueOptions.has(token), `a102r28_argument_unknown:${token}`);
    invariant(!values.has(token), `a102r28_argument_duplicate:${token}`);
    const value = argv[index + 1];
    invariant(
      typeof value === "string" && value.length > 0 && !value.startsWith("--"),
      `a102r28_argument_value_missing:${token}`,
    );
    values.set(token, value);
    index += 1;
  }
  for (const required of ["--kind", "--root", "--output"]) {
    invariant(values.has(required), `a102r28_argument_required:${required}`);
  }
  const kind = values.get("--kind");
  invariant(
    kind === "source" || kind === "materials",
    `a102r28_argument_kind:${kind}`,
  );
  const sourceZip = values.get("--source-zip") ?? null;
  invariant(
    kind === "materials" ? sourceZip !== null : sourceZip === null,
    kind === "materials"
      ? "a102r28_materials_source_zip_required"
      : "a102r28_source_source_zip_forbidden",
  );
  return {
    kind,
...
export function packageA102R28(options) {
  ...
  const verified = validateA102R28ParsedArchive(
    parsed,
    options.kind,
    binding,
  );
  invariant(
    ...
    "a102r28_archive_post_write_identity_mismatch",
  );
  invariant(
    verified.manifest.manifestSha256 === manifest.manifestSha256,
    "a102r28_archive_post_write_manifest_mismatch",
  );

  const afterRows = collectA102R28Inventory(options.root, options.kind);
  const after = inventoryFields(afterRows.rows);
  invariant(
    inventoriesEqual(before, after),
    "a102r28_input_root_changed_during_packaging",
  );

  return {
    schemaVersion: "velmere.pass36.a102r28.deterministic-package.v1",
    revisionId: REVISION_ID,
    status:
      options.kind === "source"
        ? "PASS_A102R28_DETERMINISTIC_SOURCE_PACKAGE_NO_PROMOTION"
        : "PASS_A102R28_DETERMINISTIC_MATERIALS_PACKAGE_NO_PROMOTION",
    kind: options.kind,
...
function main() {
  ...
  const result = packageA102R28(options);
...
if (invokedPath === import.meta.url) {
  try {
    ...
    process.stderr.write(
      `${JSON.stringify({
        status: "FAIL_A102R28_DETERMINISTIC_PACKAGE",
