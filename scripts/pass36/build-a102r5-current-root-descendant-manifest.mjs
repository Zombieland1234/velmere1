...
} from "./a102r5-source-boundary.mjs";
...
if (state.revisionId !== REV || state.parentRevisionId !== PARENT) {
  throw new Error("a102r5_state_identity");
}
if (parent.revisionId !== PARENT || !parent.manifestDigestSha256) {
  throw new Error("a102r5_parent_manifest_identity");
}
if (receipt.revisionId !== REV || receipt.status !== "PASS_A102R5_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION") {
  throw new Error("a102r5_local_regression_receipt_identity");
}
const inventory = collect(root);
if (inventory.rejected.length) {
  throw new Error(`a102r5_rejected:${JSON.stringify(inventory.rejected)}`);
}
...
const core = {
  schemaVersion:
    "velmere.pass36.a102r5.action-required-current-root-descendant-manifest.v1",
  revisionId: REV,
  ...
  completedThrough: 89,
  coveredWorkRange:
    "A90-A102_LOCAL_SERVER_ACCOUNT_ENTITLEMENT_BROWSER_BEARER_PERSISTENCE_PAID_DEPTH_AND_VERIFIER_HARDENING_NO_FORMAL_PASS_CREDIT",
...
console.log(JSON.stringify({
  status: "BUILT_A102R5_DESCENDANT_ACTION_REQUIRED_NO_PROMOTION",
