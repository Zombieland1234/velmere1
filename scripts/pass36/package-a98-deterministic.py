...
PARENT='VELMERE_PASS36_A97R1_SCOPED_PAYMENT_OPERATOR_ASSERTIONS_DUAL_CONTROL_AND_SINGLE_USE_REQUEUE'
TS='1980-01-01T00:00:00.000Z'
SOURCE_MANIFEST='_velmere/PASS36_A98R0_SOURCE_ONLY_MANIFEST.json'
MATERIALS_MANIFEST='MANIFESTS/PASS36_A98R0_MATERIALS_MANIFEST.json'
...
def source_manifest(rows):
    ...
    core={'schemaVersion':'velmere.pass36.a98.source-only-package-manifest.v1','revisionId':REV,'parentRevisionId':PARENT,'normalizedTimestamp':TS,'fileCount':len(entries),'byteLength':total,'pathSetSha256':pathset,'aggregateSha256':agg,'entries':entries,'manifestPath':SOURCE_MANIFEST,'manifestExcludedFromOwnInventory':True,'checkpointClass':'ACTION_REQUIRED_NON_PASS','completedThrough':89,'a90ToA98PassCredit':False,'exactReleaseCredit':False,'globalDecision':'NO_GO','live':False,'saleEnabled':False,'productionApproved':False,'worldClassProven':False}
    return {**core,'manifestSha256':sha256_bytes(canonical(core))}

def materials_manifest(rows,source_zip:pathlib.Path|None):
    entries,total,pathset,agg=inventory_fields(rows)
    core={'schemaVersion':'velmere.pass36.a98.materials-package-manifest.v1','revisionId':REV,'parentRevisionId':PARENT,'normalizedTimestamp':TS,'fileCount':len(entries),'byteLength':total,'pathSetSha256':pathset,'aggregateSha256':agg,'entries':entries,'manifestPath':MATERIALS_MANIFEST,'manifestExcludedFromOwnInventory':True,'checkpointClass':'ACTION_REQUIRED_NON_PASS','globalDecision':'NO_GO','live':False,'saleEnabled':False,'productionApproved':False,'worldClassProven':False,'sourceArchiveBinding':None}
    if source_zip:
