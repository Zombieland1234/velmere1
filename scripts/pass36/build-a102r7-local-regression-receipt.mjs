...
import { REV, PARENT, RECEIPT } from "./a102r7-source-boundary.mjs";
...
const groups = [
  { name: "regression_core", expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_REGRESSION_CORE_NO_PROMOTION", checks: 9 },
  { name: "regression_extended_a", expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_REGRESSION_EXTENDED_A_NO_PROMOTION", checks: 4 },
  { name: "regression_extended_b1", expected: "PASS_A102R7_CLEAN_UNPACK_GROUP_REGRESSION_EXTENDED_B1_NO_PROMOTION", checks: 3 },
];
const groupRuns = [];
const results = [];
for (const group of groups) {
  const command = [node, "scripts/pass36/verify-a102r7-clean-unpack-tail.mjs", "--group", group.name];
...
const receipt = {
  schemaVersion: "velmere.pass36.a102r7.local-regression-receipt.v2",
  revisionId: REV,
  parentRevisionId: PARENT,
  generatedAt: "2026-07-29T22:20:00.000Z",
  phase: "FINAL",
  status: groupsPassed && failed.length === 0 && results.length === 17
    ? "PASS_A102R7_LOCAL_REGRESSION_ACTION_REQUIRED_NO_PROMOTION"
    : "FAIL_A102R7_LOCAL_REGRESSION",
  summary: {
  ...
  environmentBlockers: [
    ...
    { id: "real_mobile_wallet_and_staging_inputs", classification: "BLOCKED_EXTERNAL", required: "real Android/iOS MetaMask/Phantom navigation, referrer observation, staging, rights, legal and observers", available: false, credit: false },
  ],
  keyDenominators: {
    a102r7MobileWalletDeeplinkPrivacyChecks: 43, a102r6PrivateAccountBrowserStateChecks: 30,
  ...
  realEvidence: {
    a102ObservationRuns: 0, browserRows: 0, mobileWalletProviderRows: 0,
    externalProviderReferrerObservations: 0, stagingStages: 0, providerRights: 0,
