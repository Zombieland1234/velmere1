...
MANIFEST_REL = "_velmere/PASS36_A95R0_SOURCE_ONLY_MANIFEST.json"
...
def verify_manifest(root: pathlib.Path):
    ...
    checks.append(('manifest_no_promotion',manifest.get('a90ToA95PassCredit') is False and manifest.get('exactReleaseCredit') is False and manifest.get('globalDecision')=='NO_GO' and manifest.get('live') is False and manifest.get('saleEnabled') is False and manifest.get('productionApproved') is False and manifest.get('worldClassProven') is False,None))
...
def run_step(root: pathlib.Path, receipt_dir: pathlib.Path, index: int, step: dict):
    start=time.monotonic(); env={'PATH':os.environ.get('PATH',''),'LANG':'C.UTF-8','LC_ALL':'C.UTF-8','TZ':'UTC','CI':'1','NO_COLOR':'1','NODE_ENV':'test','VELMERE_A95_CLEAN_UNPACK':'1','VELMERE_A95_NO_WRITE':'1'}
...
def main():
    ...
    steps=[
      {'id':'a58_release_integrity_literal_first_child','cmd':[sys.executable.replace('python3','node') if False else 'node','scripts/pass36/verify-a58-release-integrity.mjs'],'expectedStatus':'PASS_RELEASE_INTEGRITY_NO_PROMOTION','a58':True,'timeout':420},
      {'id':'a95_authority_descendant','cmd':['node','scripts/pass36/verify-a95-action-required-authority.mjs'],'expectedStatus':'PASS_A95_ACTION_REQUIRED_AUTHORITY_NO_STAGING_CREDIT','timeout':300},
      {'id':'a95_staging_subject_admission','cmd':['node','scripts/pass36/verify-a95-staging-subject-admission.mjs'],'timeout':240},
      ...
      {'id':'orphan_build_lock_dual_review','cmd':['node','scripts/deployment/test-orphan-build-lock-recovery.mjs'],'timeout':180},
      ...
      {'id':'cross_surface_semantic_mutations','cmd':['node','scripts/pass36/test-a94r2-cross-surface-value-truth.mjs'],'timeout':180},
      ...
      {'id':'a59_static_budget_regression','cmd':['node','scripts/pass36/test-a59-build-graph-route-css-budget-recovery.mjs'],'timeout':240},
    ...
    status='ACTION_REQUIRED_EXTERNAL_AND_EXACT_RELEASE_CLOSURE' if not failures else 'FAIL_A95R0_CLEAN_UNPACK_LOCAL_CONTRACT'
    receipt={'schemaVersion':'velmere.pass36.a95r0.clean-unpack-verification.v1','revisionId':REV,'status':status,'localContractPassed':not failures,'a58LiteralFirstChild':bool(results) and results[0]['id']=='a58_release_integrity_literal_first_child','sourceArchiveManifestSha256':manifest.get('manifestSha256'),'checks':len(checks),'passed':len(checks)-len(failures),'failed':len(failures),'failures':failures,'steps':results,'notExecutedOrNotCredited':[
    ...
    out=receipt_dir/'PASS36_A95R0_CLEAN_UNPACK_RECEIPT.json'; out.write_text(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
