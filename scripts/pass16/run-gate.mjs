...
const policyPath = path.join(root, "config", "pass16", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS16 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass16-gates");
...
console.log(`PASS16 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass16.gate-execution.v1",
...
console.log(`PASS16 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS16 gate receipt: ${path.relative(root, receiptPath)}`);
