...
const policyPath = path.join(root, "config", "pass22", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS22 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass22-gates");
...
console.log(`PASS22 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass22.gate-execution.v1",
...
console.log(`PASS22 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS22 gate receipt: ${path.relative(root, receiptPath)}`);
