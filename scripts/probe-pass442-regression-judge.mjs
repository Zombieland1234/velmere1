...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "Velmere-PASS442-Regression-Judge/1.0" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
...
function regressionJudge(rows) {
  ...
  const coreScore = ["price", "change", "volume", "candles"].filter(any).length * 25;
  const fakeLiveRisk = Math.max(0, (providerCount < 2 ? 42 : 12) + (any("price") ? 0 : 35) + (coreScore < 50 ? 18 : 0));
  const checks = [
    { id: "core_fields_no_backslide", score: coreScore },
    { id: "second_provider_regression", score: providerCount >= 2 ? 96 : 28 },
    { id: "fake_live_budget_no_backslide", score: Math.max(0, 100 - fakeLiveRisk) },
    { id: "pdf_payload_regression_lock", score: 90 },
    { id: "missing_data_visible", score: rows.flatMap((row) => missing(row, providerCount)).length ? 74 : 100 },
  ];
  const score = Math.round(checks.reduce((sum, item) => sum + item.score, 0) / checks.length);
  const blocked = checks.filter((item) => item.score < 34).length;
  const watch = checks.filter((item) => item.score >= 55 && item.score < 76).length;
  const mode = blocked >= 2 ? "operator_regression_review" : blocked ? "sealed_regression" : watch >= 2 ? "guarded_regression" : "quality_stable";
  return { providerCount, score, blocked, watch, mode, fakeLiveRisk, checks };
}
console.log("PASS442 regression judge · no quality backslide, missing data visible, one payload PDF/chat\n");
for (const query of targets) {
  const rows = await Promise.all([probeCoinGecko, probeDexScreener, probeBinance, probeYahoo].map((fn) => settle(fn, query)));
  const judge = regressionJudge(rows);
  console.log(`--- ${query} ---`);
  console.log(`regression: ${judge.mode} · score ${judge.score}/100 · providers ${judge.providerCount} · blocked/watch ${judge.blocked}/${judge.watch} · fake-live ${judge.fakeLiveRisk}/100`);
  for (const row of rows) console.log(`${row.provider}: ${row.status} · price ${row.price ?? "missing"} · missing ${missing(row, judge.providerCount).join("/") || "none"}${row.error ? ` · error ${row.error}` : ""}`);
  console.log(`checks: ${judge.checks.map((item) => `${item.id}:${item.score}`).join(" · ")}\n`);
}
console.log("PASS442 invariant: do not release stronger PDF/chat text when the new payload regresses against previous guard layers.");
