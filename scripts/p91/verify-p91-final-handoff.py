...
def main():
 ...
 check('ledger_exists',ledger.is_file());ledger_text=ledger.read_text();check('ledger_p91','P91R1' in ledger_text);check('ledger_zero_final','Customer FINAL: 0/20' in ledger_text);check('ledger_package_hash',pv['final']['sha256'] in ledger_text);check('ledger_rights','Rights: 2/203 inherited only' in ledger_text);check('ledger_no_go','NO_GO / STOP_SELL' in ledger_text)
 ...
 with zipfile.ZipFile(source) as z:
  infos=z.infolist();names=[x.filename for x in infos];name_set=set(names);check('zip_entries',len(infos)==pv['final']['entryCount'],len(infos));check('zip_crc',z.testzip() is None);check('zip_sorted',names==sorted(names));check('zip_no_dirs',not any(x.is_dir() or x.filename.endswith('/') for x in infos));check('zip_timestamp',all(x.date_time==(1980,1,1,0,0,0) for x in infos));check('zip_create_system',all(x.create_system==0 for x in infos));check('zip_master',master.name in name_set);check('zip_v17',v17.name in name_set);check('zip_no_ledger',ledger.name not in name_set);check('zip_identity','artifacts/closure/p91r1/P91R1_TREE_IDENTITY_EXCLUDING_SELF.json' in name_set);check('zip_checkpoint','artifacts/closure/p91r1/P91R1_CHECKPOINT_RECEIPT.json' in name_set);check('zip_risk_boundary','artifacts/closure/p91r1/P91R1_RISK_HISTORY_BOUNDARY.json' in name_set);check('zip_failure_adjudication','artifacts/closure/p91r1/P91R1_FAILURE_ADJUDICATION.json' in name_set);check('zip_migration','supabase/migrations/20260820000006_p91_risk_history_event_driven_versioned_durable_truth.sql' in name_set)
 ...
 failed=[x for x in checks if x['status']=='FAIL'];payload={'schemaVersion':'velmere.p91r1.final-four-file-verification.v1','status':'PASS' if not failed else 'FAIL','checks':{'total':len(checks),'passed':len(checks)-len(failed),'failed':len(failed),'rows':checks},'files':{'master':{'path':str(master),'bytes':master.stat().st_size,'sha256':sha(master)},'v17':{'path':str(v17),'bytes':v17.stat().st_size,'sha256':sha(v17)},'ledger':{'path':str(ledger),'bytes':ledger.stat().st_size,'sha256':sha(ledger)},'sourceOnly':{'path':str(source),'bytes':source.stat().st_size,'sha256':sha(source),'entries':pv['final']['entryCount']}},'truthBoundary':'Four-file handoff identity and package integrity only; no product FINAL, real database, provider rights, deployment or exact-Windows promotion.'}
