...
IDENTITY_REL='artifacts/closure/p91r1/P91R1_TREE_IDENTITY_EXCLUDING_SELF.json'
...
def clean_unpack(zip_path:Path,rows):
 with tempfile.TemporaryDirectory(prefix='p91-clean-') as td:
  target=Path(td)
...
def main():
 ...
 current_binaries=[x['path'] for x in rows if x['path'].startswith(('receipts/p91/','artifacts/p91/','scripts/p91/')) and Path(x['path']).suffix.lower() in {'.pdf','.zip','.woff','.woff2','.ttf','.otf','.exe','.dll','.bin','.pyc','.pyo'}]
 if current_binaries:raise RuntimeError(f'unexpected_current_binary:{current_binaries}')
 with tempfile.TemporaryDirectory(prefix='p91-package-') as td:
