...
const policyPath = path.join(root, "config", "pass17", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS17 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass17-gates");
...
console.log(`PASS17 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass17.gate-execution.v1",
...
console.log(`PASS17 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS17 gate receipt: ${path.relative(root, receiptPath)}`);
