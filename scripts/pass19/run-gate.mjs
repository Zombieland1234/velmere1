...
const policyPath = path.join(root, "config", "pass19", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS19 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass19-gates");
...
console.log(`PASS19 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass19.gate-execution.v1",
...
console.log(`PASS19 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS19 gate receipt: ${path.relative(root, receiptPath)}`);
