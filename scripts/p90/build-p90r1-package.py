...
IDENTITY_REL='artifacts/closure/p90r1/P90R1_TREE_IDENTITY_EXCLUDING_SELF.json'
...
def scan(root,rows):
 ...
 for x in rows:
  ...
  for name,pat in TOKEN_PATTERNS.items():
   if pat.search(data):findings.append({'path':x['path'],'pattern':name})
 return findings
...
def identical(a,b):
 ...
 with a.open('rb') as x,b.open('rb') as y:
  while True:
   ca=x.read(8*1024*1024);cb=y.read(8*1024*1024)
   if ca!=cb:return False
   if not ca:return True
def verify_zip(p,rows):
 expected=[x['path'] for x in rows];m={x['path']:x for x in rows}
 with zipfile.ZipFile(p) as z:
  ...
  if any(i.file_size!=m[i.filename]['byteLength'] for i in infos):raise RuntimeError('size')
  bad=z.testzip()
  if bad:raise RuntimeError(f'crc:{bad}')
 return {'entryCount':len(rows),'bytes':p.stat().st_size,'sha256':sha(p),'crc':'PASS','ordering':'lexicographic','timestamp':'1980-01-01T00:00:00Z','directoryEntries':0,'createSystem':0,'externalMode':'0600'}
def clean_unpack(zp,rows):
 with tempfile.TemporaryDirectory(prefix='p90-clean-') as td:
  t=Path(td)
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--root',required=True);ap.add_argument('--output',required=True);ap.add_argument('--verification',required=True);a=ap.parse_args();root=Path(a.root).resolve();output=Path(a.output).resolve();ver=Path(a.verification).resolve()
 ...
 current_binary=[x['path'] for x in rows if (x['path'].startswith('receipts/p90/') or x['path'].startswith('artifacts/p90/') or x['path'].startswith('scripts/p90/')) and Path(x['path']).suffix.lower() in {'.pdf','.zip','.woff','.woff2','.ttf','.otf','.exe','.dll','.bin','.pyc'}]
 if current_binary:raise RuntimeError(f'unexpected_current_binary:{current_binary}')
 with tempfile.TemporaryDirectory(prefix='p90-package-') as td:
  x=Path(td)/'a.zip';y=Path(td)/'b.zip';build_zip(root,x,rows);build_zip(root,y,rows)
  if not identical(x,y):raise RuntimeError('not_deterministic')
  va=verify_zip(x,rows);vb=verify_zip(y,rows);clean=clean_unpack(x,rows);output.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(x,output)
 final={**va,'sha256':sha(output),'cleanUnpack':clean}
 if final['sha256']!=va['sha256'] or output.stat().st_size!=va['bytes']:raise RuntimeError('final_copy')
 payload={'schemaVersion':'velmere.p90r1.deterministic-package-verification.v1','status':'PASS','output':output.name,'deterministicRebuild':'2/2 BYTE_IDENTICAL','buildA':va,'buildB':vb,'final':final,'treeIdentity':projection(rows),'packageContentManifest':{'path':MANIFEST_REL,'sha256':sha(root/MANIFEST_REL),'listedFiles':len(manifest),'verifiedExact':True},'privateKeySecretScan':{'status':'PASS','matches':0},'unexpectedCurrentBinaryScan':{'status':'PASS','matches':0},'ledgerInsideZip':False,'identityReceiptExcludesOnlySelf':True,'truthBoundary':'External verification binds final ZIP bytes after two deterministic builds, CRC of both, exact package-manifest validation, full secret scan and clean-unpack replay. It grants no real providers, rights, database/staging, deployed HTTP, Customer FINAL or exact-Windows credit.'}
 ver.parent.mkdir(parents=True,exist_ok=True);ver.write_text(json.dumps(payload,indent=2)+'\n');print(json.dumps(payload,indent=2))
