...
LOGDIR=ROOT/'artifacts/p90/logs/regression/segment-b'
OUT=ROOT/'receipts/p90/P90_CURRENT_REGRESSION_SEGMENT_B.json'
...
COMMANDS=[
 ...
 ('P77_DETERMINISTIC_DELIVERY_STATIC',['python3','scripts/p81/test-p77-deterministic-delivery-current-static.py','--source-root',str(ROOT),'--receipt',str(ROOT/'receipts/p90/P90_P77_DETERMINISTIC_DELIVERY_CURRENT_STATIC_REGRESSION.json')],23,[r'"checkCount": 23',r'"status": "PASS"']),
...
payload={'schemaVersion':'velmere.p90.current-regression-segment-b.v1','generatedAt':'2026-08-20T21:15:00.000Z','status':status,'commandsPassed':sum(x['status']=='PASS' for x in rows),'commandsExpected':len(COMMANDS),'aggregateExecutedChecksAcrossOverlappingHarnesses':sum(x['checks'] for x in rows if x['status']=='PASS'),'rows':rows,'truthBoundary':'Process-group-bounded current-byte segment. Counts overlap and grant no provider, rights, staging, deployment, Customer FINAL or exact-Windows credit.'}
