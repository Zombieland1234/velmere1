...
ART = ROOT / "artifacts/closure/p33"
MANIFEST = ART / "P33_SOURCE_MANIFEST.json"
EXCLUSIONS = ART / "P33_PACKAGE_EXCLUSIONS.json"
...
def build_manifest(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "schemaVersion": "velmere.p33.source-only-package-manifest.v1",
        ...
        "entries": rows,
        "truthBoundary": "The embedded manifest excludes itself. SOURCE_ONLY includes code/configs and compact P33 receipts, but excludes dependency trees, build outputs, raw repeated logs, generated PDF/raster corpora, external QA fonts and staging/external evidence. It grants no GO_PAID or production credit.",
    }
...
def main() -> int:
    ...
    EXCLUSIONS.write_text(json.dumps({
        "schemaVersion": "velmere.p33.source-only-package-exclusions.v1",
        "excluded": EXCLUDE_PATTERNS,
        "reason": "Keep current code and compact receipts while excluding dependencies, generated build/browser/PDF corpora, duplicate raw test logs, SQLite fixture state and external QA inputs.",
        "externalExactFontShipped": False,
    }, indent=2) + "\n", "utf-8")
    rows = inventory()
    manifest = build_manifest(rows)
    MANIFEST.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", "utf-8")
    # Re-inventory so the exclusions file and all compact receipts are included; manifest remains self-excluded.
    rows = inventory()
    manifest = build_manifest(rows)
    MANIFEST.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", "utf-8")
    out = Path(args.output).resolve(); copy_b = Path(args.copy_b).resolve()
    write_zip(out, rows); write_zip(copy_b, rows)
    sha_a = digest_file(out); sha_b = digest_file(copy_b)
    with zipfile.ZipFile(out) as archive:
        bad = archive.testzip(); entries = len([i for i in archive.infolist() if not i.is_dir()]); uncompressed = sum(i.file_size for i in archive.infolist() if not i.is_dir())
    receipt = {
        "schemaVersion": "velmere.p33.deterministic-package-receipt.v1",
        "state": "CURRENT_SOURCE_ONLY_IN_PROGRESS",
        "archive": str(out),
        "copyB": str(copy_b),
        "sha256": sha_a,
        ...
        "truthBoundary": "Deterministic source packaging and CRC do not grant exact dependency, production build, Browser, staging, provider-rights, customer-value, GO_PAID or external proof credit.",
    }
    receipt_path = Path(args.receipt).resolve(); receipt_path.parent.mkdir(parents=True, exist_ok=True); receipt_path.write_text(json.dumps(receipt, indent=2) + "\n", "utf-8")
