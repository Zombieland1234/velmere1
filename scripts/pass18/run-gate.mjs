...
const policyPath = path.join(root, "config", "pass18", "gate-policy.json");
...
if (heavy && !args.allowHeavy) {
  console.error(`PASS18 ${args.level} gate is intentionally blocked without --allow-heavy.`);
...
const receiptDirectory = path.join(root, ".velmere", "pass18-gates");
...
console.log(`PASS18 gate level=${args.level}${args.domain ? ` domain=${args.domain}` : ""}`);
...
const receipt = {
  schemaVersion: "velmere.pass18.gate-execution.v1",
...
console.log(`PASS18 gate sourceImmutable=${sourceImmutable}`);
console.log(`PASS18 gate receipt: ${path.relative(root, receiptPath)}`);
