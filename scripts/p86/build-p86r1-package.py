#!/usr/bin/env python3
"""Build and independently verify deterministic P86R1 SOURCE_ONLY ZIP bytes."""
...
def verify_one_clean_unpack(zip_path: Path, expected_rows: list[dict[str, Any]]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p86r1-clean-unpack-") as td:
        target = Path(td)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="p86r1-package-build-") as td:
    ...
    payload = {
        "schemaVersion": "velmere.p86r1.deterministic-package-verification.v1",
        "status": "PASS",
        "output": output.name,
        ...
        "truthBoundary": "External verification binds final ZIP bytes after two deterministic builds, CRC of both and one clean-unpack path/content replay. It grants no PostgreSQL/Supabase runtime, JWT/RLS proof, deployed HTTP parity, current deployment fact, exploitability, rights expansion, Customer FINAL or Audit FINAL PDF credit.",
    }
