#!/usr/bin/env python3
"""Build and independently verify deterministic P84R1 SOURCE_ONLY ZIP bytes."""
...
def verify_one_clean_unpack(zip_path: Path, expected_rows: list[dict[str, Any]]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p84r1-clean-unpack-") as td:
        target = Path(td)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="p84r1-package-build-") as td:
    ...
    payload = {
        "schemaVersion": "velmere.p84r1.deterministic-package-verification.v1",
        "status": "PASS",
        "output": output.name,
