...
const OUT = path.join(ROOT, "artifacts/r5/VELMERE_R5_CURRENT_EXECUTION_CAMPAIGN.json");
...
const payload = {
  schemaVersion: "velmere.r5.local-current-execution-campaign.v1",
  ...
  offlineDependencyEvidence: {
    ...
    customerOwnedMarketEvidenceAuthority: {
      authorityPath: "lib/market-integrity/customer-owned-market-evidence-authority.ts",
      routePath: "lib/server/market-integrity-route-modules/customer-owned-market-evidence.ts",
      testPath: "scripts/current-execution/test-customer-owned-market-impact-attested-route.mts",
      authoritySha256: sha256(fs.readFileSync(path.join(ROOT, "lib/market-integrity/customer-owned-market-evidence-authority.ts"))),
      routeSha256: sha256(fs.readFileSync(path.join(ROOT, "lib/server/market-integrity-route-modules/customer-owned-market-evidence.ts"))),
      testSha256: sha256(fs.readFileSync(path.join(ROOT, "scripts/current-execution/test-customer-owned-market-impact-attested-route.mts"))),
      providerNetworkCallsExpected: 0,
  ...
  classification: failures.length === 0
    ...
    : "FAIL_LOCAL_CAMPAIGN",
  truthBoundary: "Local current-source execution only. R5 adds an account-bound, asset-bound and snapshot-bound signed customer-owned Market Impact evidence route with zero provider network calls, no live/risk/FINAL promotion and provider modes still fail-closed. Exact React/ReactDOM/TypeScript/PGlite dependencies, authorized staging/runtime services, durable receipt storage/replay controls, attributable rights review and exact Windows remain WITHHELD; no row-level Customer FINAL is granted.",
};
